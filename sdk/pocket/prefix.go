...
// (see pockets/README.md): a host can relocate a pocket under e.g. "/blog"
// without the pocket's own Register knowing or cooperating, because the
// pocket only ever sees the RouteRegistrar it was handed.
//
// PrefixRegistrar only changes the path a handler is registered under. It does
// not rewrite anything the pocket itself renders — links, redirects, form
// actions. A pocket whose views build absolute paths (e.g. a hardcoded "/")
// will still point at the un-prefixed root; see pockets/README.md's
...
// per pocket it wants to relocate and passes it as Mount.Router instead of
// its bare router.
...
// Mount.Router; the pocket's Register never learns it was grouped. This is
...
// patterns Go 1.22+ ServeMux uses for a pocket's root route all collapse to
