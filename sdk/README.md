...
| `workers` | facility: worker pool (adaptive polling, coalesced wake channel, middleware, panic recovery, graceful drain) + generic `Runner[T Job]` (claim → hooks → process → complete/fail); first consumer is `pockets/jobs`' runtime |
