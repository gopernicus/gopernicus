...
// DecodeCursor decodes a cursor token and validates it matches the expected
// order field. Returns nil (not an error) when the token is empty (first page)
// or its order field does not match (stale cursor, treat as first page).
// Returns an error only for malformed tokens.
