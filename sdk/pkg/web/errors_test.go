...
func TestValidationResponse_WithoutFieldCodes(t *testing.T) {
	var ve sdk.ValidationError
	ve.Add("email", "", "is required")
	ve.Add("password", "", "too short")
	err := fmt.Errorf("request validation: %w", ve.Err())

	for name, mapper := range map[string]func(error) *Error{
		"ErrValidation": ErrValidation,
		"ErrFromDomain": ErrFromDomain,
	} {
		t.Run(name, func(t *testing.T) {
			w := httptest.NewRecorder()
			RespondJSONError(w, mapper(err))
			...
			want := `{"message":"validation failed","code":"validation_failed","fields":[{"field":"email","message":"is required"},{"field":"password","message":"too short"}]}` + "\n"
...
func TestErrorBodies_UnchangedWhenNewFieldsAreZero(t *testing.T) {
	tests := []struct {
		...
		{
			"ValidationWithoutCode",
			ErrValidation(sdk.Refuse("name", "", "name is required")),
