...
func TestReadBody_StructuralFailures(t *testing.T) {
	tests := []struct {
		...
		{"TrailingArrayCloser", `{"title":"a"}]`, "request body must contain exactly one JSON object"},
		{"TrailingObjectCloser", `{"title":"a"}}`, "request body must contain exactly one JSON object"},
	}
...
// ExampleReadBody shows reading presence and typed values before checking errors.
