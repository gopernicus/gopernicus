...
// sdk/pkg/web.ServerConfig exactly. web must not import environment and
...
func TestParseEnvTags_NamedStringSlices(t *testing.T) {
	type modes []Mode
	var cfg struct {
		Modes modes `env:"MODES" separator:";"`
	}
	t.Setenv("GOPERNICUS_TEST_MODES", "development; production")
	if err := ParseEnvTags("GOPERNICUS_TEST", &cfg); err != nil {
		t.Fatal(err)
	}
	if !slices.Equal(cfg.Modes, modes{ModeDevelopment, ModeProduction}) {
		t.Fatalf("Modes = %v", cfg.Modes)
	}
}

func TestParseEnvTags_UnsupportedTypesRegardlessOfValue(t *testing.T) {
	for _, cfg := range []any{
		&struct {
			Count uint `env:"VALUE"`
		}{},
		&struct {
			Child struct{} `env:"VALUE"`
		}{},
		&struct {
			Values []int `env:"VALUE"`
		}{},
		&struct {
			Pointer *string `env:"VALUE"`
		}{},
		&struct {
			Interface any `env:"VALUE"`
		}{},
	} {
		for _, present := range []bool{false, true} {
			t.Run(fmt.Sprintf("%T/present=%t", cfg, present), func(t *testing.T) {
				const key = "GOPERNICUS_TEST_UNSUPPORTED_VALUE"
				t.Setenv(key, "dummy")
				if !present {
				...
				if err := ParseEnvTags("GOPERNICUS_TEST_UNSUPPORTED", cfg); err == nil || !strings.Contains(err.Error(), key) {
					t.Fatalf("expected located unsupported type error, got %v", err)
				}
			})
		}
	}
...
func TestParseEnvTags_OverflowLeavesValueUnchanged(t *testing.T) {
	cfg := struct {
		Rate float32 `env:"RATE"`
	}{Rate: 2.5}
	t.Setenv("GOPERNICUS_TEST_RATE", "1e40")
	if err := ParseEnvTags("GOPERNICUS_TEST", &cfg); !errors.Is(err, strconv.ErrRange) {
		t.Fatalf("expected range error, got %v", err)
	}
	if cfg.Rate != 2.5 {
		t.Fatalf("overflow changed Rate to %v", cfg.Rate)
	}
	count := struct {
		Count int `env:"COUNT"`
	}{Count: 7}
	...
	t.Setenv("GOPERNICUS_TEST_COUNT", tooLarge)
	if err := ParseEnvTags("GOPERNICUS_TEST", &count); !errors.Is(err, strconv.ErrRange) {
		t.Fatalf("expected int range error, got %v", err)
	}
	if count.Count != 7 {
		t.Fatalf("overflow changed Count to %v", count.Count)
	}
}

func TestParseEnvTags_DiagnosticsOmitValuesAndLocateNestedFields(t *testing.T) {
	const dummy = "DUMMY_VALUE_NOT_A_SECRET"
	for _, cfg := range []any{
		&struct {
			Child struct {
		...
		t.Run(fmt.Sprintf("%T", cfg), func(t *testing.T) {
			t.Setenv("GOPERNICUS_TEST_INVALID_VALUE", dummy)
			err := ParseEnvTags("GOPERNICUS_TEST_INVALID", cfg)
			if err == nil {
				t.Fatal("expected parse failure")
			}
			for _, fragment := range []string{"GOPERNICUS_TEST_INVALID_VALUE", "Child.Value", "type "} {
				if !strings.Contains(err.Error(), fragment) {
					t.Errorf("error lacks %q: %v", fragment, err)
				}
			}
			for cause := err; cause != nil; cause = errors.Unwrap(cause) {
				if strings.Contains(cause.Error(), dummy) {
					t.Errorf("error contains rejected value: %v", cause)
				}
			}
			var number *strconv.NumError
			if errors.As(err, &number) {
				t.Error("error retains input-bearing NumError")
			}
		})
	}
}

func TestParseEnvTags_ZeroPreseedStillUsesDefaults(t *testing.T) {
	cfg := struct {
		Enabled bool     `env:"ENABLED" default:"true"`
		Count   int      `env:"COUNT" default:"3"`
		Names   []string `env:"NAMES" default:"fallback"`
	}{Enabled: false, Count: 0, Names: []string{}}
	for _, key := range []string{"ENABLED", "COUNT", "NAMES"} {
		t.Setenv("GOPERNICUS_TEST_ZERO_"+key, "")
	}
	if err := ParseEnvTags("GOPERNICUS_TEST_ZERO", &cfg); err != nil {
		t.Fatal(err)
	}
	if !cfg.Enabled || cfg.Count != 3 || !slices.Equal(cfg.Names, []string{"fallback"}) {
		t.Fatalf("zero preseed precedence changed: %+v", cfg)
	}
}
