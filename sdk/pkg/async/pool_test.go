...
func TestPool_Close_WaitsForInFlight(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		select {
		case err := <-closed:
			t.Fatalf("Close returned before its task finished: %v", err)
		default:
		}
		close(release)
...
func TestPool_NonPositiveConcurrency_Defaults(t *testing.T) {
	pool := NewPool(WithLogger(quietLogger()), WithMaxConcurrency(0))
	defer pool.Close(context.Background())

	if pool.config.maxConcurrency != 100 {
		t.Errorf("maxConcurrency = %d, want fallback 100", pool.config.maxConcurrency)
	}
}

func TestNewPoolRejectsNilOption(t *testing.T) {
	defer func() {
		if got := recover(); got != "async.NewPool: nil option" {
