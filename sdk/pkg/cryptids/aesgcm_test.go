...
// Use the old nonce-explicit API to pin compatibility with stored envelopes.
func TestAESGCMLegacyEnvelopeCompatibility(t *testing.T) {
	...
	current, err := NewAESGCM(key)
	if err != nil {
		t.Fatal(err)
	}
	const plaintext = "persisted OAuth payload é"
	nonce := bytes.Repeat([]byte{0x17}, legacy.NonceSize()) // Test fixture only.
	oldEnvelope := legacy.Seal(nonce, nonce, []byte(plaintext), nil)
	got, err := current.Decrypt(base64.RawURLEncoding.EncodeToString(oldEnvelope))
	if err != nil || got != plaintext {
		t.Fatalf("read legacy ciphertext: %q, %v", got, err)
	}
	encoded, err := current.Encrypt(plaintext)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(envelope) != len(plaintext)+12+16 {
		t.Fatalf("envelope length = %d", len(envelope))
	}
	decoded, err := legacy.Open(nil, envelope[:12], envelope[12:], nil)
	if err != nil || string(decoded) != plaintext {
		t.Fatalf("legacy reader of new ciphertext: %q, %v", decoded, err)
	}
	wrongKey, err := NewAESGCM(bytes.Repeat([]byte{0x43}, 32))
	if err != nil {
		t.Fatal(err)
	}
	if _, err := wrongKey.Decrypt(encoded); err == nil {
		t.Fatal("wrong key accepted")
	}
	envelope[len(envelope)-1] ^= 1
	if _, err := current.Decrypt(base64.RawURLEncoding.EncodeToString(envelope)); err == nil {
		t.Fatal("tampered authentication tag accepted")
	}
}

func TestAESGCMShortInputClassification(t *testing.T) {
	enc := newTestAESGCM(t)
	for size := 1; size < 28; size++ {
		_, err := enc.Decrypt(base64.RawURLEncoding.EncodeToString(make([]byte, size)))
		if err == nil || errors.Is(err, sdk.ErrInvalidInput) != (size < 12) {
			t.Errorf("%d-byte envelope: %v; want error, invalid-input only below 12", size, err)
		}
	}
}
