...
func TestRunner_BasicLifecycleCompletes(t *testing.T) {
	...
	process := func(ctx context.Context, job fakeJob) error {
		job.status = "done"
		return nil
	}
	runner := NewRunner[fakeJob](store, process, WithRunnerLogger(silentLogger()), WithMaxAttempts(1))
...
func TestRunner_RetryThenFailAtMaxAttempts(t *testing.T) {
	...
	runner := NewRunner[fakeJob](store, process, WithRunnerLogger(silentLogger()), WithMaxAttempts(3))
...
func TestRunner_RetryThenSucceed(t *testing.T) {
	...
	runner := NewRunner[fakeJob](store, process, WithRunnerLogger(silentLogger()), WithMaxAttempts(5))
...
func TestRunner_EmptyClaimPropagatesNoWork(t *testing.T) {
	...
	process := func(ctx context.Context, job fakeJob) error { return nil }
	runner := NewRunner[fakeJob](store, process, WithRunnerLogger(silentLogger()))
...
func TestRunner_CustomClockReachesStore(t *testing.T) {
	...
	process := func(ctx context.Context, job fakeJob) error { return nil }
	runner := NewRunner[fakeJob](clockStore, process, WithRunnerLogger(silentLogger()), WithClock(func() time.Time { return fixed }))
...
func TestRunner_WorkerIDReachesStore(t *testing.T) {
	...
	process := func(ctx context.Context, job fakeJob) error { return nil }
	runner := NewRunner[fakeJob](spy, process, WithRunnerLogger(silentLogger()))
...
func TestRunner_DefaultMaxAttempts(t *testing.T) {
	...
	process := func(ctx context.Context, job fakeJob) error { return errors.New("fail") }
	runner := NewRunner[fakeJob](store, process, WithRunnerLogger(silentLogger())) // no WithMaxAttempts
...
func TestRunnerLoggerOptions(t *testing.T) {
	var defaultLogs, customLogs bytes.Buffer
	previous := slog.Default()
	slog.SetDefault(slog.New(slog.NewTextHandler(&defaultLogs, nil)))
	t.Cleanup(func() { slog.SetDefault(previous) })
	logger := slog.New(slog.NewTextHandler(&customLogs, nil))
	process := func(context.Context, fakeJob) error { return nil }
	for _, tc := range []struct {
		name   string
		runner []RunnerOption
		fenced []FencedRunnerOption
		custom bool
	}{
		{name: "default"},
		{name: "last logger wins", runner: []RunnerOption{WithRunnerLogger(silentLogger()), WithRunnerLogger(logger)}, fenced: []FencedRunnerOption{WithFencedLogger(silentLogger()), WithFencedLogger(logger)}, custom: true},
		{name: "nil selects default", runner: []RunnerOption{WithRunnerLogger(logger), WithRunnerLogger(nil)}, fenced: []FencedRunnerOption{WithFencedLogger(logger), WithFencedLogger(nil)}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			defaultLogs.Reset()
			customLogs.Reset()
			store := newMemStore(fakeJob{id: "plain"})
			if err := NewRunner(store, process, tc.runner...).WorkFunc()(context.Background()); err != nil {
				t.Fatal(err)
			}
			fenced := newFencedFakeStore()
			fenced.enqueue("fenced", time.Time{})
			if err := NewFencedRunner(fenced, process, tc.fenced...).WorkFunc()(context.Background()); err != nil {
				t.Fatal(err)
			}
			used, unused := &defaultLogs, &customLogs
			if tc.custom {
				used, unused = unused, used
			}
			if unused.Len() != 0 {
				t.Fatalf("unselected logger received output: %s", unused)
			}
			for _, want := range []string{"job completed", "fenced job completed", "job_id=plain", "job_id=fenced"} {
				if !strings.Contains(used.String(), want) {
					t.Errorf("logs missing %q: %s", want, used)
				}
			}
			if !store.isCompleted("plain") || fenced.snapshot("fenced").status != "completed" {
				t.Fatal("runner did not complete its job")
			}
		})
	}
}

func TestWorkerConstructorsRejectNilOptions(t *testing.T) {
	process := func(context.Context, fakeJob) error { return nil }
	for _, tc := range []struct {
		name      string
		construct func()
	}{
		{"workers.NewPool", func() { NewPool(func(context.Context) error { return nil }, nil) }},
		{"workers.NewRunner", func() { NewRunner(newMemStore(), process, nil) }},
		{"workers.NewFencedRunner", func() { NewFencedRunner(newFencedFakeStore(), process, nil) }},
	} {
		t.Run(tc.name, func(t *testing.T) {
			defer func() {
