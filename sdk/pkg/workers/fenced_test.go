...
func TestFencedRunner_CurrentOwnerCompletes(t *testing.T) {
	...
	runner := NewFencedRunner[fakeJob](store, process, WithFencedLogger(silentLogger()),
...
func TestFencedRunner_ProcessFailureFailsUnderLease(t *testing.T) {
	...
	process := func(ctx context.Context, job fakeJob) error { return errors.New("boom") }
	runner := NewFencedRunner[fakeJob](store, process, WithFencedLogger(silentLogger()),
		WithLeaseTokenFunc(func() string { return "L1" }))
...
func TestFencedRunner_RetryReschedulesInsteadOfDeadLetter(t *testing.T) {
	...
	deadLettered := false
	process := func(ctx context.Context, job fakeJob) error { return errors.New("transient") }
	runner := NewFencedRunner[fakeJob](store, process, WithFencedLogger(silentLogger()),
		WithLeaseTokenFunc(func() string { return "L1" }),
		WithFencedClock(func() time.Time { return fixed }),
		WithFencedRetryDecider(func(_ error, attempt int) (time.Duration, bool) { return time.Minute, attempt < 3 }))
...
func TestFencedRunner_RetryDeciderDeadLettersPermanentImmediately(t *testing.T) {
	...
	process := func(ctx context.Context, job fakeJob) error { return permanent }
	runner := NewFencedRunner[fakeJob](store, process, WithFencedLogger(silentLogger()),
		WithLeaseTokenFunc(func() string { return "L1" }),
	...
	fixed := time.Date(2026, 7, 13, 12, 0, 0, 0, time.UTC)
	process2 := func(ctx context.Context, job fakeJob) error { return errors.New("transient") }
	runner2 := NewFencedRunner[fakeJob](store2, process2, WithFencedLogger(silentLogger()),
		WithLeaseTokenFunc(func() string { return "L1" }),
		WithFencedClock(func() time.Time { return fixed }),
		WithFencedRetryDecider(func(err error, attempt int) (time.Duration, bool) {
			if errors.Is(err, permanent) {
...
func TestFencedRunner_RetryExhaustionDeadLetters(t *testing.T) {
	...
	hookRan := false
	process := func(ctx context.Context, job fakeJob) error { return errors.New("permanent") }
	runner := NewFencedRunner[fakeJob](store, process, WithFencedLogger(silentLogger()),
		WithLeaseTokenFunc(func() string { return "L1" }),
		WithFencedRetryDecider(func(_ error, attempt int) (time.Duration, bool) { return time.Minute, false }))
...
func TestFencedRunner_ProcessTimeoutCancelledPromptly(t *testing.T) {
	...
	ctx, cancel := context.WithCancel(context.Background())
	process := func(pctx context.Context, job fakeJob) error {
