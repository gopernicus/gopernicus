...
// name, the active worker count, and the iteration, successful-iteration, and error counts
...
// dropped signal missed. A nil channel disables wakeups — the pool behaves
...
// public snapshot plus the private successes total it reports as a delta.
...
// (1 worker, 5s poll, 30s idle). A nil option panics.
func NewPool(work WorkFunc, opts ...PoolOption) *Pool {
	...
	for _, opt := range opts {
		if opt == nil {
			panic("workers.NewPool: nil option")
		}
		opt(c)
	}
...
// done, and waits for in-flight calls to return. Their contexts are canceled;
// cancellation is cooperative. Run returns the first local ErrPoolShutdown,
// or nil on normal shutdown. Each Pool can run once; later calls return ErrAlreadyRun.
...
func (p *Pool) worker(ctx context.Context, stop func(error), workerID string) {
	defer p.wg.Done()
	...
	for {
		...
		case errors.Is(err, ErrPoolShutdown):
			...
			stop(fmt.Errorf("worker %s: %w", workerID, err))
			return
		case errors.Is(err, ErrWorkerShutdown):
			p.log.InfoContext(ctx, "worker shutting down as requested", "worker_id", workerID, "error", err)
			return
		}
		...
		if errors.Is(err, ErrNoWork) {
			...
			p.log.ErrorContext(ctx, "worker iteration failed", "pool", p.name, "worker_id", workerID, "error", err)
		}
		timer.Reset(next)
	}
}
...
// snapshot plus the private successes total.
func (p *Pool) heartbeatSnapshot() heartbeatCounters {
	return heartbeatCounters{stats: p.stats.snapshot(), successes: p.stats.successes.Load()}
}
...
func (p *Pool) runHeartbeat(ctx context.Context, previous heartbeatCounters, done chan<- struct{}) {
	...
	for {
		...
		p.log.InfoContext(ctx, "pool alive",
			...
			"successful_iterations", current.successes-previous.successes,
...
// panic is counted, logged with its stack, and returned as an error so
