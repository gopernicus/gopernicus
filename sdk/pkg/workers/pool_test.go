...
func TestPool_ErrPoolShutdown(t *testing.T) {
	...
	case err := <-done:
		if !errors.Is(err, ErrPoolShutdown) {
			t.Fatalf("Run = %v, want pool shutdown", err)
		}
	case <-time.After(2 * time.Second):
		t.Fatal("pool did not stop after ErrPoolShutdown")
	}
}
...
func TestPool_PanicRecoveryContained(t *testing.T) {
	...
	waitFor(t, time.Second, func() bool { return pool.Stats().Panics > 0 })
...
func assertBeat(t *testing.T, r slog.Record) (iterations, claims, errs int64) {
	...
	claims = intAttr(t, r, "successful_iterations")
