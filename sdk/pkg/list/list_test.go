...
func TestRequest_NormalizedLimit(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := Request{Limit: tt.limit}.NormalizedLimit(Limits{})
			if got != tt.want {
...
// TestRequest_NormalizedLimit_Limits proves the Limits resolution: the zero
// value falls back to the list constants, a Default-only or Max-only Limits
...
func TestRequest_NormalizedLimit_Limits(t *testing.T) {
	tests := []struct {
		name   string
		limit  int
		limits Limits
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := Request{Limit: tt.limit}.NormalizedLimit(tt.limits)
			if got != tt.want {
...
func TestRequest_Validate(t *testing.T) {
	tests := []struct {
		...
		{"empty_first_page", Request{}, false},
		{"cursor_default_strategy", Request{Cursor: "abc"}, false},
		{"cursor_strategy_first_page", Request{Strategy: StrategyCursor}, false},
		{"cursor_strategy_with_cursor", Request{Strategy: StrategyCursor, Cursor: "abc"}, false},
		{"cursor_strategy_with_offset", Request{Strategy: StrategyCursor, Offset: 20}, true},
		{"default_strategy_with_offset", Request{Offset: 20}, true},
		{"offset_strategy_zero", Request{Strategy: StrategyOffset, Offset: 0}, false},
		{"offset_strategy_positive", Request{Strategy: StrategyOffset, Offset: 20}, false},
		{"offset_strategy_with_cursor", Request{Strategy: StrategyOffset, Cursor: "abc"}, true},
		{"offset_strategy_negative", Request{Strategy: StrategyOffset, Offset: -1}, true},
		{"unknown_strategy", Request{Strategy: Strategy("sideways")}, true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := tt.req.Validate()
...
func TestRequest_ResolvedStrategy(t *testing.T) {
	tests := []struct {
		name string
		req  Request
		want Strategy
	}{
		{"empty_is_cursor", Request{}, StrategyCursor},
		{"explicit_cursor", Request{Strategy: StrategyCursor}, StrategyCursor},
		{"explicit_offset", Request{Strategy: StrategyOffset}, StrategyOffset},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
