...
// TestParseRequestSearch pins the transport-edge parsing rules: a term is
...
func TestParseRequestSearch(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			req, err := ParseRequest(Params{Search: tt.param})
			if err != nil {
				t.Fatalf("ParseRequest(Search=%q) error = %v; a search term's CONTENTS are never invalid", tt.param, err)
			}
...
// TestParseRequestSearchIsOrthogonal proves search does not disturb the
// existing page/strategy/count parsing.
func TestParseRequestSearchIsOrthogonal(t *testing.T) {
