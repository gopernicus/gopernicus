...
// StreamWriter incrementally writes SSE events to an HTTP response. Unlike
// SSEStream (which drains a channel), StreamWriter gives the handler direct
// control over when and what to send.
//
// This enables the "respond with JSON or upgrade to a stream" pattern used by
// AI/LLM APIs, progress endpoints, and protocols like MCP Streamable HTTP:
//
...
// http.ResponseController, which reaches the real Flusher through middleware
// wrappers implementing Unwrap; an unflushable writer surfaces as an error on
// the first Send.
...
// Send writes a single SSE event to the stream. On the first call it sets the
// SSE headers and flushes them to the client (the lazy header write that lets a
// handler decide between a JSON response and a stream up to the first Send).
...
// AcceptsStream reports whether the client can accept an SSE stream by looking
// for text/event-stream in the Accept header.
