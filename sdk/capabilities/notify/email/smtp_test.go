...
// Constructor and MIME coverage. smtp_protocol_test.go exercises Send against
// owned loopback SMTP peers, including cancellation and DATA acknowledgement.
...
// Parse actual wire output and verify decoded content rather than obsolete bytes.
func TestBuildMessagePlainRoundTrip(t *testing.T) {
	msg := Message{From: "sender@example.com", To: []string{"a@example.com", "b@example.com"}, Subject: strings.Repeat("Café ☕ ", 30), Text: "Line one\n" + strings.Repeat("é", 1200)}
	...
	if parsed.Header.Get("Content-Transfer-Encoding") != "quoted-printable" {
		t.Fatal("missing transfer encoding")
	}
	body, err := io.ReadAll(quotedprintable.NewReader(parsed.Body))
	if err != nil {
		t.Fatal(err)
	}
	if string(body) != strings.ReplaceAll(msg.Text, "\n", "\r\n") {
		t.Fatalf("decoded body changed")
	}
	for _, line := range bytes.Split(raw, []byte("\r\n")) {
		if len(line) > 998 {
			t.Fatalf("oversized MIME line: %d", len(line))
		}
	}
