...
func registerLayoutHTML(t *testing.T, tr *templateRegistry, layoutType LayoutType, layer TemplateLayer, content string) {
	...
	if tr.layouts[layoutType] == nil {
		tr.layouts[layoutType] = make(map[TemplateLayer]*layoutPair)
	}
	tr.layouts[layoutType][layer] = &layoutPair{html: tmpl}
}
...
func renderBranded(t *testing.T, branding *Branding, layoutType LayoutType) (string, string) {
	...
	registerBrandingContent(t, tr)
	tr.setBranding(branding)

	html, text, err := tr.render(RenderRequest{Template: "test:body", Subject: "SubjectMarker", Layout: layoutType})
	if err != nil {
...
func TestAppLayoutOverrideWinsOverBundledLogo(t *testing.T) {
	...
	html, _, err := tr.render(RenderRequest{Template: "test:body", Layout: LayoutTransactional})
	if err != nil {
