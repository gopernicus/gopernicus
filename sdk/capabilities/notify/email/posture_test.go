...
// metadatalessSender is a Sender that does not implement notify.CapabilityReporter — a
...
type structuralSender struct{ caps notify.Capabilities }

func (structuralSender) Send(context.Context, Message) error { return nil }
func (s structuralSender) Capabilities() notify.Capabilities { return s.caps }
...
func TestInspectSender(t *testing.T) {
	tests := []struct {
		...
		sender       Sender
		wantDeclared bool
		wantCaps     notify.Capabilities
	}{
		{
			name:         "bundled Console declares development-only",
			sender:       NewConsole(slog.New(slog.NewTextHandler(io.Discard, nil))),
			wantDeclared: true,
			wantCaps:     notify.Capabilities{TransportSecurity: notify.TransportSecurityNone, DevelopmentOnly: true},
		},
		{
			name:         "bundled SMTP declares production-capable",
			sender:       NewSMTP(SMTPConfig{Host: "mail.example.com", Port: "587"}),
			wantDeclared: true,
			wantCaps:     notify.Capabilities{TransportSecurity: notify.TransportSecurityStartTLS, DevelopmentOnly: false},
		},
		{
			name:         "third-party TLS sender declares production-capable",
			sender:       structuralSender{caps: notify.Capabilities{TransportSecurity: notify.TransportSecurityTLS}},
			wantDeclared: true,
			wantCaps:     notify.Capabilities{TransportSecurity: notify.TransportSecurityTLS},
		},
		{
...
func TestCheckSender(t *testing.T) {
	...
	tls := structuralSender{caps: notify.Capabilities{TransportSecurity: notify.TransportSecurityTLS}}
	devThirdParty := structuralSender{caps: notify.Capabilities{TransportSecurity: notify.TransportSecurityNone, DevelopmentOnly: true}}
	...
	tests := []struct {
		...
		},
		{
			...
			wantErr: notify.ErrInsecureTransport, wantErrSubstring: "development-only", wantPostureKept: true,
		},
		{
		...
		},
		{
			...
			wantErr: notify.ErrInsecureTransport, wantErrSubstring: "no capability metadata",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			posture, err := notify.CheckTransport(tt.mode, tt.sender)

			if tt.wantErr != nil {
				if !errors.Is(err, tt.wantErr) {
					t.Fatalf("notify.CheckTransport() error = %v, want errors.Is %v", err, tt.wantErr)
				}
			...
			if err != nil {
				t.Fatalf("notify.CheckTransport() error = %v, want nil", err)
			}
...
func TestCheckSenderErrorIsCapabilityOwned(t *testing.T) {
	_, err := notify.CheckTransport(environment.ModeProduction, metadatalessSender{})
	if err == nil {
		t.Fatal("want error")
	}
