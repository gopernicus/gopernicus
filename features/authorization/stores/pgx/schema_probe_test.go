...
// suite (POSTGRES_TEST_DSN). They prove the AZ3-2.1 greenfield schema APPLIES
// cleanly and that iam_scopes and iam_mutations — which have no Go consumer yet —
// exist with their CHECK constraints ENFORCED live, not merely declared in text.
// Absent a DSN they skip loudly, like the conformance suite.
package pgx
...
func TestSchemaProbe(t *testing.T) {
	...
	rejects := []struct {
		...
		{"mutation invalid kind", `INSERT INTO iam_mutations (mutation_id, scope_kind, scope_type, scope_id, operation, payload_encoding, payload_digest, outcome, revision, schema_digest, created_at) VALUES ('m0000000000000000000000003', 'bogus', 'doc', 'd1', 'grant', 'v', 'd', 'applied', 1, 's', NOW())`},
		{"mutation non-persisted outcome", `INSERT INTO iam_mutations (mutation_id, scope_kind, scope_type, scope_id, operation, payload_encoding, payload_digest, outcome, revision, schema_digest, created_at) VALUES ('m0000000000000000000000004', 'resource', 'doc', 'd1', 'grant', 'v', 'd', 'semantic_conflict', 1, 's', NOW())`},
		{"mutation empty digest", `INSERT INTO iam_mutations (mutation_id, scope_kind, scope_type, scope_id, operation, payload_encoding, payload_digest, outcome, revision, schema_digest, created_at) VALUES ('m0000000000000000000000005', 'resource', 'doc', 'd1', 'grant', 'v', '', 'applied', 1, 's', NOW())`},
		{"relationship empty relation", `INSERT INTO iam_relationships (resource_type, resource_id, relation, subject_type, subject_id, created_at) VALUES ('doc', 'd', '', 'user', 'u', NOW())`},
