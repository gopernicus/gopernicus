...
// Live turso-specific mutation tests beyond the shared storetest suite. They prove
// two dialect properties the shared suite exercises only indirectly: an ABSENT
// scope anchor observed by a guard is recorded at revision 0 and materialized as a
// concrete revision-0 anchor in canonical order (so the anchor state matches the
// pgx sibling), and a guard panic rolls the whole BEGIN IMMEDIATE transaction back
// to a coarse infrastructure error with no relationship, revision, or receipt
// trace. They require TURSO_DATABASE_URL/TURSO_AUTH_TOKEN and skip loudly without
// them, like the conformance suite.
//
// Mechanical adaptation vs pgx (recorded honestly): the pgx TestMutationAbsentAnchorNoPhantom
// drives a live 0→1 change to the dependency scope FROM INSIDE the guard, because
// pgx locks anchors individually (SELECT ... FOR UPDATE) and a concurrent
// transaction can commit against a different anchor mid-guard. Under libSQL that is
// architecturally impossible AND would self-deadlock: BEGIN IMMEDIATE takes the
// single write intent for the whole apply, so no other writer can commit while the
// guarded transaction is open (a nested write inside the guard would block on the
// lock the outer transaction already holds). Full write serialization is strictly
// STRONGER than pgx's per-anchor locking — it precludes the phantom window by
// construction — so this test proves the RECORDING + canonical anchor
// materialization discipline (the parity the plan requires) deterministically, and
// the shared ConcurrentSingleWinner / ConcurrentStaleWriterStorm cases carry the
// live single-commit-winner proof.
package turso
...
func liveRepos(t *testing.T) (*tursodb.DB, authorization.Repositories) {
	t.Helper()
...
// TestMutationStormForensics is the turso SQL forensic the shared storetest
...
// on concrete-subject grants. Under BEGIN IMMEDIATE the storm writers serialize,
// so a gapless applied run and a single winner per revision are the required
// parity with the pgx sibling. It drives every grant to a terminal state before
...
func TestMutationStormForensics(t *testing.T) {
	...
	var total, distinctID, minRev, maxRev, distinctRev int64
	if err := db.QueryRow(ctx,
		`SELECT COUNT(*), COUNT(DISTINCT mutation_id), MIN(revision), MAX(revision), COUNT(DISTINCT revision)
		   FROM iam_mutations WHERE scope_kind='resource' AND scope_type='doc' AND scope_id='F' AND outcome='applied'`).
		Scan(&total, &distinctID, &minRev, &maxRev, &distinctRev); err != nil {
	...
	if minRev != 1 || maxRev != n || distinctRev != n {
		t.Fatalf("applied revision run not gapless: min=%d max=%d distinct=%d over %d rows, want 1..%d", minRev, maxRev, distinctRev, total, n)
	}
...
// revision anchor records revision 0 and that the repository materializes that
// scope's anchor as a concrete revision-0 row in canonical order before commit —
// the same anchor state the pgx sibling produces, so the guarded write commits on a
// consistent snapshot and a later writer to that scope becomes a detectable 0→1
// change (proven live by the shared stale-writer storm). Under BEGIN IMMEDIATE the
// dependency cannot change WITHIN the guarded transaction, so there is no phantom
// window to race against; this asserts the recording + canonical anchoring
// discipline that upholds the contract.
func TestMutationAbsentAnchorNoPhantom(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("guarded grant on a consistent snapshot must commit: %v", err)
	}
	if rcpt.Outcome != mutation.OutcomeApplied {
		t.Fatalf("guarded grant outcome = %q, want applied", rcpt.Outcome)
	}
	if observed != 0 {
		t.Fatalf("a guard observing an absent anchor must record revision 0, got %d", observed)
	}
	// The dependency scope's anchor was materialized as a concrete revision-0 row
	// (canonical-order lock set), matching the pgx sibling's anchor state.
	var rev int64
	if err := db.QueryRow(ctx,
		`SELECT revision FROM iam_scopes WHERE scope_kind = 'resource' AND scope_type = 'doc' AND scope_id = 'S'`).Scan(&rev); err != nil {
		t.Fatalf("the dependency anchor must be materialized at revision 0: %v", err)
	}
	if rev != 0 {
		t.Fatalf("the materialized dependency anchor must be revision 0, got %d", rev)
	}
	// The guarded write committed on the consistent snapshot.
	if ok, _ := repos.Relationships.CheckRelationExists(ctx, "doc", "M", "viewer", "user", "u3"); !ok {
		t.Fatalf("the guarded grant must have committed its row")
	}
}
...
func TestMutationGuardPanicRollsBack(t *testing.T) {
	...
	var n int
	if err := db.QueryRow(ctx,
		`SELECT COUNT(*) FROM iam_mutations WHERE mutation_id = ?`, string(id)).Scan(&n); err != nil {
