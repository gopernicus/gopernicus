...
-- cross-kind-coexistence case). SQLite ADD COLUMN with a constant default and
-- DROP INDEX are both supported, so no table rebuild is needed; dropping and
-- recreating the partial unique index is mandatory — the old key had no kind
-- column.
