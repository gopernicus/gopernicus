...
// NewContactHandlers constructs the contact handlers over svc.
func NewContactHandlers(svc messagingService) *ContactHandlers {
	return &ContactHandlers{svc: svc}
}

// Form renders the public contact form.
func (h *ContactHandlers) Form(w http.ResponseWriter, r *http.Request) {
	web.Render(r.Context(), w, http.StatusOK, views.ContactForm(views.ContactModel{}))
}
...
func (h *ContactHandlers) Submit(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := h.svc.Submit(r.Context(), model.Name, model.Email, model.Message); err != nil {
		if errors.Is(err, errs.ErrInvalidInput) {
			...
			web.Render(r.Context(), w, http.StatusBadRequest, views.ContactForm(model))
			return
		}
	...
	web.Render(r.Context(), w, http.StatusOK, views.ContactThanks())
}
...
func (h *ContactHandlers) Inquiries(w http.ResponseWriter, r *http.Request) {
	...
	web.Render(r.Context(), w, http.StatusOK, views.InquiriesList(items))
}
