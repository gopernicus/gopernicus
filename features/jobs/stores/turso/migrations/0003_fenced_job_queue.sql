-- The fenced/keyed durable queue (turso flavor of the postgres 0003, identical
...
-- order, which the claim ORDER BY and keyset pagination rely on); ints are
-- INTEGER. The status CHECK mirrors the job.Status vocabulary.
--
-- payload is BLOB, NOT text: the fenced checkpoint stores opaque encrypted
-- ciphertext that must round-trip BYTE-FOR-BYTE, including arbitrary non-UTF8
-- bytes a TEXT column would mangle. The conformance suite's
-- CheckpointWhileClaimCurrent case asserts a byte-exact non-UTF8 round-trip,
-- which only a binary column satisfies.
