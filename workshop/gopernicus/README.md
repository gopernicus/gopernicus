...
| `new pocket <name> --module <path> [--aggregate <agg>]` | a standalone charter-conformant pocket: FS2 socket, `domain/<agg>/` rim (entity, doc-pinned Storer, `order.go`), `<agg>svc`, `storetest/` with the six-case pagination family + `DBGeneratedIDOnEmpty`, public `memstore/`, and BOTH dialect store modules (boot probe, `ExportMigrations`, inline-id-DEFAULT migration, env-gated conformance). Monorepo-shaped targets get the manual registration checklist printed |
...
`make check` emits a host and a pocket into temp dirs, rewrites the
...
Known seam: G5 (`guard-pocket-core-sdk-only`) keeps a hardcoded pocket
