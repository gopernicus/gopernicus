...
Scaffold repository directories, `queries.sql` files, and `bridge.yml` files for
every table mapped to a domain in `gopernicus.yml`. Existing repos are skipped --
the command never overwrites files.
...
| Command | Scope |
|---|---|
| `gopernicus new repo auth/users` | Single table (queries.sql + bridge.yml). |
| `gopernicus boot repos auth` | All tables in the `auth` domain (queries.sql + bridge.yml each). |
