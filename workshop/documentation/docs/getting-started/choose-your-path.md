...
3. Review the [pocket catalog](../pockets/overview.md) and [integration catalog](../integrations/catalog.md).
...
Most application customization belongs here: choose implementations, configure pocket seams, add host routes, wrap a route registrar, implement a repository, or supply views.
...
## I am authoring a reusable pocket

A pocket is a library-shaped hexagon with a public rim and sealed interior.
...
| pocket SQL and migrations | pocket `stores/<dialect>` sibling |
| app-specific use case | host `internal/logic` |
| generated route/repository customization | explicit pocket seam, host adapter, or future generation tooling |

Workshop currently emits a host or starting pocket anatomy. Application, store, and client generation remain roadmap work; see the [Workshop overview](../workshop/overview.md) for the current command surface and planned additions.
