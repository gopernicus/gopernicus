...
`pockets/jobs` is a datastore-free durable queue and scheduling pocket built on `sdk/foundation/workers`. It provides ordinary jobs and schedules plus a hardened fenced queue that implements the SDK keyed-work protocol.
...
The public jobs `Service` implements `work.Enqueuer`, `work.Replacer`, and `work.StatusReader`. Consuming pockets depend on SDK interfaces, never on jobs. Executor callbacks and rich job aggregates remain host-side wiring.
...
Use the public `pockets/jobs/memstore` for zero-infrastructure hosts, or select pgx/Turso sibling modules. All run the pocket conformance suite. The SQL migration source includes the ordinary queue, schedules, and fenced queue.
