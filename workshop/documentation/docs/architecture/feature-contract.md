...
description: Anatomy, rules, extension tiers, mounting, persistence, and testing for reusable feature modules.
---

# Feature contract

A Gopernicus feature is a **library-shaped hexagon**: a reusable domain capability with public entities and ports, sealed services and delivery, optional sibling adapters, and a narrow socket the host composes.

Authentication, authorization, CMS, events, and jobs are the reference features.
...
- `Register(feature.Mount)`: optional shipped transport registration.

CMS currently retains an earlier `Register(mount, repos, config)` shape while its full public `Service` surface is completed. Do not assume every feature has identical construction syntax; the contract is converging without hiding the difference.
...
3. **No cross-feature imports.** The consuming feature declares a port; the host wires it.
...
7. **Transport uses SDK web primitives.** Feature HTTP code uses the shared responders and error mapping rather than forking mechanism.
...
`sdk/feature.Mount` is deliberately narrow:
...
`RouteRegistrar` has one `Handle` method matching `web.WebHandler`. A feature can register routes without importing the concrete router. `Events` is an optional best-effort emit rail, not a transaction or job queue.

The mount is not a service locator. Feature-specific dependencies belong in that feature's `Repositories` and `Config`.
...
A host can wrap the registrar it passes to a feature:

```go
mount := feature.Mount{
    Router: feature.Group{
        Prefix:     "/account",
...
`feature.PrefixRegistrar` changes only paths registered with the host. `feature.Group` also prepends middleware. Because wrappers implement the same one-method interface, a host can deny, replace, re-path, or wrap individual routes with its own registrar.

:::warning Prefixes do not rewrite rendered URLs

Prefixing registration does not change links, form actions, or redirects emitted by a feature. The current CMS views contain host-rooted paths, so mounting the whole CMS below a prefix is not yet transparent.

:::

## Persistence contract

A durable feature generally ships:
...
- the feature can run without either shipped dialect;
- `make check` guards the new core and modules.

Workshop can emit a starting anatomy; see [Create a feature](../guides/create-feature.md).
