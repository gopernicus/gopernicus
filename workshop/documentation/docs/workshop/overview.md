---
title: Workshop CLI
description: Developer tooling for Gopernicus hosts, pockets, and migration ledgers.
---

# Workshop CLI

`workshop/gopernicus` is the stdlib-only developer CLI. It emits initial host and pocket source trees, delegates database work to host-owned runners, and is not linked into generated applications at runtime.
...
Workshop currently accepts identity inputs—module path, pocket name, aggregate name, and datastore choice. It emits a starting tree that can be edited as ordinary Go source.

| Available today | Planned or under consideration |
|---|---|
| bare host composition root | schema/annotation-driven app generation |
| CRUD-shaped pocket anatomy | per-field entity generation |
...
The command surface and the generation roadmap are separate concerns: the current commands are useful without generation, and future generation can build on the same module and pocket contracts.
...
- pocket source is embedded as templates, not imported pocket packages;
