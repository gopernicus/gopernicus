...
Every example is a real host module, not documentation-only pseudocode. Together they demonstrate optional dependencies, explicit cross-feature wiring, background-runtime ownership, datastore substitution, and the optional GOTH UI surface.
...
| `examples/goth-showcase` | `go run ./cmd/server` | GOTH catalog and browser behavior without a feature dependency |
...
Start here. `examples/minimal` gives the CMS feature a host-local memory store and registers a custom Product content type without a migration. It also demonstrates embedded GOTH asset serving.
...
`examples/cms` replaces the memory repositories with `features/cms/stores/turso`, uses `integrations/datastores/turso` for the connection, and owns the merged migration ledger under `examples/cms/workshop/migrations`.
...
`examples/auth-cms` is the most complete composition proof. It wires feature-to-feature needs without any feature importing another feature:
...
- the events gateway consumes the same bus that features emit through;
...
`examples/jobs-minimal` uses the public `features/jobs/memstore` and starts the runtime explicitly. Its demo protocol exercises a normal job, a retry, a dead-letter, and a recurring schedule.
...
`examples/goth-showcase` renders the GOTH component catalog independently of a feature. The sibling `e2e` project contains Playwright and accessibility coverage for Chromium, Firefox, and WebKit.
