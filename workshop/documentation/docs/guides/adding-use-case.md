...
Open `core/cases/projectadmin/case.go`. Gopernicus follows the Go idiom of accepting interfaces and returning structs — define an interface for each dependency your case needs.
...
Gopernicus convention places interfaces at the top of the file, before the Case struct. Keeping them narrow — only the methods this case actually calls — makes testing and reasoning about dependencies easier.
...
For durable processing, use the `@event: <type> outbox` annotation in `queries.sql` to persist the event atomically with the business write. The generator handles the transactional outbox plumbing. See [Infrastructure Events](/docs/gopernicus/infrastructure/events#transactional-outbox) for details. Chain `.WithTenant(tenantID)` for tenant-scoped routing.
