...
`features/events` builds a durable outbox and authenticated SSE gateway on top of the SDK event-bus vocabulary. The event bus and the feature are different layers: a host may use the bus without the outbox/SSE feature.
...
if err := eventsSvc.Register(feature.Mount{
    Router: router,
    Logger: log,
}); err != nil {
    return err
...
Do not describe `Mount.Events` as transactional. Durable records travel through feature repositories, not the mount emitter.
