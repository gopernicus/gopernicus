...
`features/authentication` is a datastore-free identity feature for human and machine principals. It owns users, multiple identifiers, credentials, sessions, challenges, recovery, OAuth linking, API keys, invitations, security events, and optional user administration.

It also implements `sdk/foundation/identity.Resolver` and exposes middleware other host routes and features can use.
...
`Config.Views == nil` keeps the feature JSON-only. Supplying the `features/authentication/views/goth` adapter adds HTML pages and form handling without changing JSON contracts.
...
| `jobs` | durable, cross-instance keyed/fenced work through the jobs feature | production and multi-instance deployments |

The host owns either runtime. In jobs mode, authentication exposes delivery callbacks the host registers with `jobs.NewFencedRuntime`; the feature does not import jobs.
...
Both pgx and Turso store modules implement the feature repositories and export the authentication migration set. The host owns its final migration ledger and upgrades.

The feature core imports neither store nor UI. `features/authentication/views/goth` maps the technology-neutral `Views` port onto `ui/goth`; hosts can embed that default and override pages, or implement the port with another renderer.

## Not shipped

Multi-factor authentication is not part of the current feature. Method and assurance vocabulary reserve a future path, but there is no TOTP, passkey/WebAuthn, or recovery-code MFA surface today.
