...
Following the Go idiom -- accept interfaces, return structs -- define the interface where it is consumed, not where it is implemented.
...
Keep interfaces small. If you need multiple capabilities (send, receive, check status), consider separate interfaces rather than one large one.
...
- **Structured logging**: Accept `*slog.Logger` and log at Debug level for success, Error level for failures
- **Context propagation**: Pass `context.Context` through to external calls for tracing and cancellation
- **Error wrapping**: Wrap external errors with `fmt.Errorf("send sms: %w", err)` for clear error chains
