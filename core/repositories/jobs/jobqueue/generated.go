...
var (
	ErrJobQueueNotFound         = fmt.Errorf("jobqueue: %w", errs.ErrNotFound)
	ErrJobQueueAlreadyExists    = fmt.Errorf("jobqueue: %w", errs.ErrAlreadyExists)
	ErrJobQueueInvalidReference = fmt.Errorf("jobqueue: %w", errs.ErrInvalidReference)
	ErrJobQueueInvalidInput     = fmt.Errorf("jobqueue: %w", errs.ErrInvalidInput)
)
...
// JobQueue is the database entity.
...
// CreateJobQueue is the input for creating a job_queue.
type CreateJobQueue struct {
...
// UpdateJobQueue is the input for updating a job_queue.
type UpdateJobQueue struct {
	EventType     *string          `json:"event_type,omitempty"`
	CorrelationID *string          `json:"correlation_id,omitempty"`
	TenantID      *string          `json:"tenant_id,omitempty"`
	AggregateType *string          `json:"aggregate_type,omitempty"`
...
func EncodeJobQueueCursor(record JobQueue, orderField string) (string, error) {
	orderValue := GetJobQueueOrderValue(record, orderField)
	return fop.EncodeCursor(orderField, orderValue, record.JobID)
}

func GetJobQueueOrderValue(record JobQueue, field string) any {
	switch field {
	case "job_id":
		return record.JobID
	case "event_type":
...
func (r *Repository) List(ctx context.Context, filter FilterList, orderBy fop.Order, page fop.PageStringCursor) ([]JobQueue, fop.Pagination, error) {
	if orderBy.Field == "" {
...
func (r *Repository) Get(ctx context.Context, jobID string) (JobQueue, error) {
	result, err := r.store.Get(ctx, jobID)
	if err != nil {
		return JobQueue{}, fmt.Errorf("get: %w", err)
	}
	return result, nil
}

// Create generates an ID, sets defaults, and delegates to the store.
func (r *Repository) Create(ctx context.Context, input CreateJobQueue) (JobQueue, error) {
	if input.JobID == "" {
		id, err := r.generateID()
		if err != nil {
			return JobQueue{}, fmt.Errorf("generate id: %w", err)
		}
		input.JobID = id
	}
	result, err := r.store.Create(ctx, input)
	if err != nil {
		return JobQueue{}, fmt.Errorf("create: %w", err)
	}
	return result, nil
}

// Update delegates to the store.
func (r *Repository) Update(ctx context.Context, jobID string, input UpdateJobQueue) (JobQueue, error) {
	result, err := r.store.Update(ctx, jobID, input)
	if err != nil {
		return JobQueue{}, fmt.Errorf("update: %w", err)
	}
	return result, nil
}
...
func (r *Repository) Delete(ctx context.Context, jobID string) error {
	if err := r.store.Delete(ctx, jobID); err != nil {
