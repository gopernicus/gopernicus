...
func TestGeneratedJobScheduleStore_Create(t *testing.T) {
	ctx, db, store := setupTestStore(t)
	pgxfixtures.TruncatePublicSchema(t, ctx, db.Pool)

	created := fixtures.CreateTestJobScheduleWithDefaults(t, ctx, db)
	_ = created
	...
	result, err := store.Get(ctx, created.ScheduleID)
	require.NoError(t, err)
	assert.Equal(t, created.ScheduleID, result.ScheduleID)
	assert.Equal(t, created.ScheduleID, result.ScheduleID)
	assert.Equal(t, created.Name, result.Name)
	assert.Equal(t, created.EventType, result.EventType)
	assert.Equal(t, created.CronExpr, result.CronExpr)
	assert.Equal(t, created.Payload, result.Payload)
	assert.Equal(t, created.Enabled, result.Enabled)
	assert.Equal(t, created.LastJobID, result.LastJobID)
}

func TestGeneratedJobScheduleStore_Get(t *testing.T) {
	ctx, db, store := setupTestStore(t)
	pgxfixtures.TruncatePublicSchema(t, ctx, db.Pool)
...
func TestGeneratedJobScheduleStore_List(t *testing.T) {
	ctx, db, store := setupTestStore(t)
	pgxfixtures.TruncatePublicSchema(t, ctx, db.Pool)
	...
	for i := 1; i < numRecords; i++ {
		fixtures.CreateTestJobScheduleWithDefaults(t, ctx, db)
	}
	_ = created

	t.Run("returns records", func(t *testing.T) {
		filter := jobschedules.FilterList{}
		orderBy := fop.NewOrder(jobschedules.DefaultOrderBy, jobschedules.DefaultOrderDirection)
	...
	t.Run("no duplicates", func(t *testing.T) {
		...
		for _, r := range results {
			assert.False(t, seen[r.ScheduleID], "duplicate record: %s", r.ScheduleID)
			seen[r.ScheduleID] = true
		}
	})
}

func TestGeneratedJobScheduleStore_Delete(t *testing.T) {
	ctx, db, store := setupTestStore(t)
	pgxfixtures.TruncatePublicSchema(t, ctx, db.Pool)
	...
	err := store.Delete(ctx, created.ScheduleID)
	require.NoError(t, err)

	// Verify record is gone.
	_, err = store.Get(ctx, created.ScheduleID)
	require.Error(t, err)
}

func TestGeneratedJobScheduleStore_CreateDuplicate(t *testing.T) {
	ctx, db, store := setupTestStore(t)
	pgxfixtures.TruncatePublicSchema(t, ctx, db.Pool)
	...
	input := jobschedules.CreateJobSchedule{
	...
	require.Error(t, err)
	assert.ErrorIs(t, err, jobschedules.ErrJobScheduleAlreadyExists)
}

func TestGeneratedJobScheduleStore_Update(t *testing.T) {
	ctx, db, store := setupTestStore(t)
	pgxfixtures.TruncatePublicSchema(t, ctx, db.Pool)
	...
	result, err := store.Update(ctx, created.ScheduleID, jobschedules.UpdateJobSchedule{
		Name: &newValue,
	})
	require.NoError(t, err)
