// Package pockets defines the registration contract between a host application
// and a pocket module (Django-app / Rails-engine shaped). It carries only
// stdlib types plus sdk/pkg/web and sdk/capabilities/events (both stdlib-only): a pocket depends
...
type Mount struct {
	...
	// on. Success means admission, not persistence or handler completion. An event is
	...
	// events.Noop to disable valid notifications. Noop still validates input.
