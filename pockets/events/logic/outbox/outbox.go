...
type EntryRepository interface {
	...
	// records is a no-op that returns nil. Validate every record before inserting
	// any row; invalid identity/type returns sdk.ErrInvalidInput. Payloads are
	// opaque bytes, including empty input. The whole batch commits or none does.
	// Stores snapshot payloads and metadata; later caller mutation cannot alter
	// the stored record. Listed entries are independent snapshots too.
