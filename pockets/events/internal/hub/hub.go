...
type Hub struct {
	...
	mu        sync.Mutex
	conns     map[sdk.Principal][]*connection
	closed    bool
	closeOnce sync.Once
	closeErr  error
...
func New(bus sdkevents.Bus, cfg Config) (*Hub, error) {
	h := &Hub{
		...
		conns:     make(map[sdk.Principal][]*connection),
	}
	if h.log == nil {
		h.log = slog.Default()
	}
...
// its event channel plus an unregister func the caller MUST defer. It enforces
...
func (h *Hub) Connect(subject sdk.Principal, opts ConnectOptions) (<-chan sdkevents.Event, func(), error) {
	h.mu.Lock()
	defer h.mu.Unlock()

	if h.closed {
		return nil, nil, sdkevents.ErrClosed
	}
	...
	c := &connection{
		...
		ch:           make(chan sdkevents.Event, h.bufSize),
	}
	if len(opts.Types) > 0 {
...
func (h *Hub) remove(subject sdk.Principal, target *connection) {
	h.mu.Lock()
	defer h.mu.Unlock()
	...
	for i, c := range list {
		if c == target {
			...
			list[len(list)-1] = nil
			h.conns[subject] = list[:len(list)-1]
			break
		}
	}
...
// handle is the bus subscription: it non-blocking sends the immutable event to every accepting connection, dropping on a full buffer.
...
// Frame projects an allowed event on its connection goroutine. The SSE id: is the durable rail's
...
// Projector opts in.
func (h *Hub) Frame(e sdkevents.Event) Frame {
	id := e.CorrelationID()
...
func (h *Hub) recordDrop(subject sdk.Principal) {
	n := h.dropped.Add(1)
