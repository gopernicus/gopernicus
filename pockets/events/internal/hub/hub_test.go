...
func (b *plainBus) Emit(ctx context.Context, e sdkevents.Event) error {
	if b.handler != nil {
...
func recv(t *testing.T, ch <-chan sdkevents.Event) sdkevents.Event {
	t.Helper()
	select {
	case f := <-ch:
		return f
...
func expectNoFrame(t *testing.T, ch <-chan sdkevents.Event) {
	t.Helper()
	select {
	case f := <-ch:
...
func TestConnect_MetadataOnlyProjection(t *testing.T) {
	...
	frames, unregister, err := h.Connect(sdk.Principal{Type: "user", ID: "u1"}, ConnectOptions{})
	if err != nil {
...
func TestConnect_TypesFilter(t *testing.T) {
	...
	frames, unregister, err := h.Connect(sdk.Principal{Type: "user", ID: "u1"}, ConnectOptions{Types: []string{"content.updated"}})
	if err != nil {
		t.Fatalf("Connect: %v", err)
	}
	defer unregister()
...
func TestConnect_ResourceScopedFilter(t *testing.T) {
	...
	frames, unregister, err := h.Connect(sdk.Principal{Type: "user", ID: "u1"}, ConnectOptions{ResourceType: "entry", ResourceID: "e1"})
	if err != nil {
		t.Fatalf("Connect: %v", err)
	}
	defer unregister()
...
func TestConnect_PerSubjectCap(t *testing.T) {
	...
	_, u1, err := h.Connect(sdk.Principal{Type: "user", ID: "u1"}, ConnectOptions{})
	if err != nil {
		t.Fatalf("Connect 1: %v", err)
	}
	defer u1()
	_, u2, err := h.Connect(sdk.Principal{Type: "user", ID: "u1"}, ConnectOptions{})
	if err != nil {
		t.Fatalf("Connect 2: %v", err)
	}
	defer u2()

	if _, _, err := h.Connect(sdk.Principal{Type: "user", ID: "u1"}, ConnectOptions{}); err != ErrTooManyConnections {
		t.Fatalf("Connect 3 err = %v, want ErrTooManyConnections", err)
	}

	// A different subject is unaffected by another subject's cap.
	if _, u3, err := h.Connect(sdk.Principal{Type: "user", ID: "u2"}, ConnectOptions{}); err != nil {
	...
	u1()
	if _, u4, err := h.Connect(sdk.Principal{Type: "user", ID: "u1"}, ConnectOptions{}); err != nil {
...
func TestConnect_DropsWhenBufferFull(t *testing.T) {
	...
	_, unregister, err := h.Connect(sdk.Principal{Type: "user", ID: "u1"}, ConnectOptions{})
	if err != nil {
		t.Fatalf("Connect: %v", err)
	}
	defer unregister()
...
func TestPrincipalCapAndClose(t *testing.T) {
	bus := sdkevents.NewMemory()
	defer bus.Close(context.Background())
	h, err := New(bus, Config{MaxConnsPerSubject: 1})
	if err != nil {
		t.Fatal(err)
	}
	for _, principal := range []sdk.Principal{{Type: "team:user", ID: "1"}, {Type: "team", ID: "user:1"}} {
		_, remove, err := h.Connect(principal, ConnectOptions{})
		if err != nil {
			t.Fatal(err)
		}
		defer remove()
	...
	if _, _, err := h.Connect(sdk.Principal{Type: "user", ID: "new"}, ConnectOptions{}); !errors.Is(err, sdkevents.ErrClosed) {
