...
// It returns the in-mem dispatcher for status assertions. BodySenders routes non-email
// kinds.
func wireSyncDelivery(t *testing.T, svc *Service, mailer email.Sender, bodySenders map[string]delivery.BodySender) *memDispatcher {
	t.Helper()
	quiet := slog.New(slog.NewTextHandler(io.Discard, nil))
	router, err := delivery.NewRouter(mailer,
		delivery.WithMailFrom("noreply@example.com"),
		delivery.WithBodySenders(bodySenders),
		delivery.WithRouterLogger(quiet))
	if err != nil {
		t.Fatalf("delivery.NewRouter: %v", err)
	}
	disp := newMemDispatcher()
	enc := fakeEncrypter{}
	dsvc, err := delivery.NewService(disp, enc)
	if err != nil {
		t.Fatalf("delivery.NewService: %v", err)
	}
	proc, err := delivery.NewJobsProcessor(enc,
		router,
		delivery.WithProcessorInitializer((&deliveryInitializer{service: svc})))
	if err != nil {
...
func TestForgotPasswordDoesNotBlockOnProvider(t *testing.T) {
	...
	quiet := slog.New(slog.NewTextHandler(io.Discard, nil))
	router, err := delivery.NewRouter(blocking,
		delivery.WithMailFrom("noreply@example.com"),
		delivery.WithRouterLogger(quiet))
	if err != nil {
		t.Fatalf("delivery.NewRouter: %v", err)
	...
	proc, err := delivery.NewJobsProcessor(enc,
		router,
		delivery.WithProcessorInitializer((&deliveryInitializer{service: h.svc})))
	if err != nil {
		t.Fatalf("delivery.NewJobsProcessor: %v", err)
	}
	h.svc.queue = dsvc
...
func TestDeliveryStatusReflectsState(t *testing.T) {
	...
	if _, err := h.svc.queue.Enqueue(ctx, delivery.Command{
		...
		Envelope:       delivery.Envelope{Destination: "s@example.com", Subject: "Verify your email", Body: "hi", HTML: "<p>hi</p>"},
	}); err != nil {
