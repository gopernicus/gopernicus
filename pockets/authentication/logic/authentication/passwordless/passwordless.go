...
// Version 2 rejects all older links because v1 could omit an existing owner.
...
type Binding struct {
	...
	// Kind is the identifier kind the link was issued for. Provisioning is
	...
	// AuthRevision is required when an owner existed at issue. Every redemption
	// checks it before admitting a session or changing that owner's credentials.
...
// HasOwner distinguishes account-bound proof from intentional address-only
// provisioning. Older account-bound payloads still require a revision.
func (b Binding) HasOwner() bool {
	return b.UserID != "" || b.IdentifierID != "" || b.AuthRevision != nil
}

// MatchesOwner validates an account-bound proof inside the redemption transaction.
// An address-only provisioning proof keeps the current-claim semantics.
func (b Binding) MatchesOwner(userID, identifierID string, revision int64) bool {
	if !b.HasOwner() {
		return b.ProvisionIfAbsent
	}
	return b.UserID == userID && b.IdentifierID == identifierID && b.AuthRevision != nil && *b.AuthRevision == revision
}
