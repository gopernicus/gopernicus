...
// opaque delivery work. It performs NO account lookup, NO challenge issue, NO
...
//
// Each admitted request owns a command, so an initializer that has already issued
// a challenge cannot lose its delivery command to a later resend. Provider sends
// may finish in any order; only the last stored challenge can verify.
func (s *Service) ResendVerification(ctx context.Context, emailAddr string) error {
	...
	generation, err := (sdk.IDGenerator{}).Generate()
	if err != nil {
		return err
	}
	key := s.idempotencyKey(sdk.AddressKindEmail, normalized, delivery.PurposeRegistrationVerification) + ":" + generation
...
//
// It uses the same challenge and rendering rails as the public worker. Each
...
func (s *Service) ResendVerificationForUser(ctx context.Context, actor Principal, userID string) (StepUpReceipt, error) {
	...
	code, issued, err := s.issueChallenge(ctx, userID, challenge.PurposeVerifyRegistration)
	if err != nil {
		return StepUpReceipt{}, err
	}
	key := s.idempotencyKey(sdk.AddressKindEmail, ident.NormalizedValue, delivery.PurposeRegistrationVerification) + ":" + issued.ID
...
func (s *Service) initRegistrationVerification(ctx context.Context, cmd delivery.Envelope) (delivery.Envelope, bool, error) {
	...
	code, err := s.issueChallengeSecret(ctx, ident.UserID, challenge.PurposeVerifyRegistration)
	if err != nil {
		return delivery.Envelope{}, false, err
	}
