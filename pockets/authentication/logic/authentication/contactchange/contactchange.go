...
//
// START creates the PendingChange, then issues a challenge scoped and bound to
// its ID. CONFIRM reads the current pending row, consumes that ID's challenge,
// then conditionally consumes the same pending ID. A wrong code cannot destroy
// its pending value, and a stale confirmation cannot consume a newer generation.
...
// up one-to-one with identifier.ApplyVerifiedChangeInput so authentication service translates a
