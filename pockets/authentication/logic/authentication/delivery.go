...
// declared here (structural) so authentication service accepts an interface and a test can inject a
...
// structurally; declared here so authentication service carries no import cycle with its host-facing
...
func (s *Service) identifierDigest(kind, normalizedValue string) string {
	...
	digest, err := cryptids.SHA256(kind + ":" + normalizedValue)
	if err != nil {
...
// issueAndEnqueueCode gives every issued challenge its own delivery generation.
// Queue admission ordering cannot deduplicate or overwrite a newer challenge's
// message. Resends still invalidate earlier codes; an already accepted provider
// send cannot be recalled. Admission failure is returned so the caller can restart.
func (s *Service) issueAndEnqueueCode(ctx context.Context, userID, purpose, kind, destination string, opts ...challengeOption) (string, error) {
	if s.queue == nil || s.deliver == nil {
		return "", ErrDeliveryDisabled
	}
	code, issued, err := s.issueChallenge(ctx, userID, purpose, opts...)
	if err != nil {
		return "", err
	}
	key := s.idempotencyKey(kind, destination, purpose) + ":" + issued.ID
	if err := s.enqueueRendered(ctx, delivery.PurposeSensitiveCode, key, delivery.Request{
		Kind: kind, Purpose: delivery.PurposeSensitiveCode,
		Destination: destination, ResolutionInput: destination, Secret: code,
	}); err != nil {
		return "", err
	}
	return key, nil
...
// deliveryInitializer is retained by the delivery executor, separately from the
// ordinary service capability handed to request handlers.
type deliveryInitializer struct{ service *Service }
...
func (initializer *deliveryInitializer) Initialize(ctx context.Context, kind, purpose string, cmd delivery.Envelope) (delivery.Envelope, bool, error) {
	s := initializer.service
	switch purpose {
...
func (s *Service) initPasswordReset(ctx context.Context, cmd delivery.Envelope) (delivery.Envelope, bool, error) {
	...
	owner, err := s.users.Get(ctx, ident.UserID)
	if errors.Is(err, sdk.ErrNotFound) {
	...
	// Re-read after the revision snapshot so a concurrent address replacement
	// cannot attach an old address proof to the new credential revision.
	ident, err = s.identifiers.Get(ctx, ident.ID)
	if errors.Is(err, sdk.ErrNotFound) {
		return delivery.Envelope{}, false, nil
	}
	if err != nil {
	...
	if ident.UserID != owner.ID || !ident.Active() || !ident.Verified() || !ident.RecoveryEnabled || ident.NormalizedValue != cmd.ResolutionInput {
		return delivery.Envelope{}, false, nil
	}
	token, err := s.issueChallengeSecret(ctx, ident.UserID, challenge.PurposePasswordReset, withStoredContext(passwordreset.Binding{
		Version: passwordreset.BindingVersion, AuthRevision: owner.AuthRevision, IdentifierID: ident.ID,
	}))
	if err != nil {
		return delivery.Envelope{}, false, err
	}
...
func (initializer *deliveryInitializer) Discard(ctx context.Context, kind, purpose string, env delivery.Envelope) error {
	s := initializer.service
	if env.Secret == "" {
		return nil
	}
