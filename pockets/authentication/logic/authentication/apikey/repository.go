...
//   - Create whose KeyHash collides with an existing key → sdk.ErrAlreadyExists.
//   - GetByHash for an unknown hash → sdk.ErrNotFound; any present row (revoked
//     or expired included) → the record.
//   - Revoke / TouchLastUsed for an unknown id → sdk.ErrNotFound.
//
// ListByServiceAccount is list-typed (design §9); ordering is pinned to
// ORDER BY created_at DESC, id DESC (the id tiebreak is contractual).
type APIKeyRepository interface {
	// Create persists a new key; a colliding KeyHash → sdk.ErrAlreadyExists.
	Create(ctx context.Context, k APIKey) (APIKey, error)
	// GetByHash returns the key for keyHash regardless of revocation/expiry;
	// unknown hash → sdk.ErrNotFound (see the pinned contract above).
	...
	ListByServiceAccount(ctx context.Context, serviceAccountID string, req list.Request) (list.Page[APIKey], error)
	...
	// sdk.ErrNotFound. Callers treat it as best-effort (a failure never fails
	// authentication).
