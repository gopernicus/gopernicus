...
// are pinned to the sdk address-kind names so a Resolver's
...
const (
	...
	KindEmail Kind = sdk.AddressKindEmail
	// KindPhone is a phone-number identifier.
	KindPhone Kind = sdk.AddressKindPhone
)
...
type Identifier struct {
	...
	// convention (sdk.DatabaseID) so the store assigns it inline (design §2.1).
	ID string
...
// (empty under sdk.DatabaseID — the store then assigns it).
func New(ids sdk.IDGenerator, norm Normalizer, userID string, kind Kind, value string, uses Uses, primary bool, verifiedAt, now time.Time) (Identifier, error) {
	if !kind.Valid() {
...
func NewRegistrationEmail(ids sdk.IDGenerator, norm Normalizer, userID, value string, now time.Time) (Identifier, error) {
