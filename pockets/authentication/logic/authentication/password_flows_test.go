...
// answer. The zero-value constructorConfig keep the flows on.
func TestPasswordFlowsDisabled_UseCasesRefuse(t *testing.T) {
	s := newServiceWithFakes(constructorConfig{PasswordFlowsDisabled: true})
	...
	if !newServiceWithFakes(constructorConfig{}).PasswordFlowsEnabled() {
		t.Fatal("zero-value constructorConfig must keep password flows enabled")
	}
}
