...
func (h *harness) grantFor(t *testing.T, sessionID, userID, purpose string) {
	...
	if _, err := h.sess.Get(ctx, sessionID); errors.Is(err, sdk.ErrNotFound) {
		if _, err := h.sess.Create(ctx, session.Session{ID: sessionID, UserID: userID, RefreshTokenHash: "grant-" + sessionID, ExpiresAt: now.Add(time.Hour), CreatedAt: now}); err != nil {
			t.Fatal(err)
		}
	} else if err != nil {
		t.Fatal(err)
	}
	if _, err := h.grants.Create(ctx, authgrant.Grant{
		...
		AuthenticatedAt: now, Assurance: session.AssuranceAAL1,
		Methods: []session.AuthenticationMethod{{Kind: session.MethodEmailCode, Assurance: session.AssuranceAAL1}},
	}, u.AuthRevision, now); err != nil {
...
func TestPasswordRemoveInvalidatesPendingReset(t *testing.T) {
	...
	if err := h.svc.ForgotPassword(ctx, normEmail); err != nil {
		t.Fatalf("ForgotPassword: %v", err)
	}
	resetToken := resetTokenFromMail(t, h.mailer.last())
...
func TestPasswordRemoveStaleRevisionRequiresFreshProof(t *testing.T) {
	h := newHarness(t, nil)
	ctx := context.Background()
	...
	if _, err := h.svc.RemovePassword(ctx, userID, code); !errors.Is(err, sdk.ErrConflict) {
		t.Fatalf("stale removal: %v", err)
	}
	if _, err := h.pw.Get(ctx, userID); err != nil {
		t.Fatalf("stale proof removed password: %v", err)
	}
}
...
func TestPasswordRemoveConcurrentRemovalReevaluatesPolicy(t *testing.T) {
	...
	if !errors.Is(err, sdk.ErrConflict) {
		t.Fatalf("concurrent last-login removal = %v, want conflict", err)
	}
	if _, err := h.pw.Get(ctx, userID); err != nil {
