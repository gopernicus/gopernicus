...
// DefaultOrder is the sort applied when a Request carries a zero-value Order:
...
//
// List is list-typed (design §9) with the same pinned ordering as every other
// paged auth port: a zero-value Request.Order means ORDER BY created_at DESC,
...
type AdminRepository interface {
	...
	// user. list.Request.WithCount is honored as it is for every other port.
	List(ctx context.Context, req list.Request) (list.Page[Summary], error)
