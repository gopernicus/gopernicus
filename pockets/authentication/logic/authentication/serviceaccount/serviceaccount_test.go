...
func TestNewValid(t *testing.T) {
	sa, err := New(sdk.IDGenerator{}, "deployer", "CI bot", "admin-1", false, "", base)
	if err != nil {
		t.Fatalf("New: %v", err)
	}
	if sa.ID == "" {
...
func TestNewActAsUserRequiresOwner(t *testing.T) {
	...
	if _, err := New(sdk.IDGenerator{}, "personal", "", "admin", true, "", base); !errors.Is(err, sdk.ErrInvalidInput) {
		t.Errorf("act-as-user with no owner: err=%v, want ErrInvalidInput", err)
	}
	sa, err := New(sdk.IDGenerator{}, "personal", "", "admin", true, "owner-9", base)
	if err != nil {
...
func TestNewRequiredFields(t *testing.T) {
	if _, err := New(sdk.IDGenerator{}, "", "d", "admin", false, "", base); !errors.Is(err, sdk.ErrInvalidInput) {
		t.Errorf("blank name: err=%v, want ErrInvalidInput", err)
	}
	if _, err := New(sdk.IDGenerator{}, "name", "d", "", false, "", base); !errors.Is(err, sdk.ErrInvalidInput) {
