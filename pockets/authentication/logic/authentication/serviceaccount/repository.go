...
//
// List is list-typed (design §9). Ordering is pinned (plan-cut amendment): a
// zero-value Request.Order means the store's default, which is
...
type ServiceAccountRepository interface {
	...
	List(ctx context.Context, req list.Request) (list.Page[ServiceAccount], error)
