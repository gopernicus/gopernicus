...
// The stored Binding must still match the active user's credential revision and
// verified recovery identifier. Successful redemption advances that revision once;
// stale or missing binding rejects without changing any state.
package passwordreset

import "time"

// RedeemInput is everything the atomic reset needs. The caller (authentication service) has
