...
func resolverFixture() (*Service, *fakeUsers, *fakeIdentifiers) {
	...
	svc := newServiceWithFakes(constructorConfig{Users: users, Identifiers: idents})
	return svc, users, idents
}

// TestResolveUserWithDisplayName: a user principal resolves to its DisplayName,
// carrying its active verified email as an sdk.AddressKindEmail Address (design §7).
func TestResolveUserWithDisplayName(t *testing.T) {
	...
	info, err := svc.Resolve(context.Background(), sdk.Principal{Type: sdk.PrincipalTypeUser, ID: "u1"})
	if err != nil {
	...
	if len(info.Addresses) != 1 || info.Addresses[0].Kind != sdk.AddressKindEmail || info.Addresses[0].Value != "alice@example.com" {
		t.Errorf("Addresses = %+v, want one email address", info.Addresses)
	}
	if info.Principal != (sdk.Principal{Type: sdk.PrincipalTypeUser, ID: "u1"}) {
...
func TestResolveUserBlankDisplayNameStaysBlank(t *testing.T) {
	...
	info, err := svc.Resolve(context.Background(), sdk.Principal{Type: sdk.PrincipalTypeUser, ID: "u2"})
	if err != nil {
		t.Fatalf("Resolve: %v", err)
	}
	if info.DisplayName != "" {
...
func TestResolveUserIsPrincipalExact(t *testing.T) {
	...
	info, err := svc.Resolve(context.Background(), sdk.Principal{Type: sdk.PrincipalTypeUser, ID: "alice@example.com"})
...
func TestResolveProjectsAllVerifiedIdentifiersPrimaryFirst(t *testing.T) {
	...
	info, err := svc.Resolve(context.Background(), sdk.Principal{Type: sdk.PrincipalTypeUser, ID: "u3"})
	if err != nil {
		t.Fatalf("Resolve: %v", err)
	}
	want := []sdk.IdentityAddress{
		{Kind: sdk.AddressKindEmail, Value: "primary@example.com"}, // primary first
		{Kind: sdk.AddressKindEmail, Value: "second@example.com"},  // then oldest of the rest
		{Kind: sdk.AddressKindPhone, Value: "+15551230000"},        // notification-only phone still projected
	}
...
func TestResolveExcludesReplacedAndUnverified(t *testing.T) {
	...
	info, err := svc.Resolve(context.Background(), sdk.Principal{Type: sdk.PrincipalTypeUser, ID: "u4"})
	if err != nil {
		t.Fatalf("Resolve: %v", err)
	}
...
func TestResolveUnverifiedRegistrationProjectsNoAddress(t *testing.T) {
	...
	info, err := h.svc.Resolve(context.Background(), sdk.Principal{Type: sdk.PrincipalTypeUser, ID: u.ID})
	if err != nil {
		t.Fatalf("Resolve: %v", err)
	}
...
func TestResolveServiceAccount(t *testing.T) {
	...
	svc := newServiceWithFakes(constructorConfig{ServiceAccounts: sas})

	info, err := svc.Resolve(context.Background(), sdk.Principal{Type: sdk.PrincipalTypeServiceAccount, ID: "sa1"})
	if err != nil {
		t.Fatalf("Resolve: %v", err)
	}
...
func TestResolveFailClosed(t *testing.T) {
	...
	cases := []struct {
		...
		{"unknown_type", newServiceWithFakes(constructorConfig{Users: users, ServiceAccounts: sas}), sdk.Principal{Type: "robot", ID: "x"}},
		{"missing_user", newServiceWithFakes(constructorConfig{Users: users, ServiceAccounts: sas}), sdk.Principal{Type: sdk.PrincipalTypeUser, ID: "ghost"}},
		{"missing_service_account", newServiceWithFakes(constructorConfig{Users: users, ServiceAccounts: sas}), sdk.Principal{Type: sdk.PrincipalTypeServiceAccount, ID: "ghost"}},
		{"machine_subsystem_off", newServiceWithFakes(constructorConfig{Users: users}), sdk.Principal{Type: sdk.PrincipalTypeServiceAccount, ID: "sa1"}},
	}
