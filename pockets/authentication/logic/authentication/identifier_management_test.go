...
// fakeContactChanges is the authentication service-test contactchange.Repository: an atomic
...
func (f *fakeContactChanges) Get(_ context.Context, userID string, kind identifier.Kind) (contactchange.PendingChange, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	k := ccKey(userID, kind)
	p, ok := f.m[k]
...
func (f *fakeContactChanges) Consume(_ context.Context, userID string, kind identifier.Kind, expectedID string) (contactchange.PendingChange, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	k := ccKey(userID, kind)
	p, ok := f.m[k]
...
func (n *recordingNotifier) Send(_ context.Context, to string, body string) error {
	n.mu.Lock()
	defer n.mu.Unlock()
	n.sent = append(n.sent, recordedNotice{to: to, body: body})
	return nil
}
...
func (h *harness) enablePhone(t *testing.T) {
	...
	h.deliveryRepo = wireSyncDelivery(t, h.svc, h.mailer, map[string]delivery.BodySender{sdk.AddressKindPhone: h.phone})
}
...
func TestIdentifierStartRateLimited(t *testing.T) {
	...
	pair, err := h.svc.mintSession(ctx, userID, h.currentAuthRevision(userID), h.svc.primaryAuthentication(session.MethodPassword))
	if err != nil {
...
func TestIdentifierRemoveConcurrentChangeRejectsOldProof(t *testing.T) {
	h := newHarness(t, nil)
	ctx := context.Background()
	...
	// removal of A commits first, so the proof for the old revision must reject.
	...
	if !errors.Is(err, sdk.ErrConflict) {
		t.Fatalf("concurrent last-method removal = %v, want sdk.ErrConflict", err)
	}
	if it, _ := h.idents.Get(ctx, b.ID); !it.Active() {
		t.Error("identifier retired despite a changed credential revision")
	}
}
