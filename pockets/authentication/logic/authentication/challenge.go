...
// challengeOption configures issueChallengeSecret / consumeChallenge / redeemToken.
...
//
// It also scopes step-up codes to a session, channel and operation.
// Email magic links may be issued for an address with NO
...
// issueChallengeSecret mints a fresh secret for (userID, purpose), protects it, and
...
func (s *Service) issueChallengeSecret(ctx context.Context, userID, purpose string, opts ...challengeOption) (string, error) {
	secret, _, err := s.issueChallenge(ctx, userID, purpose, opts...)
	return secret, err
}

func (s *Service) issueChallenge(ctx context.Context, userID, purpose string, opts ...challengeOption) (string, challenge.Challenge, error) {
	spec, ok := challengeSpecs[purpose]
	if !ok {
		return "", challenge.Challenge{}, ErrUnknownChallengePurpose
	}
	if s.challenges == nil || s.protector == nil {
		return "", challenge.Challenge{}, fmt.Errorf("challenge subsystem not wired: %w", sdk.ErrForbidden)
	}
	...
	case formatCode:
		...
		if err != nil {
			return "", challenge.Challenge{}, fmt.Errorf("generate code: %w", err)
		}
		keyID = s.protector.ActiveKeyID()
		digest, err = s.protector.DigestCode(keyID, userID, purpose, secret)
		if err != nil {
			return "", challenge.Challenge{}, fmt.Errorf("digest code: %w", err)
		}
		if o.hasContext {
	...
	default: // formatToken
		...
		if err != nil {
			return "", challenge.Challenge{}, fmt.Errorf("generate token: %w", err)
		}
	...
	stored, err := s.challenges.Replace(ctx, ch)
	if err != nil {
		return "", challenge.Challenge{}, err
	}
...
func (s *Service) consumeChallenge(ctx context.Context, userID, purpose, presented string, opts ...challengeOption) (challenge.Consumed, error) {
	...
	consumed, outcome, err := s.challenges.ConsumeCode(ctx, subjectKey, purpose, candidates, expected, spec.maxAttempts, s.now())
	if err != nil {
