...
func saturatedInProcessService(t *testing.T, capacity int) *delivery.Service {
	t.Helper()
	q, err := delivery.NewInProcessQueue(delivery.WithQueueAdmission(delivery.QueueAdmissionConfig{Capacity: capacity, AdmissionDeadline: 20 * time.Millisecond}))
	if err != nil {
		t.Fatal(err)
	}
	dsvc, err := delivery.NewService(q, fakeEncrypter{})
	if err != nil {
...
func closedInProcessService(t *testing.T) *delivery.Service {
	...
	q, err := delivery.NewInProcessQueue(delivery.WithQueueAdmission(delivery.QueueAdmissionConfig{Capacity: 4, AdmissionDeadline: 20 * time.Millisecond}))
	if err != nil {
		t.Fatal(err)
	}
	rt, err := delivery.NewInProcessRuntime(q,
		noopInProcessProcessor{},
		delivery.WithWorkerCount(1),
		delivery.WithShutdownDeadline(time.Second),
		delivery.WithRuntimeLogger(quiet))
	if err != nil {
