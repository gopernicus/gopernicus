...
// issues a separate proof challenge scoped and bound to that row's ID, delivering the code to the
...
func (s *Service) StartIdentifierChange(ctx context.Context, in IdentifierChangeStart) (StepUpReceipt, error) {
	...
	code, err := s.issueChallengeSecret(ctx, in.UserID, challengePurpose,
		withSubjectKey(pending.ID), withStoredContext(changeBinding{
			PendingID: pending.ID, SessionID: in.SessionID, AuthRevision: owner.AuthRevision,
		}))
	if err != nil {
		return StepUpReceipt{}, err
	}
	// The pending ID owns both the proof and queue command. A late older request
	// cannot replace the newer pending value's proof or queued delivery.
	key := s.idempotencyKey(string(in.Kind), normalized, delivery.PurposeIdentifierChangeProof) + ":" + pending.ID
	if err := s.enqueueRendered(ctx, delivery.PurposeIdentifierChangeProof, key, delivery.Request{
		Kind:            string(in.Kind),
...
// confirm order it checks the live session and current revision, reads the pending
// generation, and consumes the proof bound to that generation's session/revision.
// A wrong code counts an attempt and leaves the pending value intact for retry.
// It then conditionally consumes that pending change, evaluates credential policy,
// and applies once at the proof's revision. A concurrent credential change rejects
// the old proof. A lost authentication-claim race on the new
...
func (s *Service) ConfirmIdentifierChange(ctx context.Context, in IdentifierChangeConfirm) error {
	...
	// Read the current generation first. A stale code cannot consume either the
	// current generation's challenge or a newer pending value installed while
	// this confirmation is running.
	pending, err := s.contactChanges.Get(ctx, in.UserID, in.Kind)
	...
	if _, err := s.consumeChallenge(ctx, in.UserID, challengePurpose, in.Code,
		withSubjectKey(pending.ID), withExpectedContext(changeBinding{
			PendingID: pending.ID, SessionID: in.SessionID, AuthRevision: owner.AuthRevision,
		})); err != nil {
		return err
	}
	pending, err = s.contactChanges.Consume(ctx, in.UserID, in.Kind, pending.ID)
	if errors.Is(err, sdk.ErrNotFound) {
		return ErrChallengeInvalid
	}
	if err != nil {
	...
	if err := s.applyVerifiedIdentifierChange(ctx, in.UserID, owner.AuthRevision, pending); err != nil {
		return err
	}
	s.recordSecurityEvent(ctx, securityEventInput{
		UserID: in.UserID,
...
// the proposed set immediately before the revision-CAS Apply. A concurrent
// credential change rejects this proof rather than rebasing it onto new state;
// a removal that would leave no acceptable method set is the
...
func (s *Service) SetIdentifierUses(ctx context.Context, in IdentifierUsesInput) error {
	...
	if err := s.applyCredentialMutationAtRevision(ctx, in.UserID, owner.AuthRevision, credential.ChangeIdentifierUses{
		IdentifierID: in.IdentifierID,
...
// auth_revision (design §5.5/§5.6). It checks that the method set still has the
// proof's revision, then evaluates policy and makes one conditional write. A lost
// authentication-claim race on the new
// value surfaces as sdk.ErrAlreadyExists (the generic collision).
func (s *Service) applyVerifiedIdentifierChange(ctx context.Context, userID string, revision int64, pending contactchange.PendingChange) error {
	...
	_, err = s.identifiers.ApplyVerifiedChange(ctx, input, revision, s.now())
	return err
}
...
func (s *Service) verifiedContactChannels(ctx context.Context, userID, excludeValue string) []identifier.Identifier {
	...
	idents, err := s.identifiers.ListByUser(ctx, userID)
	if err != nil {
		s.logger.Warn("identifier-change notice listing failed", "error_kind", ErrorKind(err))
		return nil
	}
...
func (s *Service) enqueueIdentifierChangeNotices(ctx context.Context, changedKind identifier.Kind, recipients []identifier.Identifier) {
	...
	for _, it := range recipients {
		...
		if err := s.enqueueRendered(ctx, delivery.PurposeIdentifierChangeNotice, key, delivery.Request{
			...
			s.logger.Warn("identifier-change notice enqueue failed", "error_kind", ErrorKind(err))
		}
	}
}
