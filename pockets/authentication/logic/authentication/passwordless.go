...
func (s *Service) VerifyPasswordless(ctx context.Context, kind, identifierValue, code string) (TokenPair, error) {
	...
	u, err := s.passwordlessProofUser(ctx, ident)
	if err != nil {
		return TokenPair{}, err
	}
	...
	if _, err := s.consumeChallenge(ctx, ident.UserID, purpose, code,
		withExpectedContext(loginBinding{AuthRevision: u.AuthRevision, IdentifierID: ident.ID, Kind: kind, NormalizedValue: ident.NormalizedValue})); err != nil {
	...
	pair, err := s.mintSession(ctx, ident.UserID, u.AuthRevision, s.primaryAuthentication(passwordlessCodeMethod(kind)))
	if err != nil {
...
func (s *Service) RedeemPasswordless(ctx context.Context, token string) (TokenPair, error) {
	...
	if err := json.Unmarshal(consumed.Context, &binding); err != nil || binding.Version != passwordless.BindingVersion || binding.AuthRevision == nil {
	...
	if u.AuthRevision != *binding.AuthRevision {
		return TokenPair{}, ErrPasswordlessLogin
	}
	method := session.MethodEmailLink
	if binding.Kind == string(identifier.KindPhone) {
		method = session.MethodSMSLink
	}
	// Success mints through the SINGLE session path (design §4.1), stamping the
	// email-link method so the session records how the primary authentication happened.
	pair, err := s.mintSession(ctx, ident.UserID, *binding.AuthRevision, s.primaryAuthentication(method))
	if err != nil {
		// A deactivated account joins every other rejection under the one generic
		// passwordless failure (CHAU-1.5).
		if errors.Is(err, session.ErrUserNotActive) {
...
func (s *Service) initPasswordlessLink(ctx context.Context, kind string, cmd delivery.Envelope) (delivery.Envelope, bool, error) {
	...
	if provisioning {
		current, lookupErr := s.matchIdentifier(ctx, cmd.ResolutionInput)
		if lookupErr != nil && !errors.Is(lookupErr, sdk.ErrNotFound) {
			return delivery.Envelope{}, false, lookupErr
		}
		if lookupErr == nil {
			ident = current
		}
	...
	binding := s.newMagicLinkBinding(kind, destination, ident)
	if ident.UserID != "" {
		u, err := s.users.Get(ctx, ident.UserID)
		if err != nil {
			return delivery.Envelope{}, false, err
		}
		current, err := s.identifiers.Get(ctx, ident.ID)
		if err != nil {
			return delivery.Envelope{}, false, err
		}
		if !u.Active() || !current.Active() || current.ID != ident.ID || current.UserID != u.ID || current.Kind != ident.Kind || current.NormalizedValue != ident.NormalizedValue || (!current.LoginEnabled && !current.RecoveryEnabled) {
			return delivery.Envelope{}, false, ErrPasswordlessLogin
		}
		binding.AuthRevision = &u.AuthRevision
	}
	token, err := s.issueChallengeSecret(ctx, ident.UserID, challenge.PurposeLoginMagicLink,
		withStoredContext(binding),
...
func (s *Service) initPasswordlessCode(ctx context.Context, kind string, cmd delivery.Envelope) (delivery.Envelope, bool, error) {
	...
	code, err := s.issueChallengeSecret(ctx, ident.UserID, challenge.PurposeLoginOTP,
		withStoredContext(loginBinding{AuthRevision: u.AuthRevision, IdentifierID: ident.ID, Kind: kind, NormalizedValue: ident.NormalizedValue}))
	if err != nil {
		return delivery.Envelope{}, false, err
	}
	env, err := s.deliver.Render(ctx, delivery.Request{
...
func (s *Service) passwordlessProofUser(ctx context.Context, ident identifier.Identifier) (user.User, error) {
	u, err := s.users.Get(ctx, ident.UserID)
	if err != nil {
		return user.User{}, ErrPasswordlessLogin
	}
	current, ok, err := s.resolvePasswordlessLogin(ctx, string(ident.Kind), ident.NormalizedValue)
	if err != nil {
		return user.User{}, err
	}
	if !ok || current.ID != ident.ID || current.UserID != u.ID {
		return user.User{}, ErrPasswordlessLogin
	}
	return u, nil
}
