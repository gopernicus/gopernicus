...
func (f *fakeServiceAccounts) List(_ context.Context, _ list.Request) (list.Page[serviceaccount.ServiceAccount], error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	...
	return list.Page[serviceaccount.ServiceAccount]{Items: items}, nil
}
...
func (f *fakeAPIKeys) ListByServiceAccount(_ context.Context, serviceAccountID string, _ list.Request) (list.Page[apikey.APIKey], error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	...
	return list.Page[apikey.APIKey]{Items: items}, nil
}
...
func TestResolveAPIKeyCredentialRoundTrip(t *testing.T) {
	...
	p, _, ok := h.svc.verifyAPIKeyCredential(ctx, raw)
	if !ok {
...
func TestResolveAPIKeyCredentialUnknownDenies(t *testing.T) {
	h := newMachineHarness(t)
	if _, _, ok := h.svc.verifyAPIKeyCredential(context.Background(), "prefix_deadbeef"); ok {
...
func TestCreateServiceAccountActAsUserUnknownOwner(t *testing.T) {
	...
	if page, _ := h.sas.List(context.Background(), list.Request{}); len(page.Items) != 0 {
...
func principalContext(id string) context.Context {
	return sdk.WithPrincipal(context.Background(), Principal{Type: PrincipalUser, ID: id})
}
