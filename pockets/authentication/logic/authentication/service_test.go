...
// fakeIdentifiers is the authentication service-test IdentifierRepository over fakeUsers: it
...
func (f *fakePasswords) Set(ctx context.Context, userID, hash string) error {
	if f.users == nil {
		f.mu.Lock()
		defer f.mu.Unlock()
		f.m[userID] = hash
	...
	_, err := f.changeLocked(userID, user.PasswordChange{NewHash: hash, Now: time.Now()}, false)
	return err
}
...
func newHarnessDeps(t *testing.T, limiter ratelimiter.Limiter, requireVerifiedEmail bool) *harness {
	...
	wireCredentialFakes(h.users, h.pw, h.sess, h.accounts, h.grants, h.ch)
...
func TestNotificationOnlyPhoneSharedNotLoginClaim(t *testing.T) {
	...
	for _, uid := range []string{"userA", "userB"} {
		info, err := h.svc.Resolve(context.Background(), sdk.Principal{Type: sdk.PrincipalTypeUser, ID: uid})
		if err != nil {
			t.Fatalf("Resolve(%s): %v", uid, err)
		}
		if len(info.Addresses) != 1 || info.Addresses[0].Kind != sdk.AddressKindPhone {
