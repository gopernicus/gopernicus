...
// minted to unlink Google cannot unlink GitHub and leaves the Google proof usable.
func TestOAuthUnlinkWrongProvider(t *testing.T) {
	...
	// A Google code cannot unlink GitHub: it addresses a separate proof slot.
	...
	if err := h.svc.UnlinkOAuth(ctx, userID, "google", googleCode); err != nil {
		t.Fatalf("correct provider proof was lost: %v", err)
	}
	if h.linkCount(t, userID) != 1 {
		t.Fatal("correct Google proof did not unlink exactly its provider")
	}
}
...
// TestOAuthUnlinkStaleRevisionRequiresFreshProof proves a concurrent password removal
...
func TestOAuthUnlinkStaleRevisionRequiresFreshProof(t *testing.T) {
	h := newHarness(t, nil)
	ctx := context.Background()
	...
	if err := h.svc.UnlinkOAuth(ctx, userID, "google", code); !errors.Is(err, sdk.ErrConflict) {
		t.Fatalf("stale unlink = %v, want sdk.ErrConflict requiring fresh proof", err)
	}
	if h.linkCount(t, userID) != 1 {
