...
func TestLinkStartStateBindsFlowFacts(t *testing.T) {
	...
	authURL, err := h.startLink(ctx, u.ID, "google", "https://app.example.com/welcome")
	if err != nil {
		t.Fatalf("StartLink: %v", err)
	}

	stored, ok := peekOAuthState(h.states, h.lookupKey(h.provider.lastState))
	if !ok {
	...
	for _, secret := range []string{fs.CodeVerifier, fs.Nonce, fs.LinkUserID, fs.RedirectTo} {
		if secret != "" && strings.Contains(authURL.AuthorizationURL, secret) {
			t.Errorf("authorization URL %q leaks a server-side flow fact %q", authURL.AuthorizationURL, secret)
		}
	}
}
...
func TestLinkStartRejectsUnknownProvider(t *testing.T) {
	...
	if _, err := h.startLink(context.Background(), u.ID, "not-wired", ""); !errors.Is(err, sdk.ErrNotFound) {
...
func TestLinkRedirectFallsBackToSameOrigin(t *testing.T) {
	...
	if _, err := h.startLink(ctx, u.ID, "google", "https://evil.example/steal"); err != nil {
		t.Fatalf("StartLink: %v", err)
	}
	res, err := h.svc.OAuthCallback(ctx, "google", h.callbackRequest(h.provider.lastState))
	if err != nil {
...
func TestLinkCallbackTargetsStateOwnerNotCaller(t *testing.T) {
	...
	if _, err := h.startLink(ctx, owner.ID, "google", ""); err != nil {
		t.Fatalf("StartLink: %v", err)
	}
	if _, err := h.svc.OAuthCallback(ctx, "google", h.callbackRequest(h.provider.lastState)); err != nil {
		t.Fatalf("OAuthCallback: %v", err)
	}
...
func TestLinkConflictDoesNotMoveIdentity(t *testing.T) {
	...
	if _, err := h.startLink(ctx, first.ID, "google", ""); err != nil {
	...
	if _, err := h.startLink(ctx, second.ID, "google", ""); err != nil {
		t.Fatalf("StartLink(second): %v", err)
	}
	_, err := h.svc.OAuthCallback(ctx, "google", h.callbackRequest(h.provider.lastState))
	if err == nil {
