...
func TestIssueConsumeEachCodePurpose(t *testing.T) {
	...
	for _, purpose := range codePurposes {
		t.Run(purpose, func(t *testing.T) {
			...
			secret, err := h.svc.issueChallengeSecret(context.Background(), "u1", purpose)
			if err != nil {
				t.Fatalf("issueChallengeSecret: %v", err)
			}
			if len(secret) != 6 {
...
func TestIssueRedeemTokenPurpose(t *testing.T) {
	h := newChallengeHarness(t, newFakeProtector("k1", "k1"))
	token, err := h.svc.issueChallengeSecret(context.Background(), "u9", challenge.PurposeLoginMagicLink)
	if err != nil {
		t.Fatalf("issueChallengeSecret: %v", err)
	}
...
func TestConsumeCodeOldKeyStillVerifies(t *testing.T) {
	...
	secret, err := h.svc.issueChallengeSecret(context.Background(), "u1", challenge.PurposeChangeEmail)
	if err != nil {
		t.Fatalf("issueChallengeSecret: %v", err)
	}
...
func TestConsumeCodeContextMatchAndMismatch(t *testing.T) {
	t.Run("match redeems", func(t *testing.T) {
		h := newChallengeHarness(t, newFakeProtector("k1", "k1"))
		secret, err := h.svc.issueChallengeSecret(context.Background(), "u1", challenge.PurposeChangeEmail, withStoredContext("new@example.com"))
		if err != nil {
			t.Fatalf("issueChallengeSecret: %v", err)
		}
...
func TestRedeemTokenCurrentBindingInvalidation(t *testing.T) {
	...
	t.Run("current binding matches", func(t *testing.T) {
		h := newChallengeHarness(t, newFakeProtector("k1", "k1"))
		token, err := h.svc.issueChallengeSecret(context.Background(), "u1", challenge.PurposeLoginMagicLink, withStoredContext(issued))
		if err != nil {
			t.Fatalf("issueChallengeSecret: %v", err)
		}
...
func TestConsumeCodeLockout(t *testing.T) {
	h := newChallengeHarness(t, newFakeProtector("k1", "k1"))
	secret, err := h.svc.issueChallengeSecret(context.Background(), "u1", challenge.PurposeLoginOTP)
	if err != nil {
		t.Fatalf("issueChallengeSecret: %v", err)
	}
	wrong := "000000"
...
func TestRedeemTokenExpired(t *testing.T) {
	h := newChallengeHarness(t, newFakeProtector("k1", "k1"))
	token, err := h.svc.issueChallengeSecret(context.Background(), "u1", challenge.PurposeLoginMagicLink)
	if err != nil {
		t.Fatalf("issueChallengeSecret: %v", err)
	}
...
func TestChallengeRepositoryFailuresSurface(t *testing.T) {
	...
	t.Run("Replace", func(t *testing.T) {
		h := newChallengeHarness(t, newFakeProtector("k1", "k1"))
		h.ch.replErr = boom
		if _, err := h.svc.issueChallengeSecret(context.Background(), "u1", challenge.PurposeLoginOTP); !errors.Is(err, boom) {
			t.Fatalf("issueChallengeSecret err = %v, want boom", err)
		}
	})
...
func TestChallengeUnknownPurpose(t *testing.T) {
	h := newChallengeHarness(t, newFakeProtector("k1", "k1"))
	if _, err := h.svc.issueChallengeSecret(context.Background(), "u1", "bogus"); !errors.Is(err, ErrUnknownChallengePurpose) {
		t.Fatalf("issueChallengeSecret err = %v, want ErrUnknownChallengePurpose", err)
	}
...
func TestChallengeEmptyPresented(t *testing.T) {
	h := newChallengeHarness(t, newFakeProtector("k1", "k1"))
	if _, err := h.svc.issueChallengeSecret(context.Background(), "u1", challenge.PurposeLoginOTP); err != nil {
		t.Fatalf("issueChallengeSecret: %v", err)
	}
...
func TestChallengeSubsystemOff(t *testing.T) {
	...
	if _, err := svc.issueChallengeSecret(context.Background(), "u1", challenge.PurposeLoginOTP); !errors.Is(err, sdk.ErrForbidden) {
		t.Fatalf("off issueChallengeSecret err = %v, want forbidden", err)
	}
