...
func serviceWithChecker(t *testing.T, checker CompromisedPasswordChecker, failOpen bool) *harness {
	t.Helper()
	h := newHarness(t, nil)
...
func TestHostPasswordValidatorReplacesDefaultLimits(t *testing.T) {
	type contextKey struct{}
	ctx := context.WithValue(context.Background(), contextKey{}, "request")
	checker := &stubChecker{}
	var received []string
	svc := newServiceWithFakes(constructorConfig{
		ValidatePassword: func(gotCtx context.Context, password string) error {
			if gotCtx.Value(contextKey{}) != "request" {
				t.Fatal("validator lost request context")
			}
			received = append(received, password)
			return nil
		},
		Compromised: checker,
	})
	for _, password := range []string{"short", strings.Repeat("a", 300)} {
		if err := svc.validatePassword(ctx, password); err != nil {
			t.Fatalf("host-approved password rejected: %v", err)
		}
	}
	if len(received) != 2 || checker.calls != 2 {
		t.Fatalf("host/breach calls = %d/%d, want 2/2", len(received), checker.calls)
	}
}

func TestHostPasswordValidatorRejectsAcrossSettingFlows(t *testing.T) {
	ctx := context.Background()
	refused := fmt.Errorf("host password rule: %w", sdk.ErrInvalidInput)
	for _, flow := range []string{"register", "set", "change", "reset"} {
		t.Run(flow, func(t *testing.T) {
			h := newHarness(t, nil)
			var userID string
			if flow == "change" {
				userID = h.mustRegister(t, "change@example.com", baselinePassword).ID
			}
			if flow == "set" {
				userID = "passwordless"
			...
			calls := 0
			checker := &stubChecker{}
			h.svc.compromised = checker
			h.svc.passwordValidator = func(_ context.Context, password string) error {
				calls++
				if password != "host-refused-password" {
					t.Fatalf("validated current password instead of new password")
				}
				return refused
			}
			var err error
			switch flow {
			case "register":
				_, err = h.svc.Register(ctx, "new@example.com", "host-refused-password", "New")
			case "set":
				_, err = h.svc.SetPassword(ctx, "session", userID, "host-refused-password")
			case "change":
				_, err = h.svc.ChangePassword(ctx, userID, baselinePassword, "host-refused-password")
			case "reset":
				err = h.svc.ResetPassword(ctx, "unused-token", "host-refused-password")
			}
			if err != refused || calls != 1 || checker.calls != 0 {
				t.Fatalf("err=%v, host calls=%d, breach calls=%d", err, calls, checker.calls)
			}
		})
	}
}

func TestHostPasswordValidatorPreservesErrorsAndBreachPolicy(t *testing.T) {
	down := errors.New("host validator unavailable")
	checker := &stubChecker{bad: "short"}
	svc := newServiceWithFakes(constructorConfig{
		ValidatePassword: func(context.Context, string) error { return down },
		Compromised:      checker,
	})
	if err := svc.validatePassword(context.Background(), "short"); err != down {
		t.Fatalf("host error changed: %v", err)
	}
	if checker.calls != 0 {
		t.Fatal("breach check ran after host rejection")
	}
	svc.passwordValidator = func(context.Context, string) error { return nil }
	if err := svc.validatePassword(context.Background(), "short"); !errors.Is(err, ErrPasswordCompromised) {
		t.Fatalf("host callback bypassed breach policy: %v", err)
	}
}

func TestExistingPasswordLoginDoesNotApplyNewPasswordPolicy(t *testing.T) {
	h := newHarness(t, nil)
	h.mustRegister(t, "existing@example.com", baselinePassword)
	h.svc.passwordValidator = func(context.Context, string) error {
		t.Error("existing password was subjected to new-password policy")
		return sdk.ErrInvalidInput
	}
	if _, _, err := h.svc.Login(context.Background(), "existing@example.com", baselinePassword); err != nil {
		t.Fatalf("login after policy change: %v", err)
	}
	if _, err := h.svc.IssueToken(context.Background(), "existing@example.com", baselinePassword); err != nil {
		t.Fatalf("token issuance after policy change: %v", err)
	}
}
