...
func TestSecurityEventOAuthLinked(t *testing.T) {
	...
	if _, err := h.svc.OAuthCallback(context.Background(), "google", h.callbackRequest(h.provider.lastState)); err != nil {
		t.Fatalf("OAuthCallback: %v", err)
	}
...
func TestSecurityEventAPIKeyAuthSuccess(t *testing.T) {
	...
	if _, _, ok := h.svc.verifyAPIKeyCredential(ctx, raw); !ok {
...
// Capture both independently enforced buckets, including for unknown users.
func loginKeysFor(t *testing.T, h *harness, email, ip string) [2]string {
	t.Helper()
	lim := &keyCapturingLimiter{}
	h.svc.limiter = lim
	_, _, _ = h.svc.Login(WithClientInfo(context.Background(), ip, "agent"), email, "password123456789")
	lim.mu.Lock()
	defer lim.mu.Unlock()
	if len(lim.keys) != 2 {
		t.Fatalf("Login used %d budget calls, want subject+IP", len(lim.keys))
	}
	return [2]string{lim.keys[0], lim.keys[1]}
}

func TestLoginKeyIsPIIFree(t *testing.T) {
	h := newHarness(t, nil)
	keys := loginKeysFor(t, h, "secret.person@example.com", "9.9.9.9")
	if !strings.HasPrefix(keys[0], "authentication:login:subject:") {
		t.Fatalf("subject key: %q", keys[0])
	}
	if keys[1] != "authentication:login:ip:9.9.9.9" {
		t.Fatalf("IP key: %q", keys[1])
	}
	for _, key := range keys {
		for _, pii := range []string{"secret.person", "example.com"} {
...
func TestLoginKeyEquivalentValuesShareBucket(t *testing.T) {
	h := newHarness(t, nil)
	a := loginKeysFor(t, h, "Case.User@Example.COM", "ip1")
	b := loginKeysFor(t, h, "case.user@example.com", "ip2")
	if a[0] != b[0] {
		t.Fatalf("equivalent subjects changed bucket: %q != %q", a[0], b[0])
	}
	if a[1] == b[1] {
		t.Fatal("distinct IPs shared an IP bucket")
	}
}
...
func TestLoginKeyUnknownAndKnownConsumeSameArm(t *testing.T) {
	...
	known := loginKeysFor(t, h, "known@example.com", "9.9.9.9")
	unknown := loginKeysFor(t, h, "nobody@example.com", "9.9.9.9")
	for _, keys := range [][2]string{known, unknown} {
		if !strings.HasPrefix(keys[0], "authentication:login:subject:") || keys[1] != "authentication:login:ip:9.9.9.9" {
			t.Fatalf("different lookup arm: %v", keys)
		}
	}
	if known[0] == unknown[0] {
		t.Fatal("different subjects shared a bucket")
	}
	if known[1] != unknown[1] {
		t.Fatal("shared IP did not share its independent bucket")
	}
}
