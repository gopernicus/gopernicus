...
func (f *fakeAuthGrants) Create(ctx context.Context, g authgrant.Grant, expectedAuthRevision int64, now time.Time) (authgrant.Grant, error) {
...
func (f *fakeAuthGrants) Consume(ctx context.Context, req authgrant.Requirement, now time.Time) (authgrant.Grant, error) {
	if err := ctx.Err(); err != nil {
		return authgrant.Grant{}, err
	}
...
func TestStepUpCodeWrongContextRejected(t *testing.T) {
	...
	// addresses a separate challenge slot, and no grant is minted.
	...
	// A different operation cannot spend the correctly bound proof.
	...
	if _, err := h.svc.CompleteStepUpWithIdentifierCode(ctx, right, code); err != nil {
		t.Fatalf("correctly bound proof was lost: %v", err)
	}
}
