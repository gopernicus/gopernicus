...
func seedInviteMeta(t *testing.T, repo *fakeInvRepo, rt, rid, rel, ident, secret string, autoAccept bool, expiresAt time.Time, md map[string]string) Invitation {
	t.Helper()
...
func TestGrantMetadataIsDefensiveCopy(t *testing.T) {
	...
	svc2 := newSvc(t, repo, granter2, constructorConfig{})
