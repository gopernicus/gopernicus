...
var inviter = sdk.Principal{Type: sdk.PrincipalTypeUser, ID: "u-inviter"}
...
func TestCreateAuthorizedPreparesInviteeContextBeforeCheck(t *testing.T) {
	...
	if got.IdentifierKind != sdk.AddressKindEmail {
		t.Errorf("check identifier kind = %q, want the defaulted %q", got.IdentifierKind, sdk.AddressKindEmail)
	}
...
func TestCreateAuthorizedResolvedSubjectCases(t *testing.T) {
	...
	cases := []struct {
		...
		{"known user", "known@x.com", "", "known@x.com", sdk.AddressKindEmail, "u-known"},
		{"unknown user", "stranger@x.com", "", "stranger@x.com", sdk.AddressKindEmail, ""},
		{"non-email identifier", "+1 (555) 010-2345", sdk.AddressKindPhone, "+15550102345", sdk.AddressKindPhone, ""},
	}
...
func TestCreateAuthorizedCheckCannotTaintCreate(t *testing.T) {
	...
	svc := newSvc(t, repo, &fakeGranter{}, constructorConfig{InviteCheck: check.check})
...
func TestListByResourceAuthorizedPosesListCheck(t *testing.T) {
	...
	page, err := svc.ListByResourceAuthorized(context.Background(), inviter, "project", "p1", list.Request{})
	if err != nil {
...
func TestTrustedOperationsStayCheckFree(t *testing.T) {
	...
	svc := newSvc(t, repo, granter, constructorConfig{InviteCheck: check.check})
	...
	page, err := svc.ListByResource(context.Background(), "project", "p1", list.Request{})
	if err != nil {
...
func TestAuthorizedOperationsFailClosedWithoutCheck(t *testing.T) {
	repo := newFakeInvRepo()
	granter := &fakeGranter{}
	svc := newSvc(t, repo, granter, constructorConfig{}) // no InviteCheck
	...
	if _, err := svc.ListByResourceAuthorized(context.Background(), inviter, "project", "p1", list.Request{}); !errors.Is(err, errInviteCheckNotWired) {
