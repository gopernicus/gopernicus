// An invitation is a request (design §6): an invite
...
// An invitation is pending until its current token is claimed for a subject.
// Accepting is durable grant-in-progress state; accepted records completed
// grants. Decline/cancel apply only before a claim. Expired is a read-time state
// for unclaimed tokens, not a lease expiry on an acceptance claim.
const (
...
// IdentifierKind is the address kind the identifier is (sdk.AddressKindEmail,
// sdk.AddressKindPhone, or any open string a wired notifier declares); it is part
...
type Invitation struct {
	...
	IdentifierKind      string // sdk.AddressKindEmail (default), sdk.AddressKindPhone, …
	ResolvedSubjectID   string // claimant while accepting; empty while pending-unresolved
	ResolvedSubjectType string // immutable once acceptance is claimed
	InvitedBy           string
	TokenHash           string
	AutoAccept          bool
	Status              string
...
// minting its record ID from ids (empty under sdk.DatabaseID — the store
...
// A blank identifierKind defaults to sdk.AddressKindEmail. The identifier is
...
func NewInvitation(ids sdk.IDGenerator, resourceType, resourceID, relation, identifier, identifierKind, invitedBy, tokenHash string, autoAccept bool, ttl time.Duration, now time.Time) (Invitation, error) {
	...
	if identifierKind == "" {
		identifierKind = sdk.AddressKindEmail
	}
	switch {
	case resourceType == "":
...
// nil-metadata  A bounds violation wraps sdk.ErrInvalidInput and no
// record is minted. Prefer this over threading a new positional argument through
// New so existing callers stay unchanged.
func NewWithMetadata(ids sdk.IDGenerator, resourceType, resourceID, relation, identifier, identifierKind, invitedBy, tokenHash string, autoAccept bool, ttl time.Duration, now time.Time, metadata map[string]string) (Invitation, error) {
	md, err := ValidateMetadata(metadata)
	if err != nil {
		return Invitation{}, err
	}
	inv, err := NewInvitation(ids, resourceType, resourceID, relation, identifier, identifierKind, invitedBy, tokenHash, autoAccept, ttl, now)
	if err != nil {
		return Invitation{}, err
	}
	inv.Metadata = md
