...
func TestCreatePendingDeliveryData(t *testing.T) {
	...
	granter := &fakeGranter{}
	hook := &recordingHook{}
	svc := newSvc(t, repo, granter, constructorConfig{UserLookup: unknownUser})
...
func TestDirectAddDeliveryData(t *testing.T) {
	...
	granter := &fakeGranter{}
	hook := &recordingHook{}
	svc := newSvc(t, repo, granter, constructorConfig{UserLookup: knownUser})
	wireSyncDeliveryHook(t, svc, &recordingMailer{}, nil, hook.hook)
...
func TestHookEnrichmentReachesDeliveredMail(t *testing.T) {
	...
	svc := newSvc(t, repo, &fakeGranter{}, constructorConfig{UserLookup: unknownUser})
...
func TestMemberAddedHookErrorIsBestEffort(t *testing.T) {
	...
	t.Run("accept", func(t *testing.T) {
		...
		stored, _ := repo.Get(context.Background(), inv.ID)
		if stored.Status != StatusAccepted || len(granter.calls) != 1 {
