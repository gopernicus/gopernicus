...
// synchronously (invitation service only enqueues pre-rendered commands, so the processor
...
func wireSyncDelivery(t *testing.T, svc *Service, mailer email.Sender, bodySenders map[string]delivery.BodySender) {
	t.Helper()
	wireSyncDeliveryHook(t, svc, mailer, bodySenders, nil)
}
...
func wireSyncDeliveryHook(t *testing.T, svc *Service, mailer email.Sender, bodySenders map[string]delivery.BodySender, hook delivery.DataHook) {
	t.Helper()
