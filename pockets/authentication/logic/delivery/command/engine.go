...
// Option configures command processing before construction.
...
// WithInitializer replaces the opaque-command initializer. Nil leaves opaque
// commands disabled; rendered commands can still be delivered.
func WithInitializer(initializer Initializer) Option {
	return func(c *processorConfig) { c.initializer = initializer }
}

// WithClock replaces the processor clock. Nil selects time.Now.
func WithClock(clock func() time.Time) Option {
	return func(c *processorConfig) { c.now = clock }
}

// WithPolicy replaces the retry and timeout policy. Zero fields select defaults;
// negative bounds are rejected during construction.
func WithPolicy(policy Config) Option {
	return func(c *processorConfig) { c.policy = policy }
}
...
func NewProcessor(encrypter cryptids.Encrypter, deliverer Deliverer, opts ...Option) (*Engine, error) {
	d := processorConfig{}
	for _, opt := range opts {
		if opt == nil {
			return nil, fmt.Errorf("delivery command: nil option: %w", sdk.ErrInvalidInput)
		}
		opt(&d)
	}
	if nilDependency(encrypter) {
		return nil, ErrEncrypterRequired
	}
	if nilDependency(deliverer) {
		return nil, ErrDelivererRequired
	}
	if d.initializer != nil && nilDependency(d.initializer) {
		return nil, fmt.Errorf("delivery command: initializer contains a typed nil: %w", sdk.ErrInvalidInput)
	}
	if d.policy.MaxAttempts < 0 || d.policy.ProviderTimeout < 0 {
		return nil, fmt.Errorf("delivery command: retry and timeout bounds must not be negative: %w", sdk.ErrInvalidInput)
	}
	now := d.now
	if now == nil {
		now = time.Now
