...
func NewService(dispatcher Dispatcher, encrypter cryptids.Encrypter) (*Service, error) {
	if nilDependency(encrypter) {
		return nil, ErrEncrypterRequired
	}
	if nilDependency(dispatcher) {
		return nil, ErrDispatcherRequired
	}
	return &Service{dispatcher: dispatcher, enc: encrypter}, nil
}
