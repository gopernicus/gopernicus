...
// TemplatesConfig.AppTemplates (design §6.2). The registry walks the "templates" subdirectory
// and exposes each file as "<namespace>:<name>".
//
//go:embed templates/*.html templates/*.txt
var coreTemplates embed.FS

// Namespace is the email content namespace the pocket registers its
...
// §6.2). Hosts use it for content-template overrides.
...
// when it names none, mirroring the "templates" default WithContentTemplates applies
...
var (
	...
	// ErrOverrideInvalid is returned by NewRouter when a TemplatesConfig.Subjects or
	// TemplatesConfig.SMSBodies entry cannot be honored: an unknown purpose key, an
...
// MessagesConfig.DeliveryData godoc for the synchronous, best-effort, and worker-side
// cases.
...
// kind (sdk.AddressKindEmail selects the email rail; any other kind selects the
...
// routerConfig holds the collaborators NewRouter needs. Mailer is required (email is
// always-on); BodySenders is the host's non-email delivery set keyed by kind;
...
type routerConfig struct {
	Mailer       email.Sender
	MailFrom     string
	BodySenders  map[string]BodySender
...
// Router is the constructor-injected, kind-aware renderer/router shared by authentication service
// and invitation service (design §6.1). It owns the deny-by-absence kind policy: email
// renders through email.Renderer and delivers through the Mailer; every other kind renders a body-only message and
...
type Router struct {
	...
	// override when routerConfig supplied one, else the in-core spec. Each is a standalone
...
func NewRouter(mailer email.Sender, opts ...RouterOption) (*Router, error) {
	d := routerConfig{}
	d.Mailer = mailer
	for _, opt := range opts {
		if opt == nil {
			return nil, fmt.Errorf("authentication NewRouter: nil option: %w", sdk.ErrInvalidInput)
		}
		opt(&d)
	}

	if nilDependency(d.Mailer) {
		return nil, ErrMailerRequired
	}
	bodySenders, err := CopyBodySenders(d.BodySenders)
	if err != nil {
		return nil, err
	}
	logger := d.Logger
	...
	emailOptions := []email.Option{email.WithContentTemplates(namespace, coreTemplates, email.LayerCore)}
	for _, o := range d.AppTemplates {
		emailOptions = append(emailOptions, email.WithContentTemplates(o.Namespace, o.FS, email.LayerApp))
	}
	for _, o := range d.AppLayouts {
		...
		emailOptions = append(emailOptions, email.WithLayouts(o.FS, dir, email.LayerApp))
	}
	if d.Branding != nil {
		emailOptions = append(emailOptions, email.WithBranding(d.Branding))
	}
	renderer, err := email.NewRenderer(emailOptions...)
	if err != nil {
...
func (r *Router) Render(ctx context.Context, req Request) (Envelope, error) {
	...
	if req.Kind == sdk.AddressKindEmail {
		...
		html, text, err := r.renderer.Render(email.RenderRequest{Template: namespace + ":" + sp.template, Subject: subject, Data: data, Layout: sp.layout})
		if err != nil {
...
// other kind is supported only when a BodySender for that kind is wired. A start
...
// promptly as a kind-tagged DeliveryError). Email uses the Mailer with both
// HTML and text; other kinds use their configured BodySender. The selected
// delivery is dispatched through notify.Send. Unsupported kinds return
// ErrKindUnsupported. DeliveryError wraps the cause without including content
// in its own error message; hosts must handle provider diagnostics deliberately.
func (r *Router) Deliver(ctx context.Context, kind string, env Envelope) error {
	...
	if kind == sdk.AddressKindEmail {
		...
		if err := notify.Send(ctx, email.NewDelivery(r.mailer, msg)); err != nil {
			return &DeliveryError{Kind: kind, Err: err}
		}
		return nil
	}

	n, ok := r.bodySenders[kind]
	if !ok {
...
// notify delivers env through a BodySender, wrapping a transport failure in a
// kind-tagged DeliveryError.
func (r *Router) notify(ctx context.Context, n BodySender, kind string, env Envelope) error {
	if err := notify.Send(ctx, notify.DeliveryFunc(func(ctx context.Context) error { return n.Send(ctx, env.Destination, env.Body) })); err != nil {
		return &DeliveryError{Kind: kind, Err: err}
	}
	return nil
}
...
// ValidateTransports checks the actual configured senders against the host's
// runtime mode, including when this router is composed without the pocket root.
func (r *Router) ValidateTransports(mode environment.Mode) error {
	if _, err := notify.CheckTransport(mode, r.mailer); err != nil {
		return err
	}
	for _, sender := range r.bodySenders {
		if _, err := notify.CheckTransport(mode, sender); err != nil {
			return err
		}
	}
	return nil
