...
import (
	...
	"github.com/gopernicus/gopernicus/pockets/authentication/logic/delivery/internal/deliverychar"
...
func newInProcessHarness(t *testing.T, s deliverychar.Scenario) deliverychar.Harness {
	...
	enc := fakeEncrypter{}
	router := newRouter(t, &stubSender{}, map[string]BodySender{sdk.AddressKindPhone: BodySender(s.Provider)})

	queue := newInProcessQueue(inProcessQueueConfig{Capacity: 64})
	svc, err := NewService(queue, enc)
	if err != nil {
		t.Fatalf("NewService: %v", err)
	}

	procDeps := jobsProcessorConfig{
		Encrypter: enc,
		Router:    router,
	...
	proc, err := NewJobsProcessor(procDeps.Encrypter,
		procDeps.Router,
		WithProcessorInitializer(procDeps.Initializer),
		WithProcessorClock(procDeps.Now),
		WithProcessorPolicy(procDeps.Config),
		WithProcessorObserver(procDeps.Observer))
	if err != nil {
		t.Fatalf("NewJobsProcessor: %v", err)
	}

	rt, err := NewInProcessRuntime(queue,
		proc,
		WithRetryPolicy(RetryPolicyConfig{MaxAttempts: s.MaxAttempts, Backoff: func(int) time.Duration { return 0 }}),
		WithWorkerCount(1))
	if err != nil {
