...
// is a BodySender so every characterized runtime (all deliver through notify)
...
func NewProvider(failFirst int, block bool) *Provider {
	return &Provider{failFirst: failFirst, block: block}
}

// Send records the send and applies the primed failure/block behavior.
func (p *Provider) Send(ctx context.Context, to string, body string) error {
	if p.block {
		<-ctx.Done()
		return ctx.Err()
	}
	p.mu.Lock()
	defer p.mu.Unlock()
	p.sends = append(p.sends, Send{To: to, Body: body})
