...
// but the actual send (Router.Deliver, an email.Sender.Send, or a BodySender
...
// It scans every sibling package of delivery under internal/logic (authentication service,
// invitation service, and any future producer) so a newly added producer is covered
...
// send; Send is an email.Sender; Notify is a BodySender.
...
func TestNoProducerBypassesDispatcherSeam(t *testing.T) {
	...
	for _, want := range []string{"authentication", "invitations"} {
		found := false
