...
func TestInProcessRuntimeConstructionNegatives(t *testing.T) {
	...
	if _, err := NewInProcessRuntime(nil,
		newCountingProcessor(1)); !errors.Is(err, ErrInProcessQueueRequired) {
		t.Fatalf("nil queue err = %v, want ErrInProcessQueueRequired", err)
	}
	if _, err := NewInProcessRuntime(q,
		nil); !errors.Is(err, ErrInProcessProcessorRequired) {
