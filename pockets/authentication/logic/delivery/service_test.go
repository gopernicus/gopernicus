...
func TestNewServiceRequiresCollaborators(t *testing.T) {
	if _, err := NewService(nil, fakeEncrypter{}); !errors.Is(err, ErrDispatcherRequired) {
		t.Fatalf("missing dispatcher err=%v, want ErrDispatcherRequired", err)
	}
	if _, err := NewService(newMemDispatcher(), nil); !errors.Is(err, ErrEncrypterRequired) {
...
func TestServiceEnqueueIdempotent(t *testing.T) {
	...
	svc, _ := NewService(disp, fakeEncrypter{})
	cmd := Command{Kind: sdk.AddressKindEmail, Purpose: PurposeRegistrationVerification, IdempotencyKey: "same", Envelope: Envelope{Destination: "u@x", Body: "b"}}
...
func TestServiceEnqueueCommandIncomplete(t *testing.T) {
	svc, _ := NewService(newMemDispatcher(), fakeEncrypter{})
	for _, cmd := range []Command{
		{Purpose: PurposeMagicLink, IdempotencyKey: "k"},
		{Kind: sdk.AddressKindEmail, IdempotencyKey: "k"},
		{Kind: sdk.AddressKindEmail, Purpose: PurposeMagicLink},
	} {
...
func TestServiceEnqueueEncryptionFailure(t *testing.T) {
	fd := &fakeDispatcher{state: string(work.StatusPending)}
	svc, _ := NewService(fd, fakeEncrypter{encErr: errBoom})
	_, err := svc.Enqueue(context.Background(), Command{Kind: sdk.AddressKindEmail, Purpose: PurposeMagicLink, IdempotencyKey: "k", Envelope: Envelope{Destination: "u@x", Body: "b"}})
...
func TestServiceReplaceSupersedes(t *testing.T) {
	disp := newMemDispatcher()
	svc, _ := NewService(disp, fakeEncrypter{})
	first, _ := svc.Enqueue(context.Background(), Command{Kind: sdk.AddressKindEmail, Purpose: PurposeMagicLink, IdempotencyKey: "k", Envelope: Envelope{Destination: "u@x", Body: "old"}})
	second, err := svc.Replace(context.Background(), Command{Kind: sdk.AddressKindEmail, Purpose: PurposeMagicLink, IdempotencyKey: "k", Envelope: Envelope{Destination: "u@x", Body: "new"}})
	if err != nil {
