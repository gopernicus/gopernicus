...
// HTML golden output (captured on main 2026-08-28, before DataHook and the
...
// enrichedData is the current pocket-built invitation data shape (every field
...
func phoneNotifiers() map[string]BodySender {
	return map[string]BodySender{sdk.AddressKindPhone: &stubNotifier{}}
}

func newDataRouter(t *testing.T, d routerConfig) *Router {
	t.Helper()
	if d.Mailer == nil {
		d.Mailer = &stubSender{}
	}
	d.MailFrom = "no-reply@example.test"
	if d.BodySenders == nil {
		d.BodySenders = phoneNotifiers()
	}
	r, err := NewRouter(d.Mailer,
		WithTemplates(TemplatesConfig{AppTemplates: d.AppTemplates, AppLayouts: d.AppLayouts, Branding: d.Branding, Subjects: d.Subjects, SMSBodies: d.SMSBodies}),
		WithMailFrom(d.MailFrom),
		WithBodySenders(d.BodySenders),
		WithDataHook(d.DataHook),
		WithRouterLogger(d.Logger))
	if err != nil {
...
// A nil hook and empty override maps preserve HTML and the deliberate text templates
...
func TestRenderInvitationAndMemberAddedUnchangedWithoutHook(t *testing.T) {
	r := newDataRouter(t, routerConfig{})
	cases := []struct {
		purpose, kind, subject, body, htmlSHA string
	}{
		{PurposeInvitation, sdk.AddressKindEmail, "You have an invitation", goldenInvitationText, goldenInvitationHTMLSHA},
		{PurposeInvitation, sdk.AddressKindPhone, "", "You were invited to project p1 as member: " + goldenLink, sha("")},
		{PurposeMemberAdded, sdk.AddressKindEmail, "You were added", goldenMemberAddedText, goldenMemberAddedHTMLSHA},
		{PurposeMemberAdded, sdk.AddressKindPhone, "", "You were added to project p1 as member: " + goldenLink, sha("")},
	}
...
func TestRenderDataHookSeesPurposeAndSecretFreeData(t *testing.T) {
	...
	r := newDataRouter(t, routerConfig{DataHook: func(_ context.Context, purpose string, data map[string]any) (map[string]any, error) {
		gotPurpose, gotData = purpose, data
		return nil, nil
	}})
	env, err := r.Render(context.Background(), Request{Kind: sdk.AddressKindEmail, Purpose: PurposeInvitation, Destination: "x", Secret: "TOK", Data: enrichedData()})
	if err != nil {
...
func TestRenderDataHookAddsAndReplacesFields(t *testing.T) {
	r := newDataRouter(t, routerConfig{DataHook: func(_ context.Context, _ string, _ map[string]any) (map[string]any, error) {
		return map[string]any{"ResourceName": "Apollo", "Relation": "owner"}, nil
	}})
	for _, kind := range []string{sdk.AddressKindEmail, sdk.AddressKindPhone} {
...
func TestRenderDataHookReservedFieldsRejected(t *testing.T) {
	for _, field := range []string{"Secret", "Link", "Subject"} {
		sender := &stubSender{}
		r := newDataRouter(t, routerConfig{Mailer: sender, DataHook: func(_ context.Context, _ string, _ map[string]any) (map[string]any, error) {
...
func TestRenderDataHookInputIsDefensiveCopy(t *testing.T) {
	r := newDataRouter(t, routerConfig{DataHook: func(_ context.Context, _ string, data map[string]any) (map[string]any, error) {
	...
	orig := enrichedData()
	env, err := r.Render(context.Background(), Request{Kind: sdk.AddressKindEmail, Purpose: PurposeInvitation, Destination: "x", Secret: "TOK", Data: orig})
	if err != nil {
		t.Fatalf("Render: %v", err)
	}
...
func TestRenderRequestSecretIsAuthoritative(t *testing.T) {
	...
	env, err := r.Render(context.Background(), Request{Kind: sdk.AddressKindPhone, Purpose: PurposeLoginCode, Destination: "x", Secret: "654321", Data: map[string]any{"Secret": "EVIL"}})
	if err != nil {
		t.Fatalf("Render: %v", err)
	}
	if sawSecret {
...
func TestRenderDataHookRunsForEveryPurpose(t *testing.T) {
	...
	for purpose, sp := range specs {
		if _, err := r.Render(context.Background(), Request{Kind: sdk.AddressKindEmail, Purpose: purpose, Destination: "x", Secret: "S", Data: data}); err != nil {
			t.Fatalf("Render(%s/email): %v", purpose, err)
		}
		if sp.sms == "" {
			continue
		}
		if _, err := r.Render(context.Background(), Request{Kind: sdk.AddressKindPhone, Purpose: purpose, Destination: "x", Secret: "S", Data: data}); err != nil {
...
func TestRenderDataHookConcurrent(t *testing.T) {
	...
	for i := range n {
		...
		go func() {
			...
			env, err := r.Render(context.Background(), Request{Kind: sdk.AddressKindPhone, Purpose: PurposeInvitation, Destination: "x", Secret: "TOK", Data: data})
			if err != nil {
				errs <- err
				return
			}
...
func TestRenderSubjectOverridePrecedence(t *testing.T) {
	...
	other, err := r.Render(context.Background(), Request{Kind: sdk.AddressKindEmail, Purpose: PurposeRegistrationVerification, Destination: "x", Secret: "123456"})
	if err != nil {
...
func TestRenderSMSOverridePrecedence(t *testing.T) {
	r := newDataRouter(t, routerConfig{SMSBodies: map[string]string{PurposeInvitation: "Join {{.ResourceType}} {{.ResourceID}}: {{.Link}}"}})
	sms, err := r.Render(context.Background(), Request{Kind: sdk.AddressKindPhone, Purpose: PurposeInvitation, Destination: "x", Secret: "TOK", Data: enrichedData()})
	if err != nil {
	...
	mail, err := r.Render(context.Background(), Request{Kind: sdk.AddressKindEmail, Purpose: PurposeInvitation, Destination: "x", Secret: "TOK", Data: enrichedData()})
	if err != nil {
	...
	other, err := r.Render(context.Background(), Request{Kind: sdk.AddressKindPhone, Purpose: PurposeMemberAdded, Destination: "x", Secret: "TOK", Data: enrichedData()})
	if err != nil {
...
func TestNewRouterOverrideRejections(t *testing.T) {
	cases := []struct {
		...
		{"subject unknown purpose", routerConfig{Subjects: map[string]string{"nope": "x"}}, `unknown purpose "nope"`},
		{"sms unknown purpose", routerConfig{SMSBodies: map[string]string{"nope": "x"}}, `unknown purpose "nope"`},
		{"subject empty", routerConfig{Subjects: map[string]string{PurposeInvitation: "  \n"}}, "is empty"},
		{"sms empty", routerConfig{SMSBodies: map[string]string{PurposeInvitation: ""}}, "is empty"},
		{"sms for email-only purpose", routerConfig{SMSBodies: map[string]string{PurposePasswordReset: "Reset: {{.Link}}"}}, "email-only purpose"},
		{"subject parse failure", routerConfig{Subjects: map[string]string{PurposeInvitation: "{{.Unclosed"}}, "parse subject override"},
		{"sms parse failure", routerConfig{SMSBodies: map[string]string{PurposeInvitation: "{{if}}"}}, "parse sms override"},
	}
	for _, c := range cases {
		c.deps.Mailer = &stubSender{}
		_, err := NewRouter(c.deps.Mailer,
			WithTemplates(TemplatesConfig{AppTemplates: c.deps.AppTemplates, AppLayouts: c.deps.AppLayouts, Branding: c.deps.Branding, Subjects: c.deps.Subjects, SMSBodies: c.deps.SMSBodies}),
			WithMailFrom(c.deps.MailFrom),
			WithBodySenders(c.deps.BodySenders),
			WithDataHook(c.deps.DataHook),
			WithRouterLogger(c.deps.Logger))
...
func TestRenderOverrideMissingKeyFails(t *testing.T) {
	...
	if _, err := r.Render(context.Background(), Request{Kind: sdk.AddressKindEmail, Purpose: PurposeInvitation, Destination: "x", Secret: "TOK", Data: enrichedData()}); err == nil || !strings.Contains(err.Error(), "Nope") {
		t.Errorf("subject with missing key: err=%v, want a missing-key failure", err)
	}
	if _, err := r.Render(context.Background(), Request{Kind: sdk.AddressKindPhone, Purpose: PurposeMemberAdded, Destination: "x", Secret: "TOK", Data: enrichedData()}); err == nil || !strings.Contains(err.Error(), "Nope") {
...
func TestRenderSMSIgnoresSubjectTemplate(t *testing.T) {
	r := newDataRouter(t, routerConfig{Subjects: map[string]string{PurposeInvitation: "{{.Nope}}"}})
	env, err := r.Render(context.Background(), Request{Kind: sdk.AddressKindPhone, Purpose: PurposeInvitation, Destination: "x", Secret: "TOK", Data: enrichedData()})
	if err != nil {
		t.Fatalf("Render(sms): %v", err)
	}
