...
func TestServiceConsumesDispatcher(t *testing.T) {
	...
	sub := fd.submits[0]
	if sub.kind != sdk.AddressKindEmail || sub.purpose != PurposeRegistrationVerification || sub.key != "svc-key" {
	...
	if _, err := svc.Replace(ctx, Command{Kind: sdk.AddressKindEmail, Purpose: PurposeMagicLink, IdempotencyKey: "svc-key", Envelope: Envelope{Destination: "u@x.test", Body: "b"}}); err != nil {
...
func TestDeliveryStatusNoLeak(t *testing.T) {
	ctx := context.Background()
	fd := &fakeDispatcher{state: string(work.StatusPending)}
	svc, err := NewService(fd, fakeEncrypter{})
	if err != nil {
		t.Fatalf("NewService: %v", err)
	}
