...
func sealOpaqueEnvelope(t *testing.T, key string) []byte {
	t.Helper()
	env, err := deliverycmd.NewOpaque(sdk.AddressKindPhone, "characterization", key, "+15550001111")
	if err != nil {
...
func sealRenderedEnvelope(t *testing.T, key, secret string) []byte {
	t.Helper()
	env, err := deliverycmd.NewRendered(sdk.AddressKindPhone, "characterization", key, "+15550001111", "", "Sign in: "+secret, "", secret)
	if err != nil {
...
func phoneRouter(t *testing.T, notifier BodySender) *Router {
	t.Helper()
	return newRouter(t, &stubSender{}, map[string]BodySender{sdk.AddressKindPhone: notifier})
}
...
func (n *recordingNotifier) Send(_ context.Context, _ string, body string) error {
	n.mu.Lock()
	defer n.mu.Unlock()
...
func newInProcProcessor(t *testing.T, notifier BodySender, init Initializer, obs deliverycmd.Observer, cfg deliverycmd.Config) *JobsProcessor {
	t.Helper()
	p, err := NewJobsProcessor(fakeEncrypter{},
		phoneRouter(t, notifier),
		WithProcessorInitializer(init),
		WithProcessorPolicy(cfg),
		WithProcessorObserver(obs))
	if err != nil {
...
func TestInProcessRetryReusesSameSecret(t *testing.T) {
	t.Parallel()
	notifier := &recordingNotifier{failFirst: 1} // first send fails → retry
	...
	rt, err := NewInProcessRuntime(q,
		proc,
		WithRetryPolicy(RetryPolicyConfig{Backoff: func(int) time.Duration { return time.Millisecond }}),
		WithWorkerCount(1),
		WithShutdownDeadline(2*time.Second))
	if err != nil {
...
func TestInProcessAttemptCapDeadLettersThenDiscards(t *testing.T) {
	...
	rt, err := NewInProcessRuntime(q,
		proc,
		WithRetryPolicy(RetryPolicyConfig{MaxAttempts: maxAttempts, Backoff: func(int) time.Duration { return time.Millisecond }}),
		WithWorkerCount(1),
		WithShutdownDeadline(2*time.Second))
	if err != nil {
		t.Fatalf("NewInProcessRuntime: %v", err)
...
func TestInProcessPermanentDeadLettersImmediately(t *testing.T) {
	...
	rt, err := NewInProcessRuntime(q,
		proc,
		WithRetryPolicy(RetryPolicyConfig{MaxAttempts: 5, Backoff: func(int) time.Duration { return time.Second }}),
		WithWorkerCount(1),
		WithShutdownDeadline(2*time.Second))
	if err != nil {
		t.Fatalf("NewInProcessRuntime: %v", err)
...
func TestInProcessBackoffCancellationPrompt(t *testing.T) {
	...
	rt, err := NewInProcessRuntime(q,
		proc,
		WithRetryPolicy(RetryPolicyConfig{MaxAttempts: 5, Backoff: func(int) time.Duration { return 10 * time.Second }}),
		WithWorkerCount(1),
		WithShutdownDeadline(2*time.Second))
	if err != nil {
		t.Fatalf("NewInProcessRuntime: %v", err)
...
func TestInProcessSupersededDuringBackoffCannotTransition(t *testing.T) {
	...
	rt, err := NewInProcessRuntime(q,
		proc,
		WithRetryPolicy(RetryPolicyConfig{MaxAttempts: 5, Backoff: func(int) time.Duration { return backoff }}),
		WithWorkerCount(2),
		WithShutdownDeadline(2*time.Second))
	if err != nil {
		t.Fatalf("NewInProcessRuntime: %v", err)
...
func TestInProcessObserverTransitions(t *testing.T) {
	...
	notifier := &recordingNotifier{failFirst: 1_000_000} // never succeeds
	...
	rt, err := NewInProcessRuntime(q,
		proc,
		WithRetryPolicy(RetryPolicyConfig{MaxAttempts: 2, Backoff: func(int) time.Duration { return time.Millisecond }}),
		WithWorkerCount(1),
		WithShutdownDeadline(2*time.Second))
	if err != nil {
		t.Fatalf("NewInProcessRuntime: %v", err)
...
func (n *blockingCountingNotifier) Send(ctx context.Context, _, _ string) error {
	n.calls.Add(1)
	<-ctx.Done()
	return ctx.Err()
}
...
func TestInProcessProviderTimeoutBounded(t *testing.T) {
	t.Parallel()
	const maxAttempts = 3
	notifier := &blockingCountingNotifier{}
...
var (
	...
	_ BodySender           = (*blockingCountingNotifier)(nil)
