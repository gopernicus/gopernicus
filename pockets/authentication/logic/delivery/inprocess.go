...
// runs it.
func NewInProcessQueue(opts ...QueueOption) (*InProcessQueue, error) {
	cfg := inProcessQueueConfig{}
	for _, opt := range opts {
		if opt == nil {
			return nil, fmt.Errorf("authentication NewInProcessQueue: nil option: %w", sdk.ErrInvalidInput)
		}
		opt(&cfg)
	}
	...
	return newInProcessQueue(cfg), nil
}
...
// host explicitly runs this via Service.Runtime.Run. Accepted, in-flight work is LOST
...
func NewInProcessRuntime(queue *InProcessQueue, processor InProcessProcessor, opts ...InProcessRuntimeOption) (*InProcessRuntime, error) {
	cfg := inProcessRuntimeConfig{}
	for _, opt := range opts {
		if opt == nil {
			return nil, fmt.Errorf("authentication NewInProcessRuntime: nil option: %w", sdk.ErrInvalidInput)
		}
		opt(&cfg)
	}
...
// included. It mirrors the generic fenced runner's processOnce recover (sdk/pkg/workers)
