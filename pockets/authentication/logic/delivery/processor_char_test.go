...
import (
	...
	cmd "github.com/gopernicus/gopernicus/pockets/authentication/logic/delivery/command"
...
func newProcessorHarnessWith(t *testing.T, s deliverychar.Scenario, obs cmd.Observer) deliverychar.Harness {
	...
	var initializer cmd.Initializer
	if s.Initializer != nil {
		initializer = charInit{n: s.Initializer}
	}
	proc, err := cmd.NewProcessor(fakeEncrypter{}, routerDeliverer{router: router},
		cmd.WithInitializer(initializer), cmd.WithClock(clk.now),
		cmd.WithPolicy(cmd.Config{MaxAttempts: s.MaxAttempts, ProviderTimeout: s.ProviderTimeout, Backoff: s.Backoff}),
	)
	if err != nil {
