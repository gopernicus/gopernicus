...
// jobsProcessorConfig holds the collaborators NewJobsProcessor needs. Encrypter and
...
type jobsProcessorConfig struct {
...
func NewJobsProcessor(encrypter cryptids.Encrypter, router *Router, opts ...ProcessorOption) (*JobsProcessor, error) {
	d := jobsProcessorConfig{}
	d.Encrypter = encrypter
	d.Router = router
	for _, opt := range opts {
		if opt == nil {
			return nil, fmt.Errorf("authentication NewJobsProcessor: nil option: %w", sdk.ErrInvalidInput)
		}
		opt(&d)
	}

	if nilDependency(d.Encrypter) {
		return nil, ErrEncrypterRequired
	}
	if d.Initializer != nil && nilDependency(d.Initializer) {
		return nil, fmt.Errorf("delivery: initializer contains a typed nil: %w", sdk.ErrInvalidInput)
	}
	if d.Router == nil {
	...
	engine, err := deliverycmd.NewProcessor(d.Encrypter, commandDeliverer{router: d.Router},
		deliverycmd.WithInitializer(initializer),
		deliverycmd.WithClock(d.Now),
		deliverycmd.WithPolicy(d.Config),
	)
	if err != nil {
		return nil, err
	}
