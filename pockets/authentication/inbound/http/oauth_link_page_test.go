...
func newPendingLinkFixture(t *testing.T, views Views) pendingLinkFixture {
	...
	svc := newServiceWithFakes(authenticationFixture{
		...
		OAuthNativeRedirectURIs: []string{"com.example.app:/oauth"},
		TrustOAuthEmail:         func(string, oauth.UserInfo) bool { return true },
	})
	now := time.Now().UTC()
	users.mu.Lock()
	...
	h := web.NewWebHandler()
	mount(h, mountDeps{Auth: svc, Mutation: MutationSecurity{AllowedOrigins: []string{"https://app.example.com"}}, Views: views})
...
func (f pendingLinkFixture) pendingLinkToken(t *testing.T) string {
	...
	cb := do(t, f.h, "GET", "/auth/oauth/google/callback?code=x&state="+url.QueryEscape(flowState), "", start.Result().Cookies()...)
...
func TestOAuthLinkLandingDenyByAbsence(t *testing.T) {
	...
	h := web.NewWebHandler()
	mount(h, mountDeps{Auth: svc, Views: stubViews{}})
