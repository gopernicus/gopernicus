...
func (h *handlers) startIdentifierChange(w http.ResponseWriter, r *http.Request, kind identifier.Kind, value string, uses identifierUsesDTO, makePrimary bool) {
	...
	receipt, err := h.svc.StartIdentifierChange(r.Context(), authlogic.IdentifierChangeStart{
		SessionID:   sessionID,
		UserID:      userID,
		Kind:        kind,
		Value:       value,
...
func (h *handlers) patchIdentifier(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.svc.SetIdentifierUses(r.Context(), authlogic.IdentifierUsesInput{
		SessionID:    sessionID,
		UserID:       userID,
...
func (h *handlers) deleteIdentifier(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.svc.RemoveIdentifier(r.Context(), authlogic.IdentifierRemoveInput{
		SessionID:     sessionID,
		UserID:        userID,
		IdentifierID:  web.Param(r, "id"),
