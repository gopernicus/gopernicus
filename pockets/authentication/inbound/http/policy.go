...
// policy. Validation is LOUD at construction (the pocket's adapterConfig posture), never a
...
// Keep the adapter's validated policy independent of a host replacing its value.
func snapshotHTMLPolicy(policy *HTMLResourcePolicy) *HTMLResourcePolicy {
	if policy == nil {
		return nil
	}
	result := &HTMLResourcePolicy{directives: append([]HTMLResourceDirective(nil), policy.directives...)}
	for i := range result.directives {
		result.directives[i].Sources = append([]string(nil), result.directives[i].Sources...)
	}
	return result
}
