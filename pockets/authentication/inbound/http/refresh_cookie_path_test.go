...
// test can drive the prefixed-host topology (pockets.PrefixRegistrar) end to end.
func newRefreshPathHandler(t *testing.T, refreshPath, prefix string) http.Handler {
	...
	}, authenticatorConfig{Cookie: CookieConfig{RefreshPath: refreshPath}})
	h := web.NewWebHandler()
	var r pockets.RouteRegistrar = h
	if prefix != "" {
		r = pockets.PrefixRegistrar{Prefix: prefix, Next: h}
	}
	mount(r, mountDeps{Auth: svc})
	return h
}
