...
func TestStrictJSONBody(t *testing.T) {
	...
	tests := []struct {
		...
		{"null preserved", `null`, maxJSONBodyBytes, true, http.StatusOK},
		...
		{"oversized trailing whitespace", `{"name":"a"}` + strings.Repeat(" ", 32), 16, false, http.StatusRequestEntityTooLarge},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func TestSpoofedForwardedHeaderCannotRotateLimiterBucket(t *testing.T) {
	...
	next := http.HandlerFunc(func(_ http.ResponseWriter, r *http.Request) {
		carrierIP, _ = authlogic.ClientInfoFromContext(r.Context())
	})
