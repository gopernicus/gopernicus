...
// var _ pins the seam: *testService is the concrete authService the router
// mounts.
var _ authService = (*testService)(nil)

// --- minimal fakes (transport tests wire a real authlogic.Service over these) ---
...
// is unknown. It keeps the real authlogic.Service's send sites off the fail-closed
...
func (m *memUsers) CreateWithPrimaryIdentifier(ctx context.Context, u user.User, ident identifier.Identifier) (user.User, identifier.Identifier, error) {
	return m.Provision(ctx, u, ident, user.InitialCredentials{})
}
...
func (p *memPasswords) Set(ctx context.Context, id, hash string) error {
	p.users.mu.Lock()
	defer p.users.mu.Unlock()
	_, err := p.changeLocked(id, user.PasswordChange{NewHash: hash, Now: time.Now().UTC()}, false)
	return err
}
...
func (f *memChallenges) Replace(_ context.Context, c challenge.Challenge) (challenge.Challenge, error) {
	...
	for id, ex := range f.byID {
		if ex.ResolvedSubjectKey() == c.ResolvedSubjectKey() && ex.Purpose == c.Purpose {
			delete(f.byID, id)
		}
	}
	if c.ID == "" {
...
// this stand-in coordinates those stores through the same user mutex.
...
func (f *memPasswordResets) Redeem(ctx context.Context, in passwordreset.RedeemInput) (passwordreset.RedeemResult, error) {
	f.pw.users.mu.Lock()
	defer f.pw.users.mu.Unlock()
	f.ch.mu.Lock()
	var proof challenge.Challenge
	for _, c := range f.ch.byID {
		if c.Purpose == in.Purpose && c.SecretDigest == in.TokenDigest && !c.Expired(in.Now) {
			proof = c
			break
		}
	}
	f.ch.mu.Unlock()
	owner, ok := f.pw.users.byID[proof.UserID]
	if !ok || !owner.Active() {
		return passwordreset.RedeemResult{}, sdk.ErrNotFound
	}
	binding, err := passwordreset.ParseBinding(proof.Context)
	if err != nil {
		return passwordreset.RedeemResult{}, err
	}
	ident, err := f.pw.users.idents.Get(ctx, binding.IdentifierID)
	if err != nil || !binding.Matches(owner.ID, owner.AuthRevision, ident) {
		return passwordreset.RedeemResult{}, sdk.ErrNotFound
	}
	if _, ok := f.ch.takeLiveToken(in.Purpose, in.TokenDigest, in.Now); !ok {
		return passwordreset.RedeemResult{}, sdk.ErrNotFound
	}
	if _, err := f.pw.changeLocked(owner.ID, user.PasswordChange{NewHash: in.NewPasswordHash, Now: in.Now}, false); err != nil {
		return passwordreset.RedeemResult{}, err
	}
	f.ch.purgeUserPurposes(owner.ID, in.PurgeChallengePurposes)
	return passwordreset.RedeemResult{UserID: owner.ID}, nil
}
...
func (f *memIdentifiers) ApplyVerifiedChange(_ context.Context, input identifier.ApplyVerifiedChangeInput, expectedAuthRevision int64, verifiedAt time.Time) (identifier.Identifier, error) {
	...
	u, ok := f.users.byID[input.UserID]
	if !ok {
		return identifier.Identifier{}, sdk.ErrNotFound
	}
...
// newTestHandler builds a real authlogic.Service over in-memory fakes, mounts the
