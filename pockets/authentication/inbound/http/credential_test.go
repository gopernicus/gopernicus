...
func (h *credentialHarness) expiredAccessJWT(t *testing.T) string {
	t.Helper()
	tok, err := h.signer.Sign(map[string]any{"user_id": h.userID, "session_id": h.sessionID}, time.Now().Add(-time.Hour))
	if err != nil {
...
func (h *credentialHarness) badSignatureAccessJWT(t *testing.T) string {
	...
	tok, err := other.Sign(map[string]any{"user_id": h.userID, "session_id": h.sessionID}, time.Now().Add(time.Hour))
	if err != nil {
...
func TestRouteAuthenticationOptionSnapshotsCredentialPolicy(t *testing.T) {
	h := newCredentialHarness(t)
	rawKey, _, _ := h.mintKey(t, "option-snapshot", false, "", time.Time{})
	kinds := []CredentialKind{CredentialAccessToken}
	transports := []Transport{TransportHeader}
	option := WithRouteAuthentication(BundledRouteAuthentication{
		SessionHydration: PrincipalStrategy(Accept(kinds...), Transports(transports...)),
	})

	first, err := New(h.svc.Service, environment.ModeDevelopment, option)
	if err != nil {
		t.Fatal(err)
	}
	kinds[0] = CredentialAPIKey
	transports[0] = TransportCookie
	second, err := New(h.svc.Service, environment.ModeDevelopment, option)
	if err != nil {
		t.Fatal(err)
	}

	for name, adapter := range map[string]*Adapter{"first": first, "reused": second} {
		t.Run(name, func(t *testing.T) {
			for _, tc := range []struct {
				...
				{"access-header", h.accessJWT, "", http.StatusOK},
				{"access-cookie", "", h.accessJWT, http.StatusUnauthorized},
				{"key-header", rawKey, "", http.StatusUnauthorized},
				{"key-cookie", "", rawKey, http.StatusUnauthorized},
			} {
				t.Run(tc.name, func(t *testing.T) {
					rec, got := serveGuarded(h, adapter.routes.RouteAuth.SessionHydration, h.credentialRequest(tc.header, tc.cookie))
					if rec.Code != tc.status {
						t.Fatalf("status=%d, want %d: %s", rec.Code, tc.status, rec.Body.String())
					}
					if tc.status == http.StatusOK && (!got.reached || got.cred.Kind != CredentialAccessToken || got.cred.Transport != TransportHeader) {
						t.Fatalf("credential policy changed: %+v", got)
					}
				})
			}
		})
...
func TestRequirePrincipalCredentialMatrix(t *testing.T) {
	...
	principals := map[string]Principal{
		"":     {Type: authlogic.PrincipalUser, ID: h.userID},
		"self": {Type: authlogic.PrincipalServiceAccount, ID: selfSAID},
		"act":  {Type: authlogic.PrincipalUser, ID: owner},
	}
...
func TestAPIKeyCredentialAuditRail(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		...
		if e.Actor.Type != authlogic.PrincipalServiceAccount || e.Actor.ID != saID {
...
func TestCurrentCredentialAbsent(t *testing.T) {
	...
	ctx := sdk.WithPrincipal(context.Background(), Principal{Type: authlogic.PrincipalUser, ID: "u1"})
...
func TestRequirePrincipalOptional(t *testing.T) {
	...
	t.Run("b) valid cookie admits the user principal", func(t *testing.T) {
		...
		if want := (Principal{Type: authlogic.PrincipalUser, ID: h.userID}); !got.principalOK || got.principal != want {
