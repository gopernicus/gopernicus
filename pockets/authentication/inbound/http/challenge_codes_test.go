...
func TestRespondDomainErrorNamedChallengeCodes(t *testing.T) {
	tests := []struct {
		...
		{"expired", authlogic.ErrChallengeExpired, http.StatusGone, "challenge_expired"},
		{"invalid", authlogic.ErrChallengeInvalid, http.StatusBadRequest, "challenge_invalid"},
		{"locked out", authlogic.ErrTooManyAttempts, http.StatusForbidden, "too_many_attempts"},
