...
func (stubProvider) GetAuthorizationURL(req oauth.AuthorizationRequest) (string, error) {
	return "https://accounts.google.example/authorize?state=" + req.State, nil
}
func (stubProvider) ExchangeCode(context.Context, string, string, string) (*oauth.TokenResponse, error) {
	return &oauth.TokenResponse{AccessToken: "access-tok", TokenType: "Bearer"}, nil
}
...
// newOAuthTestHandler builds a real oauth-enabled authlogic.Service over in-memory
