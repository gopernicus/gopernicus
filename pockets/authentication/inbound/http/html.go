...
// Mount ONLY when a Views port is wired (BrowserConfig.Views != nil); a nil Views leaves
...
func mountHTML(r pockets.RouteRegistrar, h *handlers, browserAccount, browserSafe web.Middleware) {
...
func (h *handlers) stepUpModel(ctx context.Context, userID string, pc PageContext, p stepUpParams) StepUpPage {
	...
	for _, it := range view.Identifiers {
		if it.Uses.Recovery && !it.VerifiedAt.IsZero() && (p.kind == "" || p.kind == it.Kind) {
			m.Kind, m.MaskedIdentifier = it.Kind, it.MaskedValue
			break
		}
	}
...
// inventory.
func maskedRecovery(view authlogic.MethodsView) string {
	for _, it := range view.Identifiers {
...
func maskedPrimary(view authlogic.MethodsView) string {
