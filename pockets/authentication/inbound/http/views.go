...
// Implementing every method from scratch (e.g. over sdk/pkg/web.Template) is
// possible but not the sold path. The pocket core never imports templ.
//
// A nil BrowserConfig.Views means the HTML surface is not registered: the HTML GET pages
...
type StepUpPage struct {
	...
	Purpose string
	Context string
	// Kind identifies the recovery channel used by both code issuance and completion.
	Kind string
