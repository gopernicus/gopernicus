...
// authService is the bundled HTTP adapter's service contract. *authlogic.Service
...
type AuthenticationService interface {
	...
	Login(ctx context.Context, email, password string) (pair authlogic.TokenPair, u user.User, err error)
	Logout(ctx context.Context, refreshToken, accessToken string) error
	Refresh(ctx context.Context, refreshToken string) (authlogic.TokenPair, error)
	ChangePassword(ctx context.Context, userID, currentPassword, newPassword string) (pair authlogic.TokenPair, err error)
	ForgotPassword(ctx context.Context, email string) error
	// ResendVerification is the PUBLIC registration-verification resend: it
	// normalizes, rate-limits, and submits an opaque issuance command, resolving no
	...
	SetPassword(ctx context.Context, sessionID, userID, newPassword string) (pair authlogic.TokenPair, err error)
	StartRemovePassword(ctx context.Context, userID string) (authlogic.StepUpReceipt, error)
	RemovePassword(ctx context.Context, userID, code string) (pair authlogic.TokenPair, err error)
	...
	CurrentPrincipal(ctx context.Context) (authlogic.Principal, bool)
	...
	CurrentUserView(ctx context.Context, userID string) (authlogic.CurrentUserView, error)
	...
	Methods(ctx context.Context, userID string) (authlogic.MethodsView, error)
	...
	BeginStepUp(ctx context.Context, in authlogic.StepUpStart) (authlogic.StepUpReceipt, error)
	CompleteStepUpWithPassword(ctx context.Context, in authlogic.StepUpCompletion, password string) (authgrant.Grant, error)
	CompleteStepUpWithIdentifierCode(ctx context.Context, in authlogic.StepUpCompletion, code string) (authgrant.Grant, error)
	...
	// authentication.BrowserConfig.BundledRouteAuth.
	...
	OAuthEnabled() bool
	OAuthNativeEnabled() bool
	OAuthBrowserSecure() bool
	OAuthProviderNames() []string
	StartOAuth(ctx context.Context, provider string, req authlogic.OAuthStartRequest) (authlogic.OAuthStart, error)
	StartLink(ctx context.Context, userID, provider string, req authlogic.OAuthStartRequest) (authlogic.OAuthStart, error)
	OAuthCallback(ctx context.Context, provider string, req authlogic.OAuthCallbackRequest) (authlogic.OAuthResult, error)
	VerifyLink(ctx context.Context, token string) (authlogic.OAuthResult, error)
	...
	StartUnlinkOAuth(ctx context.Context, userID, provider string) (authlogic.StepUpReceipt, error)
	...
	StartIdentifierChange(ctx context.Context, in authlogic.IdentifierChangeStart) (authlogic.StepUpReceipt, error)
	ConfirmIdentifierChange(ctx context.Context, in authlogic.IdentifierChangeConfirm) error
	...
	ResendVerificationForUser(ctx context.Context, actor authlogic.Principal, userID string) (authlogic.StepUpReceipt, error)
	GetUserSummary(ctx context.Context, id string) (user.Summary, error)
	SetUserStatus(ctx context.Context, actor authlogic.Principal, id string, status user.Status) (user.Summary, user.StatusChange, error)
	...
	ListServiceAccounts(ctx context.Context, req list.Request) (list.Page[serviceaccount.ServiceAccount], error)
	MintAPIKey(ctx context.Context, serviceAccountID, name string, expiresAt time.Time) (apikey.APIKey, string, error)
	ListAPIKeys(ctx context.Context, serviceAccountID string, req list.Request) (list.Page[apikey.APIKey], error)
	...
	TokenEnabled() bool
	IssueToken(ctx context.Context, email, password string) (pair authlogic.TokenPair, err error)
	...
	VerifyPasswordless(ctx context.Context, kind, identifier, code string) (authlogic.TokenPair, error)
	...
	RedeemPasswordless(ctx context.Context, token string) (authlogic.TokenPair, error)
}

type authService interface {
	AuthenticationService
	SetSessionCookies(w http.ResponseWriter, pair authlogic.TokenPair)
	ClearSessionCookies(w http.ResponseWriter)
	SessionCookieName() string
	RefreshCookieName() string
	RequirePrincipal(opts ...PrincipalOption) web.Middleware
...
func newTokenResponse(pair authlogic.TokenPair) tokenResponse {
...
// body goes to registerForm. Successful JSON responses retain their DTO/status/body/cookie contract.
...
// dispatch.go). Successful JSON responses are unchanged; a form body goes to loginForm.
func (h *handlers) loginJSON(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if errors.Is(err, authlogic.ErrRateLimited) {
...
// dispatch.go). Successful JSON responses are unchanged; a form body goes to verifyForm.
...
// (dispatched by forgotPassword, dispatch.go). Successful JSON responses are unchanged; a
...
// (dispatched by resetPassword, dispatch.go). Successful JSON responses are unchanged; a
