...
func TestContentTypeDispatchRoutesByContentType(t *testing.T) {
	...
	for _, e := range sharedPOSTs {
		if rec := doJSONReq(t, h, e.path, e.json); rec.Code == http.StatusUnsupportedMediaType || strings.TrimSpace(rec.Body.String()) == "404 page not found" {
			t.Errorf("JSON POST %s = %d, want the JSON arm reached", e.path, rec.Code)
		}
		if rec := doFormReq(t, h, e.path, e.form); rec.Code == http.StatusUnsupportedMediaType || strings.TrimSpace(rec.Body.String()) == "404 page not found" {
