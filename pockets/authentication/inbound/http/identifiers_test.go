...
func (f *memContactChanges) Get(_ context.Context, userID string, kind identifier.Kind) (contactchange.PendingChange, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *memContactChanges) Consume(_ context.Context, userID string, kind identifier.Kind, expectedID string) (contactchange.PendingChange, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	k := userID + "|" + string(kind)
	p, ok := f.m[k]
...
// real authlogic.Service over the mem stores with the contact-change rail, the
