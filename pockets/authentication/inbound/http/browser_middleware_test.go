...
func TestBrowserGatesRedirectExpiredJWT(t *testing.T) {
	bh := newBrowserHarness(t)
	expired, err := bh.signer.Sign(map[string]any{"user_id": "u1", "session_id": "s1"}, time.Now().Add(-time.Hour))
	if err != nil {
		t.Fatalf("Sign: %v", err)
	}
	for _, gate := range []func(http.Handler) http.Handler{
...
func TestBrowserGatesPreserveContextStash(t *testing.T) {
	...
	var gotP sdk.Principal
	var gotSession string
	var gotSessionOK bool
	capture := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotP, _ = sdk.PrincipalFromContext(r.Context())
	...
	if recCookie.Code != http.StatusNoContent || gotP.Type != authlogic.PrincipalUser {
	...
	if recBearer.Code != http.StatusNoContent || gotP.Type != authlogic.PrincipalUser {
	...
	if recKey.Code != http.StatusNoContent || gotP.Type != authlogic.PrincipalServiceAccount || gotP.ID != sa.ID {
