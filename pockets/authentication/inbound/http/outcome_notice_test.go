...
func TestOAuthLinkAndUnlinkNoticeRoundTrip(t *testing.T) {
	...
	cb := do(t, f.h, "GET", "/auth/oauth/google/callback?code=x&state="+url.QueryEscape(state), "", append(start.Result().Cookies(), sess)...)
...
func TestPasswordResetNoticeRoundTrip(t *testing.T) {
	...
	env, deliverable, err := f.svc.initializer.Initialize(context.Background(), sdk.AddressKindEmail, delivery.PurposePasswordReset, delivery.Envelope{ResolutionInput: "pw@example.com"})
	if err != nil {
