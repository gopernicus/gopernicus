...
func (s *memServiceAccounts) List(_ context.Context, _ listing.Request) (listing.Page[serviceaccount.ServiceAccount], error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	return listing.Page[serviceaccount.ServiceAccount]{Items: items}, nil
}
...
func (k *memAPIKeys) ListByServiceAccount(_ context.Context, saID string, _ listing.Request) (listing.Page[apikey.APIKey], error) {
	k.mu.Lock()
	defer k.mu.Unlock()
	...
	return listing.Page[apikey.APIKey]{Items: items}, nil
}
...
// newMachineHandler builds a real authlogic.Service with the machine repos wired,
...
// --- transport-edge page params (listing.ParseQuery / listing.ParseOrder) ---
