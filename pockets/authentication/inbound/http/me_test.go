...
// newMeHandler wires a real authlogic.Service over the mem stores — including the
...
func newMeHandler(t *testing.T, prefix string) meFixture {
	...
	h := web.NewWebHandler()
	var r pockets.RouteRegistrar = h
	if prefix != "" {
		r = pockets.PrefixRegistrar{Prefix: prefix, Next: h}
	}
	mount(r, mountDeps{Auth: svc, ListStrategy: list.StrategyCursor, MachineGate: allowMachineGate})
...
// TestMePrefixedMount proves the route rides pockets.PrefixRegistrar with no
