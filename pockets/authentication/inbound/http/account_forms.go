...
func accountFailure(err error) (int, string) {
	switch {
	case errors.Is(err, authlogic.ErrRateLimited),
		errors.Is(err, authlogic.ErrIdentifierChangeRateLimited),
		errors.Is(err, authlogic.ErrPasswordlessRateLimited):
...
func (h *handlers) setPasswordForm(w http.ResponseWriter, r *http.Request) {
	h.accountForm(w, r, func(form url.Values) {
		...
		if err != nil {
			if errors.Is(err, authlogic.ErrStepUpRequired) {
...
func (h *handlers) beginStepUpForm(w http.ResponseWriter, r *http.Request) {
	h.accountForm(w, r, func(form url.Values) {
		...
		if _, err := h.svc.BeginStepUp(r.Context(), authlogic.StepUpStart{
			SessionID: sessionID, UserID: userID, Purpose: purpose, Context: context, Kind: form.Get("kind"),
		}); err != nil {
			h.renderStepUp(w, r, userID, stepUpParams{kind: form.Get("kind"), purpose: purpose, context: context, operation: form.Get("operation"), returnTo: form.Get("return_to")}, err)
			return
		}
		h.prgTo(w, r, stepUpPath(stepUpParams{
			kind: form.Get("kind"), purpose: purpose, context: context, operation: form.Get("operation"), returnTo: form.Get("return_to"), sent: true,
		}))
	})
}
...
func (h *handlers) completeStepUpPasswordForm(w http.ResponseWriter, r *http.Request) {
	h.accountForm(w, r, func(form url.Values) {
		...
		purpose, context := form.Get("purpose"), form.Get("context")
		if _, err := h.svc.CompleteStepUpWithPassword(r.Context(), authlogic.StepUpCompletion{
...
func (h *handlers) completeStepUpCodeForm(w http.ResponseWriter, r *http.Request) {
	h.accountForm(w, r, func(form url.Values) {
		...
		purpose, context := form.Get("purpose"), form.Get("context")
		if _, err := h.svc.CompleteStepUpWithIdentifierCode(r.Context(), authlogic.StepUpCompletion{
			SessionID: sessionID, UserID: userID, Purpose: purpose, Context: context, Kind: form.Get("kind"),
...
func (h *handlers) startIdentifierForm(w http.ResponseWriter, r *http.Request, kind identifier.Kind, purpose, operation string) {
	h.accountForm(w, r, func(form url.Values) {
		...
		if _, err := h.svc.StartIdentifierChange(r.Context(), authlogic.IdentifierChangeStart{
...
func (h *handlers) confirmIdentifierForm(w http.ResponseWriter, r *http.Request, kind identifier.Kind) {
	h.accountForm(w, r, func(form url.Values) {
		...
		if err := h.svc.ConfirmIdentifierChange(r.Context(), authlogic.IdentifierChangeConfirm{
...
func (h *handlers) identifierEditForm(w http.ResponseWriter, r *http.Request) {
	h.accountForm(w, r, func(form url.Values) {
		...
		if form.Get("action") == "remove" {
			err := h.svc.RemoveIdentifier(r.Context(), authlogic.IdentifierRemoveInput{
		...
		err := h.svc.SetIdentifierUses(r.Context(), authlogic.IdentifierUsesInput{
