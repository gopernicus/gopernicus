...
func TestApplyPasswordAndOAuthKindsLeaveTheProjectionAlone(t *testing.T) {
	...
	if err := r.CredentialMutations.Apply(ctx, u.ID, 0, credential.RemovePassword{}); err != nil {
	...
	if err := r.CredentialMutations.Apply(ctx, u.ID, 1, credential.UnlinkOAuth{Provider: "google"}); err != nil {
	...
	if set.HasPassword || len(set.OAuth) != 0 || set.AuthRevision != 2 {
		t.Errorf("snapshot = %+v, want no password, no links, revision 2", set)
	}
	...
	if rev := revisionOf(t, r, u.ID); rev != 3 {
		t.Errorf("auth_revision = %d after a no-op mutation, want 3", rev)
	}
}
...
func TestApplyRollsBackWholeAndRetriesClean(t *testing.T) {
	...
	attempts, err := retryWithInjectedAbort(ctx, db,
		func() {
			...
			if !set.HasPassword || set.AuthRevision != 0 {
				t.Errorf("the rolled-back attempt left %+v; want the password intact at revision 0", set)
			}
		},
		func(ctx context.Context) error {
			return mutations.apply(ctx, u.ID, 0, credential.RemovePassword{}, testBase.Add(time.Hour))
		},
	)
	if err != nil {
	...
	if set.HasPassword || set.AuthRevision != 1 {
		t.Errorf("after the retry: %+v, want no password at exactly revision 1", set)
	}
}
