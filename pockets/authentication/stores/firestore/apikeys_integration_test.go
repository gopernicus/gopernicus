...
func TestAPIKeyCreateRollsBackOnLostHashClaim(t *testing.T) {
	...
	page, err := r.APIKeys.ListByServiceAccount(ctx, "sa-1", crud.ListRequest{Limit: 50})
	if err != nil {
...
func TestSearchOffsetSkipsMatchesNotScannedRows(t *testing.T) {
	...
	for offset := 0; offset < len(matches); offset++ {
		page, err := r.APIKeys.ListByServiceAccount(ctx, "sa-paging", crud.ListRequest{
			Limit:    1,
			Strategy: crud.StrategyOffset,
			Offset:   offset,
			Search:   "target",
		})
		if err != nil {
	...
	page, err := r.APIKeys.ListByServiceAccount(ctx, "sa-paging", crud.ListRequest{
		Limit: 1, Strategy: crud.StrategyOffset, Offset: len(matches), Search: "target",
	})
	if err != nil {
...
func TestSearchPreviousPageWindowIsFullUnderSearch(t *testing.T) {
	...
	pageOf := func(cursor string) crud.Page[apikey.APIKey] {
		t.Helper()
		page, err := r.APIKeys.ListByServiceAccount(ctx, "sa-paging", crud.ListRequest{
	...
	idsOf := func(page crud.Page[apikey.APIKey]) []string {
