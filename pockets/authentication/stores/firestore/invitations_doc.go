...
//
//   - invitation_pending reproduces the PARTIAL unique index over pending/accepting rows,
...
//     moves an unclaimed row off pending or acceptance completes. Reading the clock
...
func newInvitationDoc(inv invitation.Invitation) (invitationDoc, error) {
	...
	return invitationDoc{
		...
		ResolvedSubjectType: inv.ResolvedSubjectType,
...
// hash always, the active tuple while status is pending or accepting.
...
func readInvitationByTokenClaim(ctx context.Context, db *firestoredb.DB, r firestoredb.Reader, tokenHash string) (invitationDoc, error) {
	...
	if !protection.ConstantTimeDigestEqual(tokenHash, row.TokenHash) {
...
// pending reports whether the legacy-named claim remains active: pending or
// accepting. This is the partial
...
func (d invitationDoc) pending() bool {
	return d.Status == invitation.StatusPending || d.Status == invitation.StatusAccepting
}
...
func (d invitationDoc) toDomain() (invitation.Invitation, error) {
	...
	return invitation.Invitation{
		...
		ResolvedSubjectType: d.ResolvedSubjectType,
