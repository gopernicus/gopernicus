...
func (s *challengeStore) consumeCode(ctx context.Context, userID, purpose string, candidates []challenge.DigestCandidate,
	...
	*out = codeConsumption{}
	...
	// The lookup is a subject key, which can bind a session or operation.
	// The returned consumed row retains the actual user ID separately.
...
// candidate digest never matches: protection.ConstantTimeDigestEqual refuses it, so a
