...
func TestMisPointedDigestClaimConsumesNothing(t *testing.T) {
	...
	binding := seedResetOwner(t, r, userID, "hash:before")
	live, err := r.Challenges.Replace(ctx, challengeFixture(userID, challenge.PurposePasswordReset, "", digest, binding, 0, time.Hour, testBase))
	if err != nil {
...
func TestBagKeysRoundTripThroughTheEmulator(t *testing.T) {
	...
	page, err := r.SecurityEvents.List(ctx, securityevent.ListFilter{}, list.Request{Limit: 10})
	if err != nil {
