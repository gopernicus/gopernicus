...
// provisioningBinding is a well-formed v1 binding that carries the captured
// provisioning intent.
...
func TestAdoptionRevokesEveryDocumentItOwes(t *testing.T) {
	...
	if _, err := r.AuthenticationGrants.Create(ctx, grant); err != nil {
	...
	if after.AuthRevision != squatter.AuthRevision+1 {
		t.Errorf("auth_revision = %d, want %d", after.AuthRevision, squatter.AuthRevision+1)
	}
}
...
// TestRejectedTokenIsStillRedeemable is the consequence of the retention above,
// end to end: a subject-side refusal is not a spent link.
//
// The deactivated subject is the shape that can be undone — the operator
// reactivates the account and the SAME link, never re-mailed, logs them in.
func TestRejectedTokenIsStillRedeemable(t *testing.T) {
	r, _ := openRepos(t)
	ctx := context.Background()

	const addr = "reactivated@example.com"
	owner, _ := seedUser(t, r, identifier.KindEmail, addr,
		identifier.Uses{Login: true, Recovery: true}, true, testBase, testBase)
	if _, err := r.UserAdmin.SetStatus(ctx, owner.ID, user.StatusDeactivated, testBase.Add(time.Hour)); err != nil {
	...
	if _, err := r.UserAdmin.SetStatus(ctx, owner.ID, user.StatusActive, testBase.Add(2*time.Hour)); err != nil {
		t.Fatalf("SetStatus(active): %v", err)
	}

	res, err := r.Passwordless.Redeem(ctx, redeemInput(link.digest, "refresh-accepted", time.Now()))
	if err != nil {
		t.Fatalf("the retained link did not redeem after reactivation: %v", err)
	}
	if res.Outcome != passwordless.OutcomeLoginExistingVerified || res.User.ID != owner.ID {
		t.Errorf("outcome = %q user = %q, want a login for %q", res.Outcome, res.User.ID, owner.ID)
	}
}
...
func TestRedeemRollsBackWholeAndRetriesClean(t *testing.T) {
	...
	attempts, err := retryWithInjectedAbort(ctx, db,
		func() {
			...
			if row, err := readUser(ctx, db, db.ReaderFrom(ctx), squatter.ID); err != nil {
				t.Fatalf("readUser: %v", err)
			} else if row.AuthRevision != squatter.AuthRevision {
				t.Errorf("auth_revision = %d, want the untouched %d", row.AuthRevision, squatter.AuthRevision)
			}
	...
	if after.AuthRevision != squatter.AuthRevision+1 {
		t.Errorf("auth_revision = %d, want exactly one increment from %d", after.AuthRevision, squatter.AuthRevision)
	}
}
