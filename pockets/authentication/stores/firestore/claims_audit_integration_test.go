...
func claimPredicates() []claimPredicate {
	return []claimPredicate{
		...
		{
			// §5.6 the five-column tuple WHERE the STORED status is pending —
...
func sweepLifecycle(t *testing.T, r auth.Repositories, db *firestoredb.DB) {
	...
	if _, err := r.AuthenticationGrants.Consume(ctx, grant.SessionID, grant.Purpose, grant.ContextDigest, now); err != nil {
	...
	if _, err := r.ActiveSessions.CreateForActiveUser(ctx, revived); err != nil {
