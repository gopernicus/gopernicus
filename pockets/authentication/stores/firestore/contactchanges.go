...
func (s *contactChangeStore) Consume(ctx context.Context, userID string, kind identifier.Kind, expectedID string) (contactchange.PendingChange, error) {
	...
	err := retryTransact(ctx, s.db, func(ctx context.Context) error {
		return s.consume(ctx, userID, kind, expectedID, &out)
	})
	if err != nil {
		return contactchange.PendingChange{}, err
	}
...
func (s *contactChangeStore) consume(ctx context.Context, userID string, kind identifier.Kind, expectedID string, out *changeConsumption) error {
	*out = changeConsumption{}
	...
	if !found || expectedID == "" || row.ID != expectedID {
		return nil
	}
	p := row.toDomain()
...
// Get returns the current generation without consuming it.
