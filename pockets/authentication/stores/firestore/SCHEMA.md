...
explains the document model; [the release plan](../../../../plans/firestore-release.md)
tracks actual verification and publication status.
...
| SDK | `v0.9.0` |
| Connector | `integrations/datastores/firestore v0.1.0` candidate; publication requires live evidence |
...
`auth.Repositories` has eighteen fields, filled by eighteen distinct interfaces
with 64 methods. `portcalls_test.go` drives every method through both ambient
transaction contexts to require the adapter's explicit refusal.
...
|---|---|---|---|
| `Provision` | uniqueness enforced by atomic creates | user, identifier and optional password/OAuth credentials | takes identifier claims; collision rolls back all writes |
...
| Method | Reads | Writes | Claims |
|---|---|---|---|
| `Set` | active user, sessions, grants, reset proofs | password upsert, revision increment and revocation | releases session/reset claims |
| `Change` | same plus current password and expected revision/hash | conditional password change and same revocation | same |
...
| Method | Reads | Writes | Claims |
|---|---|---|---|
| `CreateForActiveUser` | users doc (active status and expected revision), refresh claim doc | session doc | takes the current-hash claim |
...
| `Link` | current user/revision, provider account, identifier and revocation set | conditional link/adoption, revision increment and credential revocation | identifier/session/reset claims move atomically |
...
### `invitations.InvitationRepository` — 8
...
| `UpdateStatus` | current row; requires pending/expired and matching expected token | status, token hash, expiry, resolved subject ID, updated_at | moves token claim on resend; releases active tuple on decline/cancel |
| `ClaimAcceptance` | current row; matching pending token, subject binding and initial expiry | accepting status, resolved subject type/ID, updated_at | retains active tuple and token claim |
| `CompleteAcceptance` | matching durable accepting/accepted token and subject | accepted status, accepted_at, updated_at; repeated completion preserves timestamps | releases active tuple, retains token claim |

`Create` accepts pending rows only. Accepting and accepted token reads bypass
expiry so a matching bound subject can resume. An accepting row cannot be resent,
cancelled or declined. All lifecycle conditions and claim movements are atomic.

### `challenge.Repository` — 4

`ConsumeCode` returns both the stored `SubjectKey` and actual `UserID`.
The subject key may bind a session/operation; it need not equal the user ID.
...
`Get` returns the current generation without consuming it. `Create` replaces
the `(user_id, kind)` document. `Consume` reads that row and deletes only when
its surrogate ID equals `expectedID`; a stale ID returns `sdk.ErrNotFound` and
leaves the replacement intact. Matching expired rows are deleted before
`sdk.ErrExpired` is returned. No claims.

### `authgrant.Repository` — 3

| Method | Reads | Writes | Claims |
|---|---|---|---|
| `Create` | current active user, expected revision and matching live session | authentication_grants doc | — |
| `Consume` | active user and matching live session; query `consume_key ==`, `consumed_at == null`, order `(created_at, id)`, 32-row pages | spend oldest expired or policy-suitable grant; unsuitable age/assurance remains available; expired consumption commits before `sdk.ErrExpired` | — |
...
| `Apply` | **BUILT (N2b)** — users doc (revision CAS) + the typed mutation's targets: `RemovePassword` → NOTHING (see below); `UnlinkOAuth` → the user's links for that provider (a query: the doc id needs `provider_user_id`); `RetireIdentifier` → the retired row, the named replacement, the active primary email; `ChangeIdentifierUses` → the row, the displaced primary of its kind, the active primary email | exactly the targeted source, its claims, the recomputed projection, `auth_revision + 1`; revoke sessions, grants and reset proofs | per mutation — see §6.3 |

Every mutation also reads its session/grant/reset-proof revocation set before
any write. `RemovePassword` does not need to read the password row itself: its
document ID is the user ID, and deleting an absent password is idempotent.
All identifier targets and replacements must belong to the acting user. Foreign
...
through its digest claim, validate the versioned binding against the active user,
current revision and same active verified recovery identifier, increment the
revision, Set the password doc, delete
...
identifier row it names; then, for an existing owner, `users/h(user_id)`.
A known-owner binding must match that exact user, identifier and current credential
revision. It cannot provision if its former claim disappeared. Only ownerless
version-2 provisioning proof uses current-owner resolution without an issued
revision; unknown or legacy versions are rejected.
...
`protection.ConstantTimeDigestEqual` — and answers `sdk.ErrNotFound` on a mismatch.
...
`invitations.resolved_subject_type` stores the durable acceptance subject type
alongside `resolved_subject_id`. Both are checked by claim completion and retries.
...
| Stored predicate | `status IN ('pending', 'accepting')` — the STORED value, never the read clock |
...
pending/accepting. Claiming acceptance retains the tuple; completion releases it.
...
constructor takes the caller's `context.Context` and caps probing with the
connector's `ProbeTimeout` (30 s). An already canceled context is rejected even
with `WithoutIndexProbe`; the database remains owned by the caller. Against the emulator the probe is
...
- **Credential mutations validate identifier ownership.** Retirement targets,
  replacement primaries and use-change targets must belong to the acting user.
  The Firestore adapter refuses foreign targets atomically with `sdk.ErrInvalidInput`.
  The SQL adapters' unguarded target updates are a separately tracked correctness
  follow-up; their behavior is not an intentional contract to preserve.
