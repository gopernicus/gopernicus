...
// and, for a known-owner proof, the recorded owner and credential revision.
...
// and UserID bind known-owner proof to that exact credential revision. Only
// ownerless provisioning proof may resolve a newly acquired current owner.
func (s *passwordlessStore) stage(ctx context.Context, r firestoredb.Reader, in passwordless.RedeemInput, now time.Time) (redemptionWrites, error) {
	...
	if !binding.MatchesOwner(current.UserID, current.ID, owner.AuthRevision) {
		return redemptionWrites{}, passwordless.ErrRedemption
	}
