...
func (s *invitationStore) ListBySubject(ctx context.Context, kind, identifierValue string, req crud.ListRequest) (crud.Page[invitation.Invitation], error) {
	if err := refuseAmbient(ctx); err != nil {
		return crud.Page[invitation.Invitation]{}, err
	}
...
// UpdateStatus applies a lifecycle transition in ONE transaction — the six
// mutable columns, the token claim (which a resend moves), and the pending claim
// (which a transition off pending releases). Unknown id → sdk.ErrNotFound.
//
// Releasing the tuple here, and only here, is what makes the partial index
// partial: after this commits with a non-pending status, a NEW pending
// invitation for the same tuple succeeds. Nothing about wall-clock expiry
...
func (s *invitationStore) UpdateStatus(ctx context.Context, id string, upd invitation.StatusUpdate) (invitation.Invitation, error) {
	...
	var next invitationDoc
	err := retryTransact(ctx, s.db, func(ctx context.Context) error {
		old, err := readInvitation(ctx, s.db, s.db.ReaderFrom(ctx), id)
		if err != nil {
			return err
		}
		next = old.applied(upd)

		w := s.db.WriterFrom(ctx)
		plan := newClaimPlan()
		if err := updateInvitation(ctx, s.db, w, plan, old, next); err != nil {
			return err
		}
		return plan.commit(ctx, w)
	})
