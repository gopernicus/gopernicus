...
func TestGrantConsumeResetsItsOutcomePerAttempt(t *testing.T) {
	...
	if _, _, err := r.Users.CreateWithPrimaryIdentifier(ctx, user.User{ID: "u1", Status: user.StatusActive}, identifier.Identifier{ID: "i1", Kind: identifier.KindEmail, NormalizedValue: "grant@example.com", LoginEnabled: true}); err != nil {
		t.Fatal(err)
	}
	if _, err := r.Sessions.Create(ctx, session.Session{ID: "s1", UserID: "u1", RefreshTokenHash: "grant-test", ExpiresAt: time.Now().Add(time.Hour)}); err != nil {
		t.Fatal(err)
	}
	now := time.Now()
	if _, err := r.AuthenticationGrants.Create(ctx, newTestGrant("s1", "u1", authgrant.PurposeSetPassword, "ctx-A", -time.Minute, now), 0, now); err != nil {
		t.Fatalf("Create: %v", err)
	}

	var out grantConsumption
	requirement := testGrantRequirement(now)
	attempts, err := retryWithInjectedAbort(ctx, db,
		func() {
		...
		func(ctx context.Context) error { return consumeGrant(ctx, db, requirement, now, &out) },
	)
	if err != nil {
		t.Fatalf("the retried consume must succeed: %v", err)
	}
...
func TestExpiredGrantUnderContentionCommitsExactlyOneOutcome(t *testing.T) {
	...
	for i := range n {
		...
		go func(i int) {
			defer wg.Done()
			<-start
			_, errs[i] = r.AuthenticationGrants.Consume(ctx, testGrantRequirement(now), now)
		}(i)
	}
	close(start)
	wg.Wait()
...
func testGrantRequirement(now time.Time) authgrant.Requirement {
	return authgrant.Requirement{SessionID: "s1", UserID: "u1", Purpose: authgrant.PurposeSetPassword, ContextDigest: "ctx-A", AuthenticatedAfter: now.Add(-time.Hour), MinAssurance: session.AssuranceAAL1}
}
