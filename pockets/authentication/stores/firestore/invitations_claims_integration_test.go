...
func invitationFixture(t *testing.T, relation, identifierValue, kind, tokenHash string, ttl time.Duration, now time.Time) invitation.Invitation {
	t.Helper()
	inv, err := invitation.NewInvitation(dbGenerated, "project", "p-claims", relation, identifierValue, kind, "inviter-1", tokenHash, false, ttl, now)
	if err != nil {
		t.Fatalf("invitation.NewInvitation(%q, %q): %v", relation, tokenHash, err)
	}
	return inv
}
...
func TestPendingClaimIncludesRelation(t *testing.T) {
	...
	member := invitationFixture(t, "member", "rel@example.com", sdk.AddressKindEmail, "hash-rel-member", time.Hour, testBase)
	admin := invitationFixture(t, "admin", "rel@example.com", sdk.AddressKindEmail, "hash-rel-admin", time.Hour, testBase)
...
func TestStoredStatusTransitionReleasesThePendingClaim(t *testing.T) {
	...
	first := invitationFixture(t, "member", "release@example.com", sdk.AddressKindEmail, "hash-release-1", time.Hour, testBase)
	...
	if _, err := r.Invitations.UpdateStatus(ctx, created.ID, invitation.StatusUpdate{
		ExpectedTokenHash: created.TokenHash,
	...
	second := invitationFixture(t, "member", "release@example.com", sdk.AddressKindEmail, "hash-release-2", time.Hour, testBase.Add(2*time.Minute))
...
func TestReadTimeExpiryRetainsThePendingClaim(t *testing.T) {
	...
	stale := invitationFixture(t, "member", "stale@example.com", sdk.AddressKindEmail, "hash-stale", time.Minute, time.Now().Add(-time.Hour))
	...
	blocked := invitationFixture(t, "member", "stale@example.com", sdk.AddressKindEmail, "hash-stale-2", time.Hour, time.Now())
...
func TestResendMovesTheTokenClaim(t *testing.T) {
	...
	inv := invitationFixture(t, "member", "resend@example.com", sdk.AddressKindEmail, "hash-resend-old", time.Hour, time.Now())
...
// Acceptance keeps the active tuple throughout the host grant boundary, then
// completion releases only that tuple. The total token claim survives both steps.
func TestInvitationAcceptanceKeepsClaimsUntilCompletion(t *testing.T) {
	r, db := openRepos(t)
	now := time.Now().Truncate(time.Microsecond).Add(123 * time.Nanosecond)
	created, err := r.Invitations.Create(t.Context(), invitationFixture(t, "member", "accept-claims@example.com", sdk.AddressKindEmail, "accept-claims-token", time.Hour, now))
	if err != nil {
		t.Fatal(err)
	}
	claim := invitation.Acceptance{TokenHash: created.TokenHash, SubjectType: "user", SubjectID: "verified-subject", Now: now.Add(time.Minute)}
	accepting, err := r.Invitations.ClaimAcceptance(t.Context(), created.ID, claim)
	if err != nil {
		t.Fatal(err)
	}
	if accepting.Status != invitation.StatusAccepting {
		t.Fatalf("status=%s", accepting.Status)
	}
	if owner, ok := pendingClaimHolder(t, db, accepting); !ok || owner.InvitationID != created.ID {
		t.Fatal("acceptance released its active tuple")
	}
	claim.Now = now.Add(2 * time.Hour)
	resumed, err := r.Invitations.ClaimAcceptance(t.Context(), created.ID, claim)
	if err != nil || resumed.ResolvedSubjectType != claim.SubjectType || resumed.ResolvedSubjectID != claim.SubjectID {
		t.Fatalf("resume claim: %v", err)
	}
	accepted, err := r.Invitations.CompleteAcceptance(t.Context(), created.ID, claim)
	if err != nil {
		t.Fatal(err)
	}
	if _, ok := pendingClaimHolder(t, db, accepted); ok {
		t.Fatal("completion stranded its active tuple")
	}
	if owner, ok := tokenClaimHolder(t, db, created.TokenHash); !ok || owner.InvitationID != created.ID {
		t.Fatal("completion released its token claim")
	}
	claim.Now = now.Add(3 * time.Hour)
	repeated, err := r.Invitations.CompleteAcceptance(t.Context(), created.ID, claim)
	if err != nil || !repeated.AcceptedAt.Equal(accepted.AcceptedAt) {
		t.Fatalf("repeated completion changed timestamp: %v", err)
	}
}
