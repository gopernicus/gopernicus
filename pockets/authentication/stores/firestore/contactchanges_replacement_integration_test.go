...
func TestExpiredConsumeCommitsItsDeletion(t *testing.T) {
	...
	if _, err := r.ContactChanges.Create(ctx, contactchange.New(userID, identifier.KindEmail, "stale@example.com",
		identifier.Uses{Login: true}, false, "", -time.Minute, time.Now())); err != nil {
		t.Fatalf("Create: %v", err)
	}

	if _, err := r.ContactChanges.Consume(ctx, userID, identifier.KindEmail); !errors.Is(err, sdk.ErrExpired) {
	...
	if _, err := r.ContactChanges.Consume(ctx, userID, identifier.KindEmail); !errors.Is(err, sdk.ErrNotFound) {
