...
func TestRedeemStrandsNoClaims(t *testing.T) {
	...
	if err := r.Passwords.Set(ctx, userID, "hash:old"); err != nil {
	...
	if _, err := r.Challenges.Replace(ctx, challengeFixture(userID, challenge.PurposePasswordReset, "", digest, nil, 0, time.Hour, time.Now())); err != nil {
...
func TestExpiredResetTokenLeavesTheChallengeIntact(t *testing.T) {
	...
	if err := r.Passwords.Set(ctx, userID, "hash:keep"); err != nil {
		t.Fatalf("seed password: %v", err)
	}
	if _, err := r.Challenges.Replace(ctx, challengeFixture(userID, challenge.PurposePasswordReset, "", digest, nil, 0, -time.Minute, time.Now())); err != nil {
