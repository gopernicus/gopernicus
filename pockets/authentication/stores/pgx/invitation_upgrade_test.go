...
func TestInvitationAcceptanceUpgrade(t *testing.T) {
	...
	if _, err := db.Exec(ctx, `INSERT INTO `+schema.Table(invitationsTable)+`
 ...
 VALUES ('existing-invitation','project','legacy','member','legacy@example.com','email','','inviter','preserved-token',true,'pending',$1,$2,$2,'{"source":"legacy"}')`, now.Add(time.Hour), now); err != nil {
		t.Fatal(err)
	}
	if _, err := Repositories(context.Background(), db, storeOpts(schema)...); !errors.Is(err, sdk.ErrNotFound) || !strings.Contains(err.Error(), "0018_invitation_acceptance.sql") {
