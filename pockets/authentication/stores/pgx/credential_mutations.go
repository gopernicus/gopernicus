...
func (s *CredentialMutationStore) Snapshot(ctx context.Context, userID string) (credential.MethodSet, error) {
	...
	err := s.db.InTx(ctx, func(tx *pgxdb.Tx) error {
		var revision int64
		if err := tx.QueryRow(ctx, `SELECT auth_revision FROM `+s.table(usersTable)+` WHERE id = @id`, pgx.NamedArgs{"id": userID}).
		...
		if err := tx.QueryRow(ctx, `SELECT EXISTS (SELECT 1 FROM `+s.table(passwordsTable)+` WHERE user_id = @id)`, pgx.NamedArgs{"id": userID}).
			Scan(&hasPassword); err != nil {
			return pgxdb.MapError(err)
		}
		set.HasPassword = hasPassword

		oauth, err := tx.Query(ctx, `SELECT provider FROM `+s.table(oauthAccountsTable)+` WHERE user_id = @id ORDER BY provider`, pgx.NamedArgs{"id": userID})
		if err != nil {
			return err
		}
		...
		idents, err := tx.Query(ctx,
			`SELECT id, kind, login_enabled, recovery_enabled, notification_enabled, is_primary, verified_at
				FROM `+s.table(identifiersTable)+` WHERE user_id = @id AND replaced_at IS NULL ORDER BY created_at, id`,
...
func (s *CredentialMutationStore) Apply(ctx context.Context, userID string, expectedAuthRevision int64, mutation credential.Mutation) error {
	return s.db.InTx(ctx, func(tx *pgxdb.Tx) error {
		var current int64
		if err := tx.QueryRow(ctx, `SELECT auth_revision FROM `+s.table(usersTable)+` WHERE id = @id FOR UPDATE`, pgx.NamedArgs{"id": userID}).
		...
		case credential.RemovePassword:
			if _, err := tx.Exec(ctx, `DELETE FROM `+s.table(passwordsTable)+` WHERE user_id = @id`, pgx.NamedArgs{"id": userID}); err != nil {
				return err
			}
		case credential.UnlinkOAuth:
			if _, err := tx.Exec(ctx, `DELETE FROM `+s.table(oauthAccountsTable)+` WHERE user_id = @id AND provider = @provider`,
			...
			if m.MakePrimary {
				if _, err := tx.Exec(ctx,
					`UPDATE `+s.table(identifiersTable)+` SET is_primary = FALSE, updated_at = @now
						...
						AND kind = (SELECT kind FROM `+s.table(identifiersTable)+` WHERE id = @identifier_id)`,
