...
func findSummary(page list.Page[user.Summary], id string) (user.Summary, bool) {
...
func testDirectoryProjection(t *testing.T, repos auth.Repositories) {
	...
	page, err := repos.UserAdmin.List(ctx, list.Request{})
	if err != nil {
		t.Fatalf("List: %v", err)
	}
	if len(page.Items) != 3 {
...
func testDirectoryOrdering(t *testing.T, repos auth.Repositories) {
	...
	page, err := repos.UserAdmin.List(ctx, list.Request{Limit: 2})
	if err != nil {
		t.Fatalf("List: %v", err)
	}
	if len(page.Items) != 2 {
	...
	next, err := repos.UserAdmin.List(ctx, list.Request{Limit: 2, Cursor: page.NextCursor})
	if err != nil {
	...
	wantCursor, err := list.EncodeCursor("created_at", second.CreatedAt, second.ID)
	if err != nil {
...
func testDirectoryOffsetCount(t *testing.T, repos auth.Repositories) {
	...
	page, err := repos.UserAdmin.List(ctx, list.Request{Limit: 2, Offset: 1, Strategy: list.StrategyOffset, WithCount: true})
	if err != nil {
	...
	all, err := repos.UserAdmin.List(ctx, list.Request{Limit: 10})
	if err != nil {
...
func testDirectoryNoDuplicates(t *testing.T, repos auth.Repositories) {
	...
	page, err := repos.UserAdmin.List(ctx, list.Request{Limit: 10})
	if err != nil {
		t.Fatalf("List: %v", err)
	}
	count := 0
...
func seedLifecycleUser(t *testing.T, repos auth.Repositories) (user.User, []session.Session) {
	...
	if repos.AuthenticationGrants != nil {
		...
		if _, err := (grantFixture{repos}).Create(ctx, g); err != nil {
...
func testLifecycleDeactivate(t *testing.T, repos auth.Repositories) {
	...
	if repos.AuthenticationGrants != nil {
		_, err := (grantFixture{repos}).Consume(ctx, sessions[0].ID, "set_password", "ctx", time.Now())
...
func testLifecycleReactivate(t *testing.T, repos auth.Repositories) {
	...
	if repos.ActiveSessions != nil {
		...
		if _, err := repos.ActiveSessions.CreateForActiveUser(ctx, s, currentAuthRevision(ctx, repos.Users, s.UserID)); err != nil {
...
func testActiveMintRoundTrip(t *testing.T, repos auth.Repositories) {
	...
	created, err := repos.ActiveSessions.CreateForActiveUser(ctx, s, currentAuthRevision(ctx, repos.Users, s.UserID))
	if err != nil {
...
func testActiveMintRefused(t *testing.T, repos auth.Repositories) {
	...
	if _, err := repos.ActiveSessions.CreateForActiveUser(ctx, s, currentAuthRevision(ctx, repos.Users, s.UserID)); !errors.Is(err, session.ErrUserNotActive) {
...
func testActiveMintUnknown(t *testing.T, repos auth.Repositories) {
	...
	if _, err := repos.ActiveSessions.CreateForActiveUser(context.Background(), s, 0); !errors.Is(err, sdk.ErrNotFound) {
...
func testActiveMintDuplicate(t *testing.T, repos auth.Repositories) {
	...
	if _, err := repos.ActiveSessions.CreateForActiveUser(ctx, first, currentAuthRevision(ctx, repos.Users, first.UserID)); err != nil {
		t.Fatalf("first fenced mint: %v", err)
	}
	second := newSession(u.ID, "shared-refresh", time.Hour, time.Now())
	if _, err := repos.ActiveSessions.CreateForActiveUser(ctx, second, currentAuthRevision(ctx, repos.Users, second.UserID)); !errors.Is(err, sdk.ErrAlreadyExists) {
...
func testActiveMintRace(t *testing.T, repos auth.Repositories) {
	...
	for round := range rounds {
		...
		go func() {
			...
			minted, mintErr = repos.ActiveSessions.CreateForActiveUser(ctx, s, currentAuthRevision(ctx, repos.Users, s.UserID))
		}()
		go func() {
			defer wg.Done()
			<-start
...
func currentAuthRevision(ctx context.Context, users user.UserRepository, userID string) int64 {
