...
func (r *reference) revokeCredentialStateLocked(userID string) {
...
func (r *reference) advanceCredentialRevisionLocked(userID string, now time.Time) int64 {
	u := r.users[userID]
	u.AuthRevision++
	u.UpdatedAt = now.UTC()
	r.users[userID] = u
...
func (r *reference) changePasswordLocked(userID string, change user.PasswordChange, conditional bool) (int64, error) {
	u, ok := r.users[userID]
	if !ok {
...
func (r refPasswords) Change(ctx context.Context, userID string, change user.PasswordChange) (int64, error) {
...
func (r refOAuthAccounts) Link(ctx context.Context, a oauthaccount.OAuthAccount, expectedAuthRevision int64, adoptIdentifierID string, now time.Time) (oauthaccount.OAuthAccount, int64, error) {
	if err := ctx.Err(); err != nil {
	...
	if adoptIdentifierID != "" {
		ident, ok := r.userIdentifiers[adoptIdentifierID]
