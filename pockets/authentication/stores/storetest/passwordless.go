...
func runPasswordless(t *testing.T, newRepos func(t *testing.T) auth.Repositories) {
	...
	t.Run("PasswordlessRedeem", func(t *testing.T) {
		...
		for _, verified := range []bool{false, true} {
			for _, mutation := range []string{"change", "reset", "missing-revision"} {
				name := map[bool]string{false: "adoption", true: "login"}[verified]
				t.Run("KnownOwnerProof/"+name+"/"+mutation, func(t *testing.T) { testPasswordlessKnownOwnerRevision(t, newRepos(t), verified, mutation) })
			}
		}
		t.Run("LegacyOwnerlessV1Rejected", func(t *testing.T) { testPasswordlessLegacyOwnerlessRejected(t, newRepos(t)) })
...
func seedLinkChallenge(t *testing.T, repos auth.Repositories, digest string, binding passwordless.Binding, expiresAt time.Time) {
	t.Helper()
	if binding.UserID != "" && binding.AuthRevision == nil {
		u, err := repos.Users.Get(context.Background(), binding.UserID)
		if err != nil {
			t.Fatal(err)
		}
		binding.AuthRevision = &u.AuthRevision
	}
	if binding.UserID != "" && binding.IdentifierID == "" {
		idents, err := repos.Identifiers.ListByUser(context.Background(), binding.UserID)
		if err != nil {
			t.Fatal(err)
		}
		for _, ident := range idents {
			if string(ident.Kind) == binding.Kind && ident.NormalizedValue == binding.NormalizedValue {
...
func testRedeemAdopt(t *testing.T, repos auth.Repositories) {
	...
	// Their grant is gone.
	if repos.AuthenticationGrants != nil {
		if _, err := (grantFixture{repos}).Consume(ctx, squatterSession.ID, "set_password", "ctx", time.Now()); !errors.Is(err, sdk.ErrNotFound) {
