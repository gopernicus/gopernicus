...
func Run(t *testing.T, newRepos func(t *testing.T) auth.Repositories) {
	...
	t.Run("ServiceAccounts", func(t *testing.T) {
		...
		runPagedFamily(t, newRepos,
			func(repos auth.Repositories, ctx context.Context, req listing.Request) (listing.Page[serviceaccount.ServiceAccount], error) {
	...
	t.Run("APIKeys", func(t *testing.T) {
		...
		runPagedFamily(t, newRepos,
			func(repos auth.Repositories, ctx context.Context, req listing.Request) (listing.Page[apikey.APIKey], error) {
	...
	t.Run("SecurityEvents", func(t *testing.T) {
		...
		runPagedFamily(t, newRepos,
			func(repos auth.Repositories, ctx context.Context, req listing.Request) (listing.Page[securityevent.SecurityEvent], error) {
	...
	t.Run("Invitations", func(t *testing.T) {
		...
		t.Run("AcceptanceClaim", func(t *testing.T) { testInvitationAcceptanceClaim(t, newRepos(t)) })
		t.Run("StaleTokenTransition", func(t *testing.T) { testInvitationStaleTokenTransition(t, newRepos(t)) })
		t.Run("ConcurrentClaimCancel", func(t *testing.T) { testInvitationConcurrentClaimCancel(t, newRepos(t)) })
		...
		t.Run("ByResource", func(t *testing.T) {
			runPagedFamily(t, newRepos,
				func(repos auth.Repositories, ctx context.Context, req listing.Request) (listing.Page[invitations.Invitation], error) {
					return repos.Invitations.ListByResource(ctx, "project", "res-seed", req)
				},
				func(t *testing.T, repos auth.Repositories) ([]invitations.Invitation, int) {
					return seedInvitationsByResource(t, repos.Invitations)
				},
				func(inv invitations.Invitation) string { return inv.ID },
				func(inv invitations.Invitation) time.Time { return inv.CreatedAt },
			)
		})
		t.Run("BySubject", func(t *testing.T) {
			runPagedFamily(t, newRepos,
				func(repos auth.Repositories, ctx context.Context, req listing.Request) (listing.Page[invitations.Invitation], error) {
					return repos.Invitations.ListBySubject(ctx, sdk.AddressKindEmail, "subject@seed.example", req)
				},
				func(t *testing.T, repos auth.Repositories) ([]invitations.Invitation, int) {
	...
	t.Run("Challenges", func(t *testing.T) {
		...
		t.Run("ConcurrentReplace", func(t *testing.T) { testChallengeConcurrentReplace(t, newRepos(t)) })
		t.Run("SubjectAndFailedReplacement", func(t *testing.T) { testChallengeSubjectAndFailedReplacement(t, newRepos(t)) })
	...
	t.Run("ContactChanges", func(t *testing.T) {
		...
		t.Run("GenerationAndConcurrentReplace", func(t *testing.T) { testContactChangeGeneration(t, newRepos(t)) })
	})
	...
	t.Run("AuthenticationGrants", func(t *testing.T) {
		...
		t.Run("AdmissionAndPolicy", func(t *testing.T) { testGrantAdmissionAndPolicy(t, newRepos(t)) })
	})
	...
	t.Run("PasswordResets", func(t *testing.T) {
		...
		for _, mode := range []string{"legacy", "unknown_version", "replaced_recovery", "later_revision"} {
			t.Run("Rejects/"+mode, func(t *testing.T) { testPasswordResetStaleBinding(t, newRepos(t), mode) })
		}
		t.Run("RecoveryChangeRace", func(t *testing.T) { testPasswordResetRecoveryRace(t, newRepos(t)) })
	})
}
...
func testPasswords(t *testing.T, repos auth.Repositories) {
	...
	ensurePasswordOwner(t, repos, "u1")
...
func testOAuthAccountsCRUD(t *testing.T, repos auth.Repositories) {
	...
	ensurePasswordOwner(t, repos, "user1")
	ensurePasswordOwner(t, repos, "user2")
...
// testServiceAccountsListPaged asserts the list-typed List pages through every
...
func pageAllServiceAccounts(t *testing.T, repo serviceaccount.ServiceAccountRepository, limit int) []string {
	...
	for i := 0; i < 100; i++ { // bound against a runaway cursor
		page, err := repo.List(ctx, listing.Request{Limit: limit, Cursor: cursor})
		if err != nil {
			t.Fatalf("List: %v", err)
		}
...
func pageAllAPIKeys(t *testing.T, repo apikey.APIKeyRepository, saID string, limit int) []string {
	...
	cursor := ""
	for i := 0; i < 100; i++ { // bound against a runaway cursor
		page, err := repo.ListByServiceAccount(ctx, saID, listing.Request{Limit: limit, Cursor: cursor})
		if err != nil {
...
func testSecurityEventsDetails(t *testing.T, repos auth.Repositories) {
	...
	read := func(userID string) securityevent.SecurityEvent {
		page, err := repo.List(ctx, securityevent.ListFilter{UserID: userID}, listing.Request{Limit: 10})
		if err != nil {
...
func pageAllSecurityEvents(t *testing.T, repo securityevent.SecurityEventRepository, filter securityevent.ListFilter, limit int) []string {
	...
	cursor := ""
	for i := 0; i < 100; i++ { // bound against a runaway cursor
		page, err := repo.List(ctx, filter, listing.Request{Limit: limit, Cursor: cursor})
		if err != nil {
			t.Fatalf("List: %v", err)
		}
...
func testInvitationsCRUD(t *testing.T, repos auth.Repositories) {
	...
	inv, err := invitations.NewInvitation(ids, "project", "p1", "member", "invitee@example.com", "", "inviter-1", "hash-crud", false, time.Hour, time.Now())
	if err != nil {
		t.Fatalf("New: %v", err)
	}
	...
	if created.ID != inv.ID || created.Status != invitations.StatusPending {
		t.Errorf("Create round-trip lost data: %+v", created)
	}
	...
	if got.IdentifierKind != sdk.AddressKindEmail {
		t.Errorf("IdentifierKind = %q, want %q (default)", got.IdentifierKind, sdk.AddressKindEmail)
	}
...
func testInvitationsUniqueness(t *testing.T, repos auth.Repositories) {
	...
	if _, err := repo.UpdateStatus(ctx, a.ID, invitations.StatusUpdate{
		ExpectedTokenHash: a.TokenHash,
		Status:            invitations.StatusDeclined,
...
func testInvitationsKindRoundTrip(t *testing.T, repos auth.Repositories) {
	...
	inv, err := invitations.NewInvitation(ids, "project", "p1", "member", "+1 (555) 010-2345", sdk.AddressKindPhone, "inviter-1", "hash-phone", false, time.Hour, time.Now())
	if err != nil {
		t.Fatalf("New: %v", err)
	}
	created, err := repo.Create(ctx, inv)
	...
	if created.IdentifierKind != sdk.AddressKindPhone {
		t.Errorf("Create IdentifierKind = %q, want %q", created.IdentifierKind, sdk.AddressKindPhone)
	}
	...
	if got.IdentifierKind != sdk.AddressKindPhone || got.Identifier != "+1 (555) 010-2345" {
	...
	if err != nil || byHash.IdentifierKind != sdk.AddressKindPhone {
...
func testInvitationsMetadataRoundTrip(t *testing.T, repos auth.Repositories) {
	...
	inv, err := invitations.NewWithMetadata(ids, "project", "p1", "member", "meta@example.com", "", "inviter-1", "hash-meta", false, time.Hour, time.Now(), md)
	if err != nil {
...
func testInvitationsCrossKindCoexistence(t *testing.T, repos auth.Repositories) {
	...
	emailInv, err := invitations.NewInvitation(ids, "project", "p1", "member", "coexist@example.com", sdk.AddressKindEmail, "inviter-1", "hash-ck-email", false, time.Hour, suiteBase)
	if err != nil {
	...
	phoneInv, err := invitations.NewInvitation(ids, "project", "p1", "member", "coexist@example.com", sdk.AddressKindPhone, "inviter-1", "hash-ck-phone", false, time.Hour, suiteBase)
	if err != nil {
	...
	dup, err := invitations.NewInvitation(ids, "project", "p1", "member", "coexist@example.com", sdk.AddressKindEmail, "inviter-1", "hash-ck-dup", false, time.Hour, suiteBase)
	if err != nil {
...
func testInvitationsTokenExpired(t *testing.T, repos auth.Repositories) {
	...
	// Created one hour ago with a one-minute lifetime → expired now.
	inv, err := invitations.NewInvitation(ids, "project", "p1", "member", "expired@example.com", "", "inviter-1", "hash-expired", false, time.Minute, time.Now().Add(-time.Hour))
	if err != nil {
		t.Fatalf("New: %v", err)
	}
...
func testInvitationsStatusTransitions(t *testing.T, repos auth.Repositories) {
	...
	claim := invitations.Acceptance{TokenHash: inv.TokenHash, SubjectType: "user", SubjectID: "user-42", Now: acceptedAt}
	if _, err := repo.ClaimAcceptance(ctx, inv.ID, claim); err != nil {
		t.Fatalf("ClaimAcceptance: %v", err)
	}
	updated, err := repo.CompleteAcceptance(ctx, inv.ID, claim)
	if err != nil {
		t.Fatalf("UpdateStatus(accepted): %v", err)
	}
	if updated.Status != invitations.StatusAccepted || updated.ResolvedSubjectID != "user-42" || !updated.AcceptedAt.Equal(acceptedAt) {
		t.Errorf("accept transition lost data: %+v", updated)
	}

	reget, err := repo.Get(ctx, inv.ID)
	if err != nil || reget.Status != invitations.StatusAccepted || reget.ResolvedSubjectID != "user-42" {
...
func testInvitationsUpdateAbsent(t *testing.T, repos auth.Repositories) {
	ctx := context.Background()
	if _, err := repos.Invitations.UpdateStatus(ctx, "nope", invitations.StatusUpdate{ExpectedTokenHash: "missing", TokenHash: "missing", ExpiresAt: suiteBase.Add(time.Hour), Status: invitations.StatusCancelled, UpdatedAt: suiteBase}); !errors.Is(err, sdk.ErrNotFound) {
...
func testInvitationsListByResourcePaged(t *testing.T, repos auth.Repositories) {
	...
	created := make([]invitations.Invitation, 0, 5)
	for i := 0; i < 5; i++ {
	...
	got := pageAllInvitations(t, func(req listing.Request) (listing.Page[invitations.Invitation], error) {
...
func testInvitationsListByResourceCollision(t *testing.T, repos auth.Repositories) {
	...
	created := make([]invitations.Invitation, 0, 4)
	for i := 0; i < 4; i++ {
...
func testInvitationsListBySubjectPaged(t *testing.T, repos auth.Repositories) {
	...
	want := invitationIDsSorted(created)
	got := pageAllInvitations(t, func(req listing.Request) (listing.Page[invitations.Invitation], error) {
		return repo.ListBySubject(ctx, sdk.AddressKindEmail, "subject@example.com", req)
	}, 2)
	if !equalStrings(got, want) {
...
func testInvitationsListBySubjectCollision(t *testing.T, repos auth.Repositories) {
	...
	want := invitationIDsSorted(created)
	got := pageAllInvitations(t, func(req listing.Request) (listing.Page[invitations.Invitation], error) {
		return repo.ListBySubject(ctx, sdk.AddressKindEmail, "collide@example.com", req)
	}, 2)
	if !equalStrings(got, want) {
...
func testInvitationsListBySubjectKindIsolation(t *testing.T, repos auth.Repositories) {
	...
	emailInv, err := invitations.NewInvitation(ids, "project", "res-e", "member", shared, sdk.AddressKindEmail, "inviter-1", "hash-iso-e", false, time.Hour, suiteBase)
	if err != nil {
		t.Fatalf("New(email): %v", err)
	}
	if _, err := repo.Create(ctx, emailInv); err != nil {
	...
	phoneInv, err := invitations.NewInvitation(ids, "project", "res-p", "member", shared, sdk.AddressKindPhone, "inviter-1", "hash-iso-p", false, time.Hour, suiteBase)
	if err != nil {
		t.Fatalf("New(phone): %v", err)
	}
	if _, err := repo.Create(ctx, phoneInv); err != nil {
	...
	emailPage, err := repo.ListBySubject(ctx, sdk.AddressKindEmail, shared, listing.Request{Limit: 10})
	if err != nil {
		t.Fatalf("ListBySubject(email): %v", err)
	}
	if len(emailPage.Items) != 1 || emailPage.Items[0].IdentifierKind != sdk.AddressKindEmail {
		t.Errorf("ListBySubject(email) leaked across kinds: %+v", emailPage.Items)
	}
	phonePage, err := repo.ListBySubject(ctx, sdk.AddressKindPhone, shared, listing.Request{Limit: 10})
	if err != nil {
		t.Fatalf("ListBySubject(phone): %v", err)
	}
	if len(phonePage.Items) != 1 || phonePage.Items[0].IdentifierKind != sdk.AddressKindPhone {
...
func mustNewInvitation(t *testing.T, resourceType, resourceID, relation, identifier, invitedBy, tokenHash string, createdAt time.Time) invitations.Invitation {
	t.Helper()
	inv, err := invitations.NewInvitation(ids, resourceType, resourceID, relation, identifier, "", invitedBy, tokenHash, false, time.Hour, createdAt)
	if err != nil {
		t.Fatalf("invitations.New: %v", err)
	}
	return inv
}

func invitationIDsSorted(invs []invitations.Invitation) []string {
	sorted := append([]invitations.Invitation(nil), invs...)
	sort.Slice(sorted, func(i, j int) bool {
		if !sorted[i].CreatedAt.Equal(sorted[j].CreatedAt) {
			return sorted[i].CreatedAt.After(sorted[j].CreatedAt)
		}
...
func pageAllInvitations(t *testing.T, list func(listing.Request) (listing.Page[invitations.Invitation], error), limit int) []string {
	t.Helper()
	var ids []string
	cursor := ""
	for i := 0; i < 100; i++ { // bound against a runaway cursor
		page, err := list(listing.Request{Limit: limit, Cursor: cursor})
		if err != nil {
			t.Fatalf("List: %v", err)
		}
...
type pagedCase[T any] struct {
	list      func(ctx context.Context, req listing.Request) (listing.Page[T], error)
	idOf      func(T) string
...
func runPagedFamily[T any](
	...
	scope func(repos auth.Repositories, ctx context.Context, req listing.Request) (listing.Page[T], error),
	...
	newCase := func(repos auth.Repositories) pagedCase[T] {
		return pagedCase[T]{
			list:      func(ctx context.Context, req listing.Request) (listing.Page[T], error) { return scope(repos, ctx, req) },
			idOf:      idOf,
			createdAt: createdAt,
		}
	}
...
func runOrderCase[T any](t *testing.T, pc pagedCase[T], created []T) {
	...
	gotAsc := pageAllOrdered(t, pc, listing.NewOrder("created_at", listing.ASC), 2)
	...
	gotDesc := pageAllOrdered(t, pc, listing.NewOrder("created_at", listing.DESC), 2)
...
func runPrevPageCase[T any](t *testing.T, pc pagedCase[T], created []T) {
	...
	page1, err := pc.list(ctx, listing.Request{Limit: 2})
	if err != nil {
	...
	page2, err := pc.list(ctx, listing.Request{Limit: 2, Cursor: page1.NextCursor})
	if err != nil {
	...
	page3, err := pc.list(ctx, listing.Request{Limit: 2, Cursor: page2.NextCursor})
	if err != nil {
...
func runOffsetModeCase[T any](t *testing.T, pc pagedCase[T], created []T) {
	...
	cursorIDs := pageAllOrdered(t, pc, listing.Order{}, 2)

	var offsetIDs []string
	for off := 0; off < 100; off += 2 {
		page, err := pc.list(ctx, listing.Request{Strategy: listing.StrategyOffset, Limit: 2, Offset: off})
		if err != nil {
	...
	zero, err := pc.list(ctx, listing.Request{Strategy: listing.StrategyOffset, Limit: 2, Offset: 0})
	if err != nil {
...
func runWithCountCase[T any](t *testing.T, pc pagedCase[T], created []T, wantTotal int) {
	...
	cursorPage, err := pc.list(ctx, listing.Request{Limit: 2, WithCount: true})
	if err != nil {
	...
	offsetPage, err := pc.list(ctx, listing.Request{Strategy: listing.StrategyOffset, Limit: 2, Offset: 2, WithCount: true})
	if err != nil {
	...
	noCount, err := pc.list(ctx, listing.Request{Limit: 2})
	if err != nil {
...
func runStaleCursorCase[T any](t *testing.T, pc pagedCase[T], created []T) {
	...
	first, err := pc.list(ctx, listing.Request{Limit: 2})
	if err != nil {
		t.Fatalf("first page: %v", err)
	}

	stale, err := listing.EncodeCursor("updated_at", pc.createdAt(created[0]), pc.idOf(created[0]))
	if err != nil {
		t.Fatalf("EncodeCursor: %v", err)
	}
	got, err := pc.list(ctx, listing.Request{Limit: 2, Cursor: stale})
	if err != nil {
...
func runCursorOffsetExclusiveCase[T any](t *testing.T, pc pagedCase[T]) {
	t.Helper()
	ctx := context.Background()
	if _, err := pc.list(ctx, listing.Request{Strategy: listing.StrategyCursor, Limit: 2, Offset: 2}); !errors.Is(err, sdk.ErrInvalidInput) {
		t.Errorf("cursor strategy + offset: err=%v, want ErrInvalidInput", err)
	}
	if _, err := pc.list(ctx, listing.Request{Strategy: listing.StrategyOffset, Limit: 2, Cursor: "anything"}); !errors.Is(err, sdk.ErrInvalidInput) {
...
func backPage[T any](t *testing.T, pc pagedCase[T], previousCursor string) listing.Page[T] {
	t.Helper()
	page, err := pc.list(context.Background(), listing.Request{Limit: 2, Cursor: previousCursor})
	if err != nil {
...
func pageAllOrdered[T any](t *testing.T, pc pagedCase[T], order listing.Order, limit int) []string {
	t.Helper()
	ctx := context.Background()
	var ids []string
	cursor := ""
	for i := 0; i < 100; i++ { // bound against a runaway cursor
		page, err := pc.list(ctx, listing.Request{Limit: limit, Cursor: cursor, Order: order})
		if err != nil {
			t.Fatalf("List: %v", err)
		}
...
func seedInvitationsByResource(t *testing.T, repo invitations.InvitationRepository) ([]invitations.Invitation, int) {
	t.Helper()
	ctx := context.Background()
	created := make([]invitations.Invitation, 0, len(familyMinutes))
	for i, m := range familyMinutes {
		// Distinct identifier per row keeps each a distinct pending tuple.
...
func seedInvitationsBySubject(t *testing.T, repo invitations.InvitationRepository) ([]invitations.Invitation, int) {
	t.Helper()
	ctx := context.Background()
	created := make([]invitations.Invitation, 0, len(familyMinutes))
	for i, m := range familyMinutes {
...
// The five tests below prove the Create side of the sdk.DatabaseID strategy:
...
func testInvitationsDBGeneratedID(t *testing.T, repos auth.Repositories) {
	ctx := context.Background()
	inv, err := invitations.NewInvitation(dbIDs, "project", "p-dbgen", "member", "dbgen@example.com", "", "inviter-1", "hash-dbgen-inv", false, time.Hour, time.Now())
	if err != nil {
		t.Fatalf("New: %v", err)
	}
...
func testPasswordResetRedeem(t *testing.T, repos auth.Repositories) {
	...
	proof := prepareResetProof(t, repos, userID)
	...
	if _, err := repos.Challenges.Replace(ctx, newChallenge(userID, passwordResetPurpose, "", digest, proof, 0, time.Hour, time.Now())); err != nil {
	...
	if repos.AuthenticationGrants != nil {
		if _, err := (grantFixture{repos}).Create(ctx, newGrant(s1.ID, userID, challenge.PurposeRemovePassword, "ctx", 5*time.Minute, time.Now())); err != nil {
	...
	if repos.AuthenticationGrants != nil {
		if _, err := (grantFixture{repos}).Consume(ctx, s1.ID, challenge.PurposeRemovePassword, "ctx", time.Now()); !errors.Is(err, sdk.ErrNotFound) {
...
func testPasswordResetUnknown(t *testing.T, repos auth.Repositories) {
	ctx := context.Background()
	ensurePasswordOwner(t, repos, "u-unknown")
...
func seedCredentialUser(t *testing.T, repos auth.Repositories) (userID string, hasOAuth bool) {
	...
	created, _, err := repos.Users.Provision(ctx, u, ident, user.InitialCredentials{PasswordHash: "hash-cred"})
	if err != nil {
		t.Fatalf("CreateWithPrimaryIdentifier: %v", err)
	}
...
func testContactChangeReplace(t *testing.T, repos auth.Repositories) {
	ctx := context.Background()
	repo := contactFixture{repos.ContactChanges}
...
func testContactChangeMissing(t *testing.T, repos auth.Repositories) {
	if _, err := repos.ContactChanges.Consume(context.Background(), "nobody", identifier.KindEmail, "missing"); !errors.Is(err, sdk.ErrNotFound) {
