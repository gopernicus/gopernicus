...
// collision-free path even when sdk.DatabaseID leaves the entity ID empty
// for the store to assign at insert.
var storageKeys = sdk.IDGenerator{}
...
// sdk.ErrInvalidInput.
func NewAsset(ids sdk.IDGenerator, filename, contentType string, size int64, now time.Time) (Asset, error) {
...
func storageKey(key, filename string) string {
	...
	name := sdk.Slugify(strings.TrimSuffix(filename, ext))
	if name == "" {
		name = "file"
	}
