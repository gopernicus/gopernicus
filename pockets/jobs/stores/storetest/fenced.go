...
func RunFencedQueue(t *testing.T, newRepo func(t *testing.T) job.FencedQueueRepository) {
	...
	t.Run("Deferral", func(t *testing.T) { runFencedDeferral(t, newRepo) })
	t.Run("Ownership", func(t *testing.T) { RunFencedOwnership(t, newRepo) })
}
...
// queue.FencedClaim.TenantID, and the fenced rail's encrypted payloads make the
