...
func RunQueue(t *testing.T, newRepo func(t *testing.T) job.QueueRepository) {
	...
	t.Run("ListFilterAndPaginate", func(t *testing.T) {
		...
		runPagedFamily(t, newRepo,
			func(repo job.QueueRepository, ctx context.Context, req listing.Request) (listing.Page[job.Job], error) {
	...
	t.Run("Deferral", func(t *testing.T) { runQueueDeferral(t, newRepo) })
	t.Run("Ownership", func(t *testing.T) { RunQueueOwnership(t, newRepo) })
}
...
func RunSchedules(t *testing.T, newRepo func(t *testing.T) schedule.Repository) {
	...
	t.Run("PendingOccurrences", func(t *testing.T) { RunOccurrences(t, newRepo) })
	...
	t.Run("ListPaginate", func(t *testing.T) {
		...
		runPagedFamily(t, newRepo,
			func(repo schedule.Repository, ctx context.Context, req listing.Request) (listing.Page[schedule.Schedule], error) {
...
func testQueueTenant(t *testing.T, repo job.QueueRepository) {
	...
	page, err := repo.List(ctx, job.ListFilter{Kind: "demo"}, listing.Request{Limit: 10})
	if err != nil {
		t.Fatalf("List: %v", err)
	}
...
func testQueueListFilterAndPaginate(t *testing.T, repo job.QueueRepository) {
	...
	got := drainPages(t, 2, func(j job.Job) string { return j.ID() },
		func(req listing.Request) (listing.Page[job.Job], error) {
			return repo.List(ctx, job.ListFilter{Kind: "email"}, req)
		})
	...
	running, err := repo.List(ctx, job.ListFilter{Status: job.StatusRunning}, listing.Request{Limit: 10})
	if err != nil {
...
func testClaimDueKindGuard(t *testing.T, repo schedule.Repository) {
	...
	lost, err := repo.ClaimDue(ctx, s, slot1, now)
	if err != nil {
	...
	won, err := repo.ClaimDue(ctx, rekinded, slot1, now)
	if err != nil {
...
func testClaimDueCAS(t *testing.T, repo schedule.Repository) {
	...
	won, err := repo.ClaimDue(ctx, s, slot1, now)
	if err != nil {
	...
	lost, err := repo.ClaimDue(ctx, s, slot0.Add(2*time.Hour), now)
	if err != nil {
	...
	if won, _ := repo.ClaimDue(ctx, after, slot1.Add(time.Hour), slot1); won {
...
func testSetLastJobAndEnabled(t *testing.T, repo schedule.Repository) {
	...
	if won, err := repo.ClaimDue(ctx, s, suiteBase.Add(time.Hour), suiteBase); err != nil || !won {
		t.Fatalf("ClaimDue: %v, %v", won, err)
	}
	jobID := schedule.OccurrenceID(s.ID, s.NextRunAt)
	if err := repo.AckOccurrence(ctx, jobID, suiteBase); err != nil {
		t.Fatalf("SetLastJob: %v", err)
	}
	got, _ := repo.Get(ctx, s.ID)
	if got.LastJobID != jobID {
		t.Errorf("LastJobID = %q, want %q", got.LastJobID, jobID)
	}
...
func testSchedulesListPaginate(t *testing.T, repo schedule.Repository) {
	...
	got := drainPages(t, 2, func(s schedule.Schedule) string { return s.ID },
		func(req listing.Request) (listing.Page[schedule.Schedule], error) {
			return repo.List(ctx, req)
		})
...
func drainPages[T any](t *testing.T, limit int, id func(T) string, fetch func(listing.Request) (listing.Page[T], error)) []T {
	t.Helper()
	var out []T
	seen := map[string]bool{}
	cursor := ""
	for i := 0; i < 1000; i++ {
		page, err := fetch(listing.Request{Limit: limit, Cursor: cursor})
		if err != nil {
...
func runPagedFamily[R any, T any](
	...
	scope func(repo R, ctx context.Context, req listing.Request) (listing.Page[T], error),
	...
	newCase := func(repo R) pagedCase[T] {
		return pagedCase[T]{
			list:      func(ctx context.Context, req listing.Request) (listing.Page[T], error) { return scope(repo, ctx, req) },
			idOf:      idOf,
			createdAt: createdAt,
		}
	}
