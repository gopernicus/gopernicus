...
func (q *FencedQueue) Replace(_ context.Context, in job.Enqueue) (job.Job, error) {
	...
	// Check insertion failure before retiring the active generation. The mutex
	// keeps this ID available until insert, matching the SQL transaction rollback.
	if in.ID == "" {
		in.ID = newID("job")
	}
	if _, exists := q.jobs[in.ID]; exists {
...
func (q *FencedQueue) insert(in job.Enqueue) (job.Job, error) {
	...
	now := time.Now().UTC()
	if in.LogicalKey != "" {
		// Preserve generation order if the wall clock repeats or moves backward.
		for _, existing := range q.jobs {
			if existing.LogicalKey == in.LogicalKey && !now.After(existing.CreatedAt) {
