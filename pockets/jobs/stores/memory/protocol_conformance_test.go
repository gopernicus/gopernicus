...
func newProtocolHarness(t *testing.T) (worktest.ReplaceQueue, worktest.Inspector) {
	t.Helper()
	fq := NewFencedQueue()
	svc, err := queueapi.NewService(queueapi.Repositories{FencedQueue: fq})
	if err != nil {
...
func TestWorkProtocolLifecycleProjection(t *testing.T) {
	ctx := context.Background()
	q := NewFencedQueue()
	svc, err := queueapi.NewService(queueapi.Repositories{FencedQueue: q})
	if err != nil {
		t.Fatal(err)
	}
	id, err := svc.EnqueueOnce(ctx, "kind", "key", []byte("payload"))
	if err != nil {
		t.Fatal(err)
	}
	check := func(want work.Status) {
		t.Helper()
		got, err := svc.LatestStatusByKey(ctx, "key")
		if err != nil || got != want {
			t.Fatalf("status=%v err=%v, want %v", got, err, want)
		}
	}
	check(work.StatusPending)
	now := time.Now().Add(time.Second)
	if _, err := q.Claim(ctx, now, "lease", time.Minute, nil); err != nil {
		t.Fatal(err)
	}
	check(work.StatusRunning)
	if err := q.Reschedule(ctx, id, "lease", now.Add(time.Second), "retry", now); err != nil {
		t.Fatal(err)
	}
	check(work.StatusPending)
	if _, err := q.Claim(ctx, now.Add(time.Second), "lease2", time.Minute, nil); err != nil {
		t.Fatal(err)
	}
	if err := q.Complete(ctx, id, "lease2", now.Add(time.Second)); err != nil {
		t.Fatal(err)
	}
	check(work.StatusCompleted)
	fresh, err := svc.EnqueueOnce(ctx, "kind", "key", []byte("fresh"))
	if err != nil || fresh == id {
		t.Fatalf("fresh=%q err=%v", fresh, err)
	}
	check(work.StatusPending)
	if _, err := q.Claim(ctx, now.Add(2*time.Second), "lease3", time.Minute, nil); err != nil {
		t.Fatal(err)
	}
	if err := q.Fail(ctx, fresh, "lease3", "permanent", now.Add(2*time.Second)); err != nil {
		t.Fatal(err)
	}
	check(work.StatusDeadLetter)
	canceled, err := svc.Replace(ctx, "kind", "key", []byte("cancel"))
	if err != nil {
		t.Fatal(err)
	}
	if err := q.Cancel(ctx, canceled, now.Add(3*time.Second)); err != nil {
		t.Fatal(err)
	}
	check(work.StatusCanceled)
}
