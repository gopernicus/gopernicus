// Package memory is the jobs pocket's in-core reference store: stdlib-only,
...
// ignored, preserving the previous setting (or the default).
...
func NewQueue(opts ...Option) *Queue {
	...
	for _, opt := range opts {
		if opt == nil {
			panic("jobs memory: NewQueue received a nil option")
		}
		opt(&cfg)
	}
	return &Queue{jobs: map[string]job.Job{}, lease: cfg.lease}
}
...
func (q *Queue) List(_ context.Context, f job.ListFilter, req list.Request) (list.Page[job.Job], error) {
	q.mu.Lock()
	defer q.mu.Unlock()
...
func page[T any](items []T, req list.Request, orderFields map[string]list.OrderField, key func(T) (time.Time, string)) (list.Page[T], error) {
