package schedules

import "github.com/gopernicus/gopernicus/sdk/pkg/list"

// OrderFields is the allow-list of sortable columns for List: only these vetted
// column names may reach a store's ORDER BY. The map key is the API-facing field
// name (it coincides with the column). created_at is the indexed spine column the
// schedules list pages by; the schedule_id tiebreak is applied by the store, not
// listed here.
var OrderFields = map[string]list.OrderField{
	"created_at": {Column: "created_at"},
}

// DefaultOrder is the sort applied when a Request carries a zero-value Order:
// created_at DESC (with the store's schedule_id DESC tiebreak). Its Field is the
// resolved column, so a backend matches it against OrderFields by column.
var DefaultOrder = list.NewOrder("created_at", list.DESC)
