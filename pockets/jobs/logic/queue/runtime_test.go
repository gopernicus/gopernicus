...
func newOneShotQueue(jobs ...Job) *oneShotQueue {
	return &oneShotQueue{
		pending:   jobs,
		completed: map[string]bool{},
		failed:    map[string]string{},
...
func (q *oneShotQueue) Enqueue(ctx context.Context, in Enqueue) (Job, error) {
	return Job{}, sdk.ErrInvalidInput
}
func (q *oneShotQueue) Claim(ctx context.Context, workerID string, now time.Time, kinds []string) (Job, error) {
	q.mu.Lock()
	defer q.mu.Unlock()
...
func (q *oneShotQueue) Get(ctx context.Context, id string) (Job, error) {
	return Job{}, sdk.ErrNotFound
}
func (q *oneShotQueue) List(ctx context.Context, _ ListFilter, _ list.Request) (list.Page[Job], error) {
...
func TestRun_UnknownKind_Fails(t *testing.T) {
	q := newOneShotQueue(Job{JobID: "j1", Kind: "mystery.kind", JobStatus: StatusPending})
	rt := newTestRuntime(t, q, runtimeTestConfig{
		Handlers:     map[string]HandlerFunc{"known.kind": func(context.Context, Job) error { return nil }},
		Workers:      1,
...
// the registered handler kinds to every Claim (#37): the store, not the handler lookup, is what
// keeps a runtime off jobs it cannot process.
func TestRun_ClaimsWithRegisteredKinds(t *testing.T) {
	q := newOneShotQueue(Job{JobID: "j3", Kind: "known.kind", JobStatus: StatusPending})
	rt := newTestRuntime(t, q, runtimeTestConfig{
		Handlers:     map[string]HandlerFunc{"known.kind": func(context.Context, Job) error { return nil }},
		Workers:      1,
		PollInterval: 20 * time.Millisecond,
...
func TestRun_KnownKind_Completes(t *testing.T) {
	q := newOneShotQueue(Job{JobID: "j2", Kind: "known.kind", JobStatus: StatusPending})
	var called bool
	rt := newTestRuntime(t, q, runtimeTestConfig{
		Handlers: map[string]HandlerFunc{"known.kind": func(context.Context, Job) error {
			called = true
			return nil
		}},
		Workers:      1,
...
func TestRun_CancelDrainsInFlightHandlerAndPersistsCompletion(t *testing.T) {
	q := newOneShotQueue(Job{JobID: "j-drain", Kind: "known.kind", JobStatus: StatusPending})
	started := make(chan struct{})
	release := make(chan struct{})

	rt := newTestRuntime(t, q, runtimeTestConfig{
		Handlers: map[string]HandlerFunc{"known.kind": func(ctx context.Context, _ Job) error {
			close(started)
			select {
			case <-ctx.Done():
				return ctx.Err()
...
// TestHeartbeatReachesThePools proves runtimeTestConfig.Heartbeat is passed through to the
...
func TestHeartbeatReachesThePools(t *testing.T) {
	...
	rt := newTestRuntime(t, newOneShotQueue(), runtimeTestConfig{ // empty: every claim is ErrNoWork
		Handlers:     map[string]HandlerFunc{"noop": func(context.Context, Job) error { return nil }},
	...
	if !strings.Contains(buf.String(), "pool alive") {
		t.Fatalf("no \"pool alive\" heartbeat line reached the logger; runtimeTestConfig.Heartbeat is not passed to the pool")
	}
}

func newTestRuntime(t *testing.T, queue QueueRepository, cfg runtimeTestConfig) *Runtime {
	t.Helper()
	svc, err := NewService(Repositories{Queue: queue})
	if err != nil {
		t.Fatal(err)
	}
	rt, err := runtimeFromTestConfig(svc, nil, cfg)
	if err != nil {
		t.Fatal(err)
	}
	return rt
