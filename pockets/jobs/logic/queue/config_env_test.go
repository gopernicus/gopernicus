...
// TestConfigEnvTags pins the JOBS_* keys on the host-loadable runtime policy.
func TestConfigEnvTags(t *testing.T) {
	...
	var cfg queueapi.RuntimePolicy
...
// TestFencedRuntimePolicyEnvTags pins the fenced runtime's keys, including the
// two it does not share with RuntimePolicy (JOBS_LEASE_FOR, JOBS_PROCESS_TIMEOUT).
func TestFencedRuntimePolicyEnvTags(t *testing.T) {
	...
	var cfg queueapi.FencedRuntimePolicy
	if err := environment.ParseEnvTags("", &cfg); err != nil {
		t.Fatalf("ParseEnvTags: %v", err)
	}
...
// TestFencedRuntimePolicyEnvNamespace proves the documented disambiguation for a
// host running both runtimes in one process: the fenced config shares the JOBS_*
// key names with RuntimePolicy, so it is parsed under a namespace and reads
// FENCED_JOBS_* instead — the unnamespaced values never reach it.
func TestFencedRuntimePolicyEnvNamespace(t *testing.T) {
	t.Setenv("JOBS_WORKERS", "7")
