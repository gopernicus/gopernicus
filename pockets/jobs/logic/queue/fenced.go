...
// FencedQueueDeferrer is optional support for temporary job gates. Defer must
// atomically validate the current live lease, release it, schedule pending work
// at availableAt and refund only this claim's attempt. Repeated or stale calls
// return sdk.ErrConflict without modifying prior attempts or payload.
type FencedQueueDeferrer interface {
...
// signatures, and queue.NewFencedRuntime adapts the port to the kernel by closing
...
// is domain-rich; the consumer-facing keyed-work protocol a consuming pocket depends on
// without importing this package is sdk/capabilities/work (Enqueuer, Replacer,
// StatusReader), which the jobs Service implements.
//
// Memory, pgx and Turso implement this port; storetest.RunFencedQueue exercises
// its contract. QueueRepository remains the ordinary queue/scheduler substrate.
//
...
//   - Reschedule selects the next available time and clears the lease; Fail
//     dead-letters permanently. The default runtime backoff selects a future time;
//     callers choosing a time at/before now make the job immediately eligible.
type FencedQueueRepository interface {
	...
	// cleanup. Zero removes nothing; negative limits request unbounded cleanup.
	// It never touches a non-terminal job.
