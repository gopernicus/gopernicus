...
// newFencedService builds a jobs pocketjobs.Components over the in-core fenced queue reference.
func newFencedService(t *testing.T) (*pocketjobs.Components, *memory.FencedQueue) {
	t.Helper()
...
func TestFencedPrimitives(t *testing.T) {
	...
	id1, err := svc.Queue.EnqueueOnce(ctx, "k", "key-1", json.RawMessage(`"a"`))
	if err != nil {
		t.Fatalf("EnqueueOnce: %v", err)
	}
	id2, err := svc.Queue.EnqueueOnce(ctx, "k", "key-1", json.RawMessage(`"b"`))
	if err != nil {
	...
	status, err := svc.Queue.LatestStatusByKey(ctx, "key-1")
	if err != nil {
	...
	id3, err := svc.Queue.Replace(ctx, "k", "key-1", json.RawMessage(`"c"`))
	if err != nil {
...
func TestFencedRuntimeCompletes(t *testing.T) {
	...
	rt, err := queueapi.NewFencedRuntime(svc.Queue, map[string]queueapi.FencedHandlerFunc{
		"delivery": func(ctx context.Context, claim queueapi.FencedClaim) error {
			handled.Add(1)
	...
	if _, err := svc.Queue.EnqueueOnce(ctx, "delivery", "key-run", json.RawMessage(`"opaque"`)); err != nil {
		t.Fatalf("EnqueueOnce: %v", err)
	}
	...
	waitFor(t, time.Second, func() bool {
		s, err := svc.Queue.LatestStatusByKey(ctx, "key-run")
...
func TestFencedRuntimeDeadLetters(t *testing.T) {
	...
	}, queueapi.WithFencedRuntimePolicy(queueapi.FencedRuntimePolicy{Workers: 1, PollInterval: 5 * time.Millisecond, IdleInterval: 5 * time.Millisecond, MaxAttempts: 1, Backoff: func(int) time.Duration { return time.Millisecond }}), queueapi.WithDeadLetters(map[string]queueapi.DeadLetterFunc{
		"delivery": func(ctx context.Context, j queueapi.Job) error {
	...
	if _, err := svc.Queue.EnqueueOnce(ctx, "delivery", "key-dl", json.RawMessage(`"x"`)); err != nil {
		t.Fatalf("EnqueueOnce: %v", err)
	}
	runCtx, cancel := context.WithCancel(ctx)
	done := make(chan struct{})
	...
	waitFor(t, time.Second, func() bool {
		s, err := svc.Queue.LatestStatusByKey(ctx, "key-dl")
...
// queueapi.Permanent disposition dead-letters on the FIRST attempt — no retry — even when the
...
func TestFencedRuntimePermanentDeadLettersImmediately(t *testing.T) {
	...
	rt, err := queueapi.NewFencedRuntime(svc.Queue, map[string]queueapi.FencedHandlerFunc{
		"delivery": func(ctx context.Context, claim queueapi.FencedClaim) error {
			attempts.Add(1)
			return queueapi.Permanent("structurally dead")
		},
	}, queueapi.WithFencedRuntimePolicy(queueapi.FencedRuntimePolicy{Workers: 1, PollInterval: 5 * time.Millisecond, IdleInterval: 5 * time.Millisecond, MaxAttempts: 50, Backoff: func(int) time.Duration { return time.Millisecond }}))
	if err != nil {
		t.Fatalf("NewFencedRuntime: %v", err)
	}

	if _, err := svc.Queue.EnqueueOnce(ctx, "delivery", "key-perm", json.RawMessage(`"x"`)); err != nil {
		t.Fatalf("EnqueueOnce: %v", err)
	}
	runCtx, cancel := context.WithCancel(ctx)
	done := make(chan struct{})
	...
	waitFor(t, time.Second, func() bool {
		s, err := svc.Queue.LatestStatusByKey(ctx, "key-perm")
		return err == nil && s == "dead_letter"
	})
	cancel()
	<-done
...
func TestNewFencedRuntime_ProcessTimeoutInsideLease(t *testing.T) {
	svc, _ := newFencedService(t)
	handlers := map[string]queueapi.FencedHandlerFunc{"k": func(context.Context, queueapi.FencedClaim) error { return nil }}

	// Inverted: timeout >= lease → loud construction failure.
	if _, err := queueapi.NewFencedRuntime(svc.Queue, handlers, queueapi.WithFencedRuntimePolicy(queueapi.FencedRuntimePolicy{LeaseFor: time.Second, ProcessTimeout: time.Second})); !errors.Is(err, queueapi.ErrProcessTimeoutExceedsLease) {
		t.Fatalf("err = %v, want queueapi.ErrProcessTimeoutExceedsLease for timeout == lease", err)
	}
	if _, err := queueapi.NewFencedRuntime(svc.Queue, handlers, queueapi.WithFencedRuntimePolicy(queueapi.FencedRuntimePolicy{LeaseFor: time.Second, ProcessTimeout: 2 * time.Second})); !errors.Is(err, queueapi.ErrProcessTimeoutExceedsLease) {
		t.Fatalf("err = %v, want queueapi.ErrProcessTimeoutExceedsLease for timeout > lease", err)
	}
	// Valid: timeout safely inside the lease.
	if _, err := queueapi.NewFencedRuntime(svc.Queue, handlers, queueapi.WithFencedRuntimePolicy(queueapi.FencedRuntimePolicy{LeaseFor: time.Second, ProcessTimeout: 200 * time.Millisecond})); err != nil {
		t.Fatalf("timeout inside lease should construct: %v", err)
	}
}

// TestServicePurgeTerminal proves the pocketjobs.Components exposes the bounded terminal purge a host
...
func TestServicePurgeTerminal(t *testing.T) {
	...
	id, err := svc.Queue.EnqueueOnce(ctx, "k", "key-purge", json.RawMessage(`"x"`))
	if err != nil {
		t.Fatalf("EnqueueOnce: %v", err)
	}
	...
	n, err := svc.Queue.PurgeTerminal(ctx, time.Now(), 10)
	if err != nil {
	...
	// A non-fenced pocketjobs.Components refuses.
	unfenced, err := pocketjobs.New(pocketjobs.Repositories{Queue: newMemQueue()})
	if err != nil {
		t.Fatalf("NewService: %v", err)
	}
	if _, err := unfenced.Queue.PurgeTerminal(ctx, time.Now(), 10); !errors.Is(err, queueapi.ErrFencedQueueRequired) {
		t.Fatalf("err = %v, want queueapi.ErrFencedQueueRequired", err)
	}
}
...
func TestNewFencedRuntime_RequiresFencedQueue(t *testing.T) {
	...
	if _, err := queueapi.NewFencedRuntime(svc.Queue, map[string]queueapi.FencedHandlerFunc{"k": func(context.Context, queueapi.FencedClaim) error { return nil }}); err != queueapi.ErrFencedQueueRequired {
		t.Fatalf("err = %v, want queueapi.ErrFencedQueueRequired", err)
	}
}
