...
// the two SET reads (FilterRelation, RelationTargetsFor) and their PARITY with
// the per-resource siblings they replace. Parity is the whole point — the engine
// answers a candidate set with these and must reach the answers N per-resource
// reads reach.
func runRelationshipSetReads(t *testing.T, newRepos func(t *testing.T) authorization.Repositories) {
	...
	t.Run("FilterRelationIsSortedDistinctSubset", func(t *testing.T) {
		...
		tuples := make([]relationship.CreateRelationship, 0, len(keysetIDs))
	...
	t.Run("FilterRelationMatchesPerResourceCheck", func(t *testing.T) {
		...
		got, err := s.FilterRelation(ctx, "doc", setReadIDs, "viewer", "user", "u1", 0)
		if err != nil {
			t.Fatalf("FilterRelation: %v", err)
		}
	...
	t.Run("FilterRelationEmptyInput", func(t *testing.T) {
		...
		got, err := s.FilterRelation(ctx, "doc", nil, "viewer", "user", "u1", 0)
		if err != nil || len(got) != 0 {
			t.Fatalf("FilterRelation over no ids must be an empty result: got %v err=%v", got, err)
		}
		got, err = s.FilterRelation(ctx, "doc", []string{}, "viewer", "user", "u1", 0)
		if err != nil || len(got) != 0 {
	...
	t.Run("RelationTargetsForMatchesPerResource", func(t *testing.T) {
		...
		got, err := s.RelationTargetsFor(ctx, "space", setReadIDs, "parent")
		if err != nil {
	...
	t.Run("RelationTargetsForEmptyInput", func(t *testing.T) {
		...
		got, err := s.RelationTargetsFor(ctx, "space", nil, "parent")
		if err != nil || len(got) != 0 {
	...
	t.Run("RelationTargetsForFoldsDuplicateIDsOverBatchBound", func(t *testing.T) {
		...
		tuples := make([]relationship.CreateRelationship, 0, overBoundIDs)
		...
		got, err := s.RelationTargetsFor(ctx, "space", ids, "parent")
		if err != nil {
...
func seedOverBoundDocs(t *testing.T, s relationship.Storer) (ids, want []string) {
	t.Helper()
	tuples := make([]relationship.CreateRelationship, 0, overBoundIDs)
	for i := 0; i < overBoundIDs; i++ {
...
func sameTargetSet(a, b []relationship.RelationTarget) bool {
	if len(a) != len(b) {
		return false
	}
