...
func runRelationshipKeyset(t *testing.T, newRepos func(t *testing.T) authorization.Repositories) {
	...
	t.Run("Direct", func(t *testing.T) {
		...
		tuples := make([]relationship.CreateRelationship, 0, len(keysetIDs)+2)
	...
	t.Run("ByRelationTarget", func(t *testing.T) {
		s := newRepos(t).Relationships
		tuples := make([]relationship.CreateRelationship, 0, len(keysetIDs)+1)
		for _, id := range keysetIDs {
...
// =============================================================================
// Engine: paged enumeration (LookupResourcesIn)
// =============================================================================
...
// non-nil, Truncated mirrors HasMore, a page with more to come is FULL and
// carries a continuation, the last page carries none, and no id repeats across
// pages.
func walkPaged(t *testing.T, svc *authorization.Service, principal authorization.PrincipalRef, permission, resourceType string, limit int) []string {
	t.Helper()
	ctx := context.Background()
	...
	for page := 0; ; page++ {
		if page > maxPagedWalkPages {
			t.Fatalf("LookupResourcesIn(%s:%s, %s, %s, limit %d) did not terminate within %d pages",
				principal.Type, principal.ID, permission, resourceType, limit, maxPagedWalkPages)
		}
		res, err := svc.LookupResourcesIn(ctx, authorization.LookupRequest{
			Principal:    principal,
		...
		if err != nil {
			t.Fatalf("LookupResourcesIn(%s:%s, %s, %s, limit %d, page %d): %v",
		...
		if res.Truncated != res.HasMore {
			t.Fatalf("page %d: deprecated Truncated=%v must mirror HasMore=%v", page, res.Truncated, res.HasMore)
		}
...
// LookupResourcesIn to exhaustion concatenates to EXACTLY the plain
// LookupResources result — same ids, same order, no repeats — and every id it
// yields independently passes Check. It returns the plain result.
func assertPagedParityAt(t *testing.T, svc *authorization.Service, principal authorization.PrincipalRef, permission, resourceType string, limits []int) []string {
	t.Helper()
	ctx := context.Background()

	plain, err := svc.LookupResources(ctx, principal, permission, resourceType)
	if err != nil {
...
func assertPagedParity(t *testing.T, svc *authorization.Service, principal authorization.PrincipalRef, permission, resourceType string) []string {
	t.Helper()
...
func firstPage(t *testing.T, svc *authorization.Service, principal authorization.PrincipalRef, permission, resourceType string, limit int) authorization.LookupResult {
	t.Helper()
	res, err := svc.LookupResourcesIn(context.Background(), authorization.LookupRequest{
		Principal:    principal,
		Permission:   permission,
		ResourceType: resourceType,
		Limit:        limit,
	...
	if err != nil {
		t.Fatalf("LookupResourcesIn(%s:%s, %s, %s, limit %d): %v", principal.Type, principal.ID, permission, resourceType, limit, err)
	}
...
func assertRefusedCursor(t *testing.T, svc *authorization.Service, what string, req authorization.LookupRequest) {
	t.Helper()
	_, err := svc.LookupResourcesIn(context.Background(), req)
	if !errors.Is(err, authorization.ErrInvalidCursor) {
...
func pagingSchema() authorization.Schema {
	return authorization.NewSchema([]authorization.ResourceSchema{
		...
		{Name: "doc", Def: authorization.ResourceTypeDef{
			Relations: map[string]authorization.RelationDef{
				...
				"folder": {AllowedSubjects: []authorization.SubjectTypeRef{{Type: "doc"}}},
			...
			Permissions: map[string]authorization.PermissionRule{
				"view": authorization.AnyOf(
					...
					authorization.Through("folder", "view"),
				),
				"edit": authorization.AnyOf(authorization.Direct("editor")),
			},
		}},
	})
}
...
func changedPagingSchema() authorization.Schema {
	schema := pagingSchema()
	doc := schema.ResourceTypes["doc"]
	doc.Relations["commenter"] = authorization.RelationDef{AllowedSubjects: []authorization.SubjectTypeRef{{Type: "user"}}}
	doc.Permissions["edit"] = authorization.AnyOf(authorization.Direct("editor"), authorization.Direct("commenter"))
	schema.ResourceTypes["doc"] = doc
	return schema
}

func newPagingService(t *testing.T, repos authorization.Repositories, schema authorization.Schema, limits authorization.EvaluationLimits) *authorization.Service {
	t.Helper()
	comps, err := authorization.NewService(repos, authorization.Config{RelationshipModel: schema, Limits: limits})
	if err != nil {
...
func runLookupPagedParity(t *testing.T, newRepos func(t *testing.T) authorization.Repositories) {
	...
	t.Run("HierarchyDescendantBeforeRoot", func(t *testing.T) {
		...
		got := assertPagedParity(t, svc, authorization.PrincipalRef{Type: "user", ID: "u_page"}, "view", "doc")
	...
	t.Run("CursorIsBoundToItsQuery", func(t *testing.T) {
		...
		svc := newPagingService(t, repos, pagingSchema(), generousLimits())
		principal := authorization.PrincipalRef{Type: "user", ID: "u_a"}
		...
		assertRefusedCursor(t, svc, "by another principal", authorization.LookupRequest{
			Principal: authorization.PrincipalRef{Type: "user", ID: "u_b"}, Permission: "view", ResourceType: "doc", Limit: 1, After: cursor,
		})
		assertRefusedCursor(t, svc, "on another permission", authorization.LookupRequest{
			Principal: principal, Permission: "edit", ResourceType: "doc", Limit: 1, After: cursor,
		})
		assertRefusedCursor(t, svc, "on another resource type", authorization.LookupRequest{
		...
		assertRefusedCursor(t, changed, "after a schema change", authorization.LookupRequest{
		...
		second, err := svc.LookupResourcesIn(ctx, authorization.LookupRequest{
			Principal: principal, Permission: "view", ResourceType: "doc", Limit: 1, After: cursor,
		})
		if err != nil {
	...
	t.Run("IntermediateOverflowIsErrorOnEveryPage", func(t *testing.T) {
		...
		svc := newPagingService(t, repos, pagingSchema(), authorization.EvaluationLimits{MaxLookupResults: 2})
		req := authorization.LookupRequest{
			Principal: authorization.PrincipalRef{Type: "user", ID: "u_many"}, Permission: "view", ResourceType: "doc", Limit: 1,
		}
		for attempt := 0; attempt < 2; attempt++ {
			if _, err := svc.LookupResourcesIn(ctx, req); !errors.Is(err, authorization.ErrEvaluationLimit) {
...
func runRolesPagedParity(t *testing.T, newRepos func(t *testing.T) authorization.Repositories) {
	...
	t.Run("ScopedWalk", func(t *testing.T) {
		...
		got := assertPagedParityAt(t, comps.Service, authorization.PrincipalRef{Type: "user", ID: "u_walk"}, "audit", "project", []int{1, 7, 50})
	...
	t.Run("DuplicateGrantingRolesAppearOnce", func(t *testing.T) {
		...
		got := assertPagedParityAt(t, comps.Service, authorization.PrincipalRef{Type: "user", ID: "u1"}, "view", "project", []int{1, 2})
	...
	t.Run("GlobalGrantHasNoPage", func(t *testing.T) {
		...
		res, err := comps.Service.LookupResourcesIn(ctx, authorization.LookupRequest{
			Principal: authorization.PrincipalRef{Type: "user", ID: "u_global"}, Permission: "view", ResourceType: "project", Limit: 1,
		})
		if err != nil {
	...
	t.Run("CursorRefusedAfterRoleModelChange", func(t *testing.T) {
		...
		cursor := firstPage(t, comps.Service, principal, "audit", "project", 1).NextCursor
		...
		changed := newRoleModelService(t, repos, authorization.Config{RoleModel: changedDecisionRoleModel()})
		assertRefusedCursor(t, changed.Service, "after a role-model change", authorization.LookupRequest{
...
func changedDecisionRoleModel() authorization.RoleModel {
