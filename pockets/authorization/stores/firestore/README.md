...
already-verified probe to avoid repeating Admin API traffic. Explicitly required live
...
This adapter serves authoritative Firestore reads and writes directly. It does
not implement a maintained TupleCache source. TupleCache currently supports the
Turso and PostgreSQL authoritative adapters; passing a Redis backend with this
adapter is rejected because no matching source is available.

The previous generation-cache options and metadata maintenance have been retired.
Stop old cache-enabled readers and writers before adopting this version. Existing
`iam_cache_invalidation/head` metadata is no longer read or updated; the host may
remove it after the old processes have stopped. Ordinary relationship, role,
guarded mutation and audit behavior remains available without cache setup.
