...
// Collection names mirror the domain tables. The additional subject claim
// implements the one-relation-per-exact-subject constraint. All collections
// are top-level, so manifest indexes have COLLECTION scope.
const (
	collectionRelationships = "iam_relationships"
	collectionSubjectClaims = "iam_relationship_subjects"
	collectionRoles         = "iam_roles"
	collectionAudit         = "iam_audit"
)
...
// the SQL families exactly — see SCHEMA.md and keys_test.go.
const keySeparator = "\x01"

// relationshipDocID addresses the complete six-field natural tuple. A duplicate
// Create collides on this document; no surrogate identity is stored.
...
// tupleSortKeys preserves the canonical six-component tuple order in two
// indexed values. The suffix starts with the separator so concrete subjects
// have a nonempty cursor PK and sort before usersets with the same prefix.
func tupleSortKeys(row relationshipDoc) (prefix, suffix string) {
	return strings.Join([]string{row.ResourceType, row.ResourceID, row.Relation, row.SubjectType, row.SubjectID}, keySeparator),
		keySeparator + row.SubjectRelation
}
