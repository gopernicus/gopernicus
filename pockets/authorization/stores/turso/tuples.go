...
const canonicalTupleKeyExpr = "('2' || char(1) || scope_kind || char(1) || resource_type || char(1) || resource_id || char(1) || relation || char(1) || subject_type || char(1) || subject_id || char(1) || subject_relation) COLLATE BINARY"
...
func newTupleStore(db *tursodb.DB, cfg config) *tupleStore { return &tupleStore{db: db, cfg: cfg} }

var _ tuples.Storer = (*tupleStore)(nil)

func (s *tupleStore) TupleCacheBinding() string { return s.cfg.tupleBinding }
func (s *tupleStore) table() string             { return "main.iam_tuples" }
func (s *tupleStore) reader(ctx context.Context) tursodb.Querier {
...
func (a *tupleArgs) bind(v any) string   { a.values = append(a.values, v); return "?" }
func (a *tupleArgs) ref(v string) string { return a.bind(v) }
func (a *tupleArgs) params() []any       { return a.values }
...
func (s *tupleStore) Contains(ctx context.Context, f tuples.Tuple) (bool, error) {
	...
	return found, tursodb.MapError(err)
}
...
func (s *tupleStore) ListTuples(ctx context.Context, q tuples.Query, req list.Request) (list.Page[tuples.Tuple], error) {
	...
	query := tursodb.ListQuery[tupleRow]{
	...
	page, err := tursodb.List(ctx, s.reader(ctx), query, req)
	if err != nil {
		return empty, err
	}
...
func (s *tupleStore) ReadTupleSnapshot(ctx context.Context, fn func(context.Context, tuples.Reader) error) (err error) {
	...
	reader := *s
	var owned *tursodb.Tx
	if reader.readQuerier == nil {
		if ambient, ok := tursodb.TxFromContext(ctx); ok {
			// The connector begins ambient write transactions with BEGIN IMMEDIATE.
			...
			defer func() {
				if rollbackErr := owned.Rollback(); err != nil && rollbackErr != nil && !errors.Is(rollbackErr, sql.ErrConnDone) {
