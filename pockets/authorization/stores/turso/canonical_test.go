...
func emptyCanonicalFixture(t testing.TB) *tursodb.DB {
	t.Helper()
	db, err := tursodb.Open(t.Context(), tursodb.Config{URL: "file:" + filepath.Join(t.TempDir(), "canonical.sqlite"), MaxOpenConns: 4})
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { db.Close() })
	if _, err := db.Exec(t.Context(), "PRAGMA journal_mode=WAL"); err != nil {
		t.Fatal(err)
	}
	return db
}

func canonicalFixture(t testing.TB, cache bool) *tursodb.DB {
	t.Helper()
...
func TestCanonicalSubjectMutationReadsOnlyItsTarget(t *testing.T) {
	...
	if _, err := db.Exec(t.Context(), "INSERT INTO main.iam_tuples ("+tupleColumns+") VALUES (1,'','','viewer','user',?,'')", strings.Repeat("x", 257)); err != nil {
		t.Fatal(err)
	}
