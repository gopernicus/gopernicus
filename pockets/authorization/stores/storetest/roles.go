...
func assign(t *testing.T, s roles.Storer, subjectType, subjectID, roleName, resourceType, resourceID string) {
	t.Helper()
	if err := s.Assign(context.Background(), roles.Assignment{
...
func hasExact(t *testing.T, s roles.Storer, subjectType, subjectID, roleName, resourceType, resourceID string) bool {
	t.Helper()
...
func runRoles(t *testing.T, newRepos func(t *testing.T) authorization.Repositories) {
	...
	t.Run("AssignIdempotent", func(t *testing.T) {
		...
		first, _ := s.ListBySubject(ctx, "user", "u1", listing.Request{})

		assign(t, s, "user", "u1", "editor", "", "")
		again, _ := s.ListBySubject(ctx, "user", "u1", listing.Request{})
		if len(again.Items) != 1 {
			t.Fatalf("duplicate global assign must keep one row, got %d", len(again.Items))
		}
		if again.Items[0] != first.Items[0] {
			t.Fatalf("duplicate assign changed the assignment: first=%+v again=%+v", first.Items, again.Items)
		}
	})
	...
	t.Run("DistinctAssignmentsCoexist", func(t *testing.T) {
		...
		page, _ := s.ListBySubject(ctx, "user", "u1", listing.Request{})
		if len(page.Items) != 3 {
	...
	t.Run("ListPagination", func(t *testing.T) {
		...
		empty, err := s.ListByResource(ctx, "doc", "absent", listing.Request{})
		if err != nil {
	...
	t.Run("RejectsUnknownOrderField", func(t *testing.T) {
		...
		// An order field outside role.OrderFields (role_key only) is rejected
		// with ErrInvalidInput identically across every backend.
		if _, err := s.ListBySubject(ctx, "user", "u1", listing.Request{Order: listing.NewOrder("role", listing.ASC)}); !errors.Is(err, sdk.ErrInvalidInput) {
	...
	t.Run("EffectiveEnumerationAgreesWithHasRole", func(t *testing.T) {
		...
		for id := range want {
			if ok, err := svc.Roles.HasRole(ctx, authmodel.PrincipalRef{Type: "user", ID: id}, "auditor", "doc", "d1"); err != nil || !ok {
				t.Fatalf("HasRole(%s)=%v,%v; enumeration and decision must agree", id, ok, err)
			}
		}
		if ok, _ := svc.Roles.HasRole(ctx, authmodel.PrincipalRef{Type: "user", ID: "u4"}, "auditor", "doc", "d1"); ok {
			t.Fatalf("HasRole must deny u4 on doc/d1 just as enumeration omits it")
		}
	})

	t.Run("ScopedRevokeGlobalRoleRemains", func(t *testing.T) {
		// Removing a scoped assignment leaves the global grant effective in
		// both permission checks and effective enumeration.
		repos := newRepos(t)
		s := repos.Roles
		svc := newServiceFor(t, repos)
		...
		// The global grant remains, matching the mutation result
		// same_role_grant_remains annotation.
		sameRoleGrantRemains, err := svc.Roles.HasRole(ctx, authmodel.PrincipalRef{Type: "user", ID: "u1"}, "editor", "doc", "d1")
		if err != nil {
	...
	t.Run("EffectivePagination", func(t *testing.T) {
		...
		assertEffectiveCoverage(t, func(req listing.Request) (listing.Page[roles.EffectiveGrant], error) {
		...
		empty, err := fresh.ListEffectiveByResource(ctx, "doc", "absent", listing.Request{})
		if err != nil {
	...
	t.Run("GlobalFallback", func(t *testing.T) {
		...
		if ok, err := svc.Roles.HasRole(ctx, authmodel.PrincipalRef{Type: "user", ID: "u1"}, "editor", "doc", "d1"); err != nil || !ok {
		...
		if ok, _ := svc.Roles.HasRole(ctx, authmodel.PrincipalRef{Type: "user", ID: "u2"}, "viewer", "doc", "dB"); ok {
			t.Fatalf("scoped grant must not satisfy a different scope")
		}
		if ok, err := svc.Roles.HasRole(ctx, authmodel.PrincipalRef{Type: "user", ID: "nobody"}, "editor", "doc", "d1"); err != nil || ok {
...
func effectiveProvenance(t *testing.T, s roles.Storer, resourceType, resourceID string) map[string]string {
	t.Helper()
	out := map[string]string{}
	cursor := ""
	for pages := 0; pages < 100; pages++ {
		page, err := s.ListEffectiveByResource(context.Background(), resourceType, resourceID, listing.Request{Limit: 2, Cursor: cursor})
		if err != nil {
...
func assertEffectiveCoverage(t *testing.T, list func(listing.Request) (listing.Page[roles.EffectiveGrant], error), want int) {
	t.Helper()
