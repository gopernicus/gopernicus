...
func ctUserset(rt, rid, relation, st, sid, subjRel string) relationships.CreateRelationship {
	c := ct(rt, rid, relation, st, sid)
	c.SubjectRelation = subjRel
	return c
}

func mustView(t *testing.T, svc authorization.Components, subjectType, subjectID, docID string, want bool) {
	t.Helper()
	res, err := svc.Decisions.Check(context.Background(), authmodel.CheckRequest{
		Principal:  authmodel.PrincipalRef{Type: subjectType, ID: subjectID},
		Permission: "view",
...
func runAdversarial(t *testing.T, newRepos func(t *testing.T) authorization.Repositories) {
	...
	t.Run("MissingUsersetRelationRejected", func(t *testing.T) {
		...
		if err := svc.Relationships.ValidateRelationships([]relationships.CreateRelationship{
	...
	t.Run("PlatformAdminIsNotMagic", func(t *testing.T) {
		for _, subjType := range []string{"user", "service_account"} {
			...
			res, err := svc.Decisions.LookupAllResourceIDs(ctx, authmodel.PrincipalRef{Type: subjType, ID: "admin1"}, "view", "doc")
			if err != nil {
				t.Fatalf("%s admin LookupAllResourceIDs: %v", subjType, err)
			}
			if res.IDs == nil {
			...
			nonAdmin, err := svc.Decisions.LookupAllResourceIDs(ctx, authmodel.PrincipalRef{Type: "user", ID: "u9"}, "view", "doc")
			if err != nil {
				t.Fatalf("non-admin LookupAllResourceIDs: %v", err)
			}
