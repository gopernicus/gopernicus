...
func transact(tx transaction.Transactor, fn func(ctx context.Context) error) error {
	return tx.Transact(context.Background(), fn)
}

// createAt is mustCreate with an explicit context (ambient or outside).
func createAt(t *testing.T, ctx context.Context, s relationships.Storer, tuples ...relationships.CreateRelationship) {
	t.Helper()
...
func existsAt(t *testing.T, ctx context.Context, s relationships.Storer, rt, rid, relation, st, sid string) bool {
	t.Helper()
...
func targetIDsAt(t *testing.T, ctx context.Context, s relationships.Storer, rt, rid, relation string) []string {
	t.Helper()
...
func setTargetsAt(ctx context.Context, s relationships.Storer, ids ...string) error {
...
func specDeletesJoinTransaction(t *testing.T, newRepos func(t *testing.T) (authorization.Repositories, transaction.Transactor)) {
	type deleteCase struct {
		...
		remove  func(ctx context.Context, s relationships.Storer) error
		present func(ctx context.Context, s relationships.Storer) bool
	}
	cases := []deleteCase{
		{
			name: "DeleteRelationshipTarget",
			seed: []relationships.CreateRelationship{ctUserset("doc", "d1", "viewer", "group", "eng", "member")},
			remove: func(ctx context.Context, s relationships.Storer) error {
				return s.DeleteRelationshipTarget(ctx, "doc", "d1", "viewer", relationships.SubjectRef{Type: "group", ID: "eng", Relation: "member"})
			},
			present: func(ctx context.Context, s relationships.Storer) bool {
		...
		},
		{
			...
			seed: []relationships.CreateRelationship{ct("doc", "d1", "owner", "user", "u1")},
			remove: func(ctx context.Context, s relationships.Storer) error {
		...
		},
		{
			...
			seed: []relationships.CreateRelationship{ct("doc", "d1", "owner", "user", "u1"), ct("doc", "d1", "viewer", "user", "u2")},
			remove: func(ctx context.Context, s relationships.Storer) error {
		...
		},
		{
			...
			seed: []relationships.CreateRelationship{ct("doc", "d1", "owner", "user", "u1"), ct("doc", "d2", "owner", "user", "u1")},
			remove: func(ctx context.Context, s relationships.Storer) error {
...
func walkSubject(t *testing.T, ctx context.Context, s relationships.Storer, st, sid string) ([]string, int64) {
	t.Helper()
	var ids []string
	var total int64
	cursor := ""
	for page := 0; page < 10; page++ {
		p, err := s.ListRelationshipsBySubject(ctx, st, sid, relationships.SubjectRelationshipFilter{}, list.Request{Limit: 1, Cursor: cursor, WithCount: page == 0})
		if err != nil {
...
func snapshotRelationships(t *testing.T, ctx context.Context, s relationships.Storer) relationshipSnapshot {
	t.Helper()
	...
	byRes, err := s.ListRelationshipsByResource(ctx, "doc", "d2", relationships.ResourceRelationshipFilter{}, list.Request{WithCount: true})
	if err != nil {
