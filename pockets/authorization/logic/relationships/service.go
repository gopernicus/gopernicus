// Package relationships owns relationship tuples, schema compilation, read ports,
// ReBAC evaluation and a separately held trusted RelationshipWriter.
...
// Option configures relationship evaluation before the service is constructed.
...
// WithLimits replaces the evaluation budget. Zero fields take defaults; negative
// fields are invalid. The resolved budget bounds traversal and result sets.
func WithLimits(limits authmodel.EvaluationLimits) Option {
	return func(c *serviceConfig) { c.limits = limits }
}

// PermissionReader supplies model-scoped relationship reads to the single
// permission walk, including inside a guarded transaction.

type ModelPermissionReader interface {
	ForModel(ReadModel) PermissionReader
}

// Service evaluates ReBAC permissions and reads relationship facts using a
// Storer and an immutable
...
func newService(store Storer, schema Schema, cfg serviceConfig) (*Service, error) {
	if isNilReader(store) {
		return nil, fmt.Errorf("authorization: relationship store is required: %w", sdk.ErrInvalidInput)
	}
	...
	model := compiled.readModel()
	reader := store.ForModel(model)
	if isNilReader(reader) {
		return nil, fmt.Errorf("authorization: nil model-scoped reader: %w", sdk.ErrInvalidInput)
	}
	return &Service{
		store:     store,
		reader:    reader,
...
// Limits returns the immutable resolved budget shared with composed services.
func (s *Service) Limits() authmodel.EvaluationLimits { return s.limits }
...
func (s *Service) EvaluateWith(ctx context.Context, reader ModelPermissionReader, req authmodel.CheckRequest) (authmodel.CheckResult, error) {
	if isNilReader(reader) {
		return authmodel.CheckResult{}, fmt.Errorf("authorization evaluate: nil permission reader: %w", sdk.ErrInvalidInput)
	}
	scoped := reader.ForModel(s.readModel)
	if isNilReader(scoped) {
		return authmodel.CheckResult{}, fmt.Errorf("authorization: nil model-scoped reader: %w", sdk.ErrInvalidInput)
	}
	return s.check(ctx, req, newBudget(s.limits, newMemoReader(scoped)))
}
...
func (s *Service) checkPermission(ctx context.Context, req authmodel.CheckRequest, checks []PermissionCheck, depth int, b *budget, stack map[stateKey]bool) (authmodel.CheckResult, error) {
	...
	for _, check := range checks {
		...
		if check.Through != "" {
			...
			b.record(authmodel.ExplainStep{
				ResourceType: req.Resource.Type, ResourceID: req.Resource.ID,
				Permission: req.Permission, Relation: check.Through,
				Kind: authmodel.ExplainKindThrough, Depth: depth, Outcome: outcomeReason(result.Allowed),
			})
			if result.Allowed {
				return result, nil
			}
			...
			b.record(authmodel.ExplainStep{
				...
				Kind: authmodel.ExplainKindDirect, Depth: depth, Outcome: outcomeReason(result.Allowed),
			})
			if result.Allowed {
				return result, nil
			}
	...
	return authmodel.CheckResult{Allowed: false, ReasonCode: authmodel.ReasonDenied, Reason: "no matching rule"}, nil
}
...
func (s *Service) checkDirectRelation(ctx context.Context, req authmodel.CheckRequest, relation string, b *budget) (authmodel.CheckResult, error) {
	if err := ctx.Err(); err != nil {
		return authmodel.CheckResult{}, err
	}
	...
	if allowed {
		return authmodel.CheckResult{Allowed: true, ReasonCode: authmodel.ReasonGranted, Reason: fmt.Sprintf("direct:%s", relation)}, nil
	}
	return authmodel.CheckResult{Allowed: false, ReasonCode: authmodel.ReasonDenied}, nil
}

func (s *Service) checkThrough(ctx context.Context, req authmodel.CheckRequest, check PermissionCheck, depth int, b *budget, stack map[stateKey]bool) (authmodel.CheckResult, error) {
	if err := ctx.Err(); err != nil {
		return authmodel.CheckResult{}, err
	}
...
func (s *Service) checkBatchOptimized(ctx context.Context, reqs []authmodel.CheckRequest) ([]authmodel.CheckResult, error) {
	...
	steps := 1 // the root frame, matching ordinary Check
...
// CreateRelationships validates the whole batch against the schema and persists
// its exact tuples. The caller's slice is not mutated.
