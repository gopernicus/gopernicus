...
func TestReasonCodeStableGrantedDenied(t *testing.T) {
	...
	none, err := svc.Check(context.Background(), authmodel.CheckRequest{
		Principal: authmodel.PrincipalRef{Type: "user", ID: "u1"}, Permission: "nonexistent", Resource: authmodel.Resource{Type: "post", ID: "p1"},
	})
	if err != nil || none.Allowed || none.ReasonCode != authmodel.ReasonDenied {
...
func TestExplainCannotChangeDecision(t *testing.T) {
	...
	store.tuples = append(store.tuples,
		...
		CreateRelationship{ResourceType: "org", ResourceID: "o1", Relation: "admin", SubjectType: "user", SubjectID: "u2"},
	...
	cases := []struct {
		...
		{"direct grant", authmodel.CheckRequest{Principal: authmodel.PrincipalRef{Type: "user", ID: "u1"}, Permission: "delete", Resource: authmodel.Resource{Type: "post", ID: "p1"}}},
		{"through grant", authmodel.CheckRequest{Principal: authmodel.PrincipalRef{Type: "user", ID: "u2"}, Permission: "view", Resource: authmodel.Resource{Type: "post", ID: "p2"}}},
		{"deny", authmodel.CheckRequest{Principal: authmodel.PrincipalRef{Type: "user", ID: "u3"}, Permission: "view", Resource: authmodel.Resource{Type: "post", ID: "p2"}}},
	}
...
func TestExplainTraceRecordsCoarseSteps(t *testing.T) {
	...
	res, expl, err := svc.CheckExplain(context.Background(), authmodel.CheckRequest{
	...
	if expl.Decision != authmodel.ReasonGranted {
	...
	for _, st := range expl.Steps {
		if st.Outcome != authmodel.ReasonGranted && st.Outcome != authmodel.ReasonDenied {
			t.Fatalf("step Outcome %q is not a coarse decision code", st.Outcome)
		}
		if st.Kind == authmodel.ExplainKindThrough && st.Relation == "org" && st.Outcome == authmodel.ReasonGranted {
			sawThroughGrant = true
		}
		if st.Kind == authmodel.ExplainKindDirect && st.Relation == "admin" && st.Outcome == authmodel.ReasonGranted {
			sawDirectGrant = true
		}
	}
...
func TestExplainFailsWithSameLimitClass(t *testing.T) {
	...
	svc, err := newService(store, testSchema(), serviceConfig{limits: authmodel.EvaluationLimits{MaxGraphStates: 1}})
	if err != nil {
	...
	req := authmodel.CheckRequest{Principal: authmodel.PrincipalRef{Type: "user", ID: "u1"}, Permission: "view", Resource: authmodel.Resource{Type: "post", ID: "p1"}}

	_, perr := svc.Check(context.Background(), req)
	_, _, eerr := svc.CheckExplain(context.Background(), req)
	if !errors.Is(perr, authmodel.ErrEvaluationLimit) {
		t.Fatalf("Check: want ErrEvaluationLimit, got %v", perr)
	}
	if !errors.Is(eerr, authmodel.ErrEvaluationLimit) {
