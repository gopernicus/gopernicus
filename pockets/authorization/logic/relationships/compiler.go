...
// every structural problem. Names follow relationship.ValidateRefField, the
...
func Compile(schema Schema) (*CompiledSchema, error) {
	...
	for rtName, rt := range schema.ResourceTypes {
		if err := authmodel.ValidateRefField("resource type name", rtName); err != nil {
...
func compileRelations(schema Schema, rtName string, rt ResourceTypeDef, navRefs map[[2]string]bool) []string {
	var errs []string
	for relName, rel := range rt.Relations {
		if err := authmodel.ValidateRefField("relation name", relName); err != nil {
		...
		for _, sub := range rel.AllowedSubjects {
			if err := authmodel.ValidateRefField("allowed subject type", sub.Type); err != nil {
				errs = append(errs, fmt.Sprintf("%s.%s: %s", rtName, relName, err))
				continue
			}
			if sub.Relation != "" {
				if err := authmodel.ValidateRefField("allowed subject relation", sub.Relation); err != nil {
					errs = append(errs, fmt.Sprintf("%s.%s: %s", rtName, relName, err))
				}
			}
			if seen[sub] {
...
func compilePermissions(schema Schema, rtName string, rt ResourceTypeDef) []string {
	var errs []string
	for permName, rule := range rt.Permissions {
		if err := authmodel.ValidateRefField("permission name", permName); err != nil {
			errs = append(errs, fmt.Sprintf("%s: %s", rtName, err))
		}
		if len(rule.AnyOf) == 0 {
		...
		for _, chk := range rule.AnyOf {
			...
			for _, ref := range []struct{ name, value string }{
				{"direct relation", chk.Relation},
				{"through relation", chk.Through},
				{"through permission", chk.Permission},
			} {
				if ref.value != "" {
					if err := authmodel.ValidateRefField(ref.name, ref.value); err != nil {
						errs = append(errs, fmt.Sprintf("%s.%s: %s", rtName, permName, err))
					}
				}
			}
			...
			case hasDirect:
				if chk.Permission != "" {
					errs = append(errs, fmt.Sprintf("%s.%s: direct relation %q must not name a through permission %q", rtName, permName, chk.Relation, chk.Permission))
				}
