...
func TestLookupRequestValidate(t *testing.T) {
	valid := authmodel.LookupRequest{
		Principal:    authmodel.PrincipalRef{Type: "user", ID: "u1"},
		Permission:   "view",
		ResourceType: "project",
	}

	for name, mutate := range map[string]func(authmodel.LookupRequest) authmodel.LookupRequest{
		"zero limit is the budget ceiling": func(r authmodel.LookupRequest) authmodel.LookupRequest { return r },
		"positive limit":                   func(r authmodel.LookupRequest) authmodel.LookupRequest { r.Limit = 5; return r },
	} {
	...
	for name, mutate := range map[string]func(authmodel.LookupRequest) authmodel.LookupRequest{
		"empty principal type": func(r authmodel.LookupRequest) authmodel.LookupRequest { r.Principal.Type = ""; return r },
		"empty principal id":   func(r authmodel.LookupRequest) authmodel.LookupRequest { r.Principal.ID = ""; return r },
		"empty permission":     func(r authmodel.LookupRequest) authmodel.LookupRequest { r.Permission = ""; return r },
		"empty resource type":  func(r authmodel.LookupRequest) authmodel.LookupRequest { r.ResourceType = ""; return r },
	} {
		t.Run(name, func(t *testing.T) {
			if err := mutate(valid).Validate(); err == nil {
...
func TestNewSchemaComposesAndCopies(t *testing.T) {
	tenant := []ResourceSchema{{
		...
		Def: ResourceTypeDef{
			...
			Permissions: map[string]Expression{"delete": AnyOf(Direct("owner"))},
		},
	}}
	project := []ResourceSchema{{
		...
		Def: ResourceTypeDef{
			...
			Permissions: map[string]Expression{"view": AnyOf(Direct("member"))},
		},
	}}
