...
func (s *stubReader) GetRelationTargets(ctx context.Context, resourceType, resourceID, relation string) ([]RelationTarget, error) {
	s.targetsCalls++
...
func (s *stubReader) RelationTargetsFor(ctx context.Context, resourceType string, resourceIDs []string, relation string) (map[string][]RelationTarget, error) {
	s.setTargetsCalls++
...
func chainSchema() Model {
	return NewSchema([]ResourceSchema{
		...
		{Name: "org", Def: ResourceTypeDef{
			...
			Permissions: map[string]Expression{"manage": AnyOf(Direct("admin"), Through("tenant", "manage"))},
		}},
		{Name: "post", Def: ResourceTypeDef{
			...
			Permissions: map[string]Expression{"view": AnyOf(Direct("owner"), Through("org", "manage"))},
		}},
	})
}

func newLimitedService(t *testing.T, store Storer, schema Model, limits authmodel.EvaluationLimits) *Service {
	t.Helper()
...
func containerTuples(n int) []CreateRelationship {
	out := []CreateRelationship{tuple("org", "o1", "admin", "user", "u1", "")}
	for i := 0; i < n; i++ {
...
func assertBatchParity(t *testing.T, svc *Service, reqs []authmodel.CheckRequest) {
	t.Helper()
	ctx := context.Background()

	want := make([]authmodel.CheckResult, len(reqs))
	var wantErr error
...
func TestCheckBatchThroughParityWithSequentialCheck(t *testing.T) {
	...
	u2 := authmodel.PrincipalRef{Type: "user", ID: "u2"}
	u3 := authmodel.PrincipalRef{Type: "user", ID: "u3"}

	assertBatchParity(t, svc, []authmodel.CheckRequest{
		{Principal: u1, Permission: "view", Resource: authmodel.Resource{Type: "post", ID: "p1"}},
		{Principal: u2, Permission: "view", Resource: authmodel.Resource{Type: "post", ID: "p1"}},
		{Principal: u1, Permission: "view", Resource: authmodel.Resource{Type: "post", ID: "p2"}},
		{Principal: u1, Permission: "view", Resource: authmodel.Resource{Type: "post", ID: "p3"}},
		{Principal: u1, Permission: "view", Resource: authmodel.Resource{Type: "post", ID: "p4"}},
		{Principal: u3, Permission: "delete", Resource: authmodel.Resource{Type: "post", ID: "p5"}},
		{Principal: u3, Permission: "view", Resource: authmodel.Resource{Type: "post", ID: "p5"}},
		{Principal: u1, Permission: "manage", Resource: authmodel.Resource{Type: "org", ID: "o1"}},
		{Principal: u2, Permission: "manage", Resource: authmodel.Resource{Type: "org", ID: "o1"}},
	})
}
...
func TestCheckBatchLimitParityRelationTargets(t *testing.T) {
	...
	svc := newLimitedService(t, store, testSchema(), authmodel.EvaluationLimits{MaxRelationTargets: 2})

	u1 := authmodel.PrincipalRef{Type: "user", ID: "u1"}
	reqs := []authmodel.CheckRequest{
		{Principal: u1, Permission: "view", Resource: authmodel.Resource{Type: "post", ID: "ok"}},
		{Principal: u1, Permission: "view", Resource: authmodel.Resource{Type: "post", ID: "wide"}},
		{Principal: u1, Permission: "view", Resource: authmodel.Resource{Type: "post", ID: "ok"}},
	}
	if _, err := svc.Check(context.Background(), reqs[1]); !errors.Is(err, authmodel.ErrEvaluationLimit) {
...
func TestCheckBatchLimitParityGraphStates(t *testing.T) {
	...
	svc := newLimitedService(t, store, testSchema(), authmodel.EvaluationLimits{MaxGraphStates: 1})

	u1 := authmodel.PrincipalRef{Type: "user", ID: "u1"}
	reqs := []authmodel.CheckRequest{
		{Principal: u1, Permission: "delete", Resource: authmodel.Resource{Type: "post", ID: "p0"}},
		{Principal: u1, Permission: "view", Resource: authmodel.Resource{Type: "post", ID: "p1"}},
	}
	if _, err := svc.Check(context.Background(), reqs[1]); !errors.Is(err, authmodel.ErrEvaluationLimit) {
...
func TestCheckBatchLimitParityThroughDepth(t *testing.T) {
	...
	reqs := []authmodel.CheckRequest{
		{Principal: u1, Permission: "manage", Resource: authmodel.Resource{Type: "org", ID: "o1"}},
		{Principal: u1, Permission: "view", Resource: authmodel.Resource{Type: "post", ID: "p0"}},
		{Principal: u1, Permission: "view", Resource: authmodel.Resource{Type: "post", ID: "p1"}},
	}
	if _, err := svc.Check(context.Background(), reqs[1]); !errors.Is(err, authmodel.ErrEvaluationLimit) {
...
func TestCheckBatchIsIndependentOfLookupBudget(t *testing.T) {
	...
	svc := newLimitedService(t, store, testSchema(), authmodel.EvaluationLimits{MaxLookupResults: 2})

	allowed, err := svc.FilterAuthorized(context.Background(),
		authmodel.PrincipalRef{Type: "user", ID: "u1"}, "view", "post", []string{"p0", "p1", "p2"})
	if err != nil {
...
func TestCheckBatchSharesContainerReads(t *testing.T) {
	...
	svc := newLimitedService(t, store, chainSchema(), authmodel.EvaluationLimits{})

	reqs := make([]authmodel.CheckRequest, n)
	for i := range reqs {
		reqs[i] = authmodel.CheckRequest{
			Principal:  authmodel.PrincipalRef{Type: "user", ID: "u1"},
			Permission: "view",
			Resource:   authmodel.Resource{Type: "post", ID: fmt.Sprintf("p%d", i)},
		}
	}
	...
	if store.setTargetsCalls != 1 || store.setDirectCalls != 2 {
		t.Fatalf("batch reads=%d targets/%d direct, want 1/2", store.setTargetsCalls, store.setDirectCalls)
	}
}
...
func TestMemoReaderReturnsIsolatedCopies(t *testing.T) {
	inner := &stubReader{targets: []RelationTarget{{Type: "org", ID: "o1"}}}
...
func TestMemoReaderDoesNotCacheErrors(t *testing.T) {
	inner := &stubReader{
		targets:   []RelationTarget{{Type: "org", ID: "o1"}},
		allowed:   true,
		failFirst: true,
	}
	m := newMemoReader(inner)
...
func TestMemoReaderSkipsCacheAfterCancellation(t *testing.T) {
	inner := &stubReader{targets: []RelationTarget{{Type: "org", ID: "o1"}}, allowed: true}
	m := newMemoReader(inner)
	ctx, cancel := context.WithCancel(context.Background())
	cancel()

	if _, err := m.GetRelationTargets(ctx, "post", "p1", "org"); !errors.Is(err, context.Canceled) {
		t.Fatalf("GetRelationTargets: %v", err)
	}
	if _, err := m.CheckRelationWithGroupExpansion(ctx, "org", "o1", "admin", "user", "u1", 10); !errors.Is(err, context.Canceled) {
	...
	if inner.targetsCalls != 1 || inner.directCalls != 1 {
		t.Fatalf("inner calls = (%d targets, %d direct), want 1 and 1 (canceled reads must not reach storage)",
...
func benchmarkCheckBatchThrough(b *testing.B, schema Model) {
	const n = 500

	store := &fakeStore{tuples: containerTuples(n)}
	svc, err := newService(store, schema, serviceConfig{})
	if err != nil {
