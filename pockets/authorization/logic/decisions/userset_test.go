...
func tuple(rt, rid, relation, st, sid, subjRel string) relationships.CreateRelationship {
	return relationships.CreateRelationship{ResourceType: rt, ResourceID: rid, Relation: relation, SubjectType: st, SubjectID: sid, SubjectRelation: subjRel}
}

func usersetView(t *testing.T, svc authorization.Components, st, sid, docID string, want bool) {
	t.Helper()
	res, err := svc.Decisions.Check(context.Background(), authmodel.CheckRequest{
		Principal:  authmodel.PrincipalRef{Type: st, ID: sid},
		Permission: "view",
