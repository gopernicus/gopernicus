...
func (r *recordingReader) CheckRelationWithGroupExpansion(ctx context.Context, rt, rid, rel, st, sid string, max int) (bool, error) {
	...
	owner.calls = append(owner.calls, fmt.Sprintf("check %s:%s#%s@%s:%s max=%d", rt, rid, rel, st, sid, max))
	return r.inner.CheckRelationWithGroupExpansion(ctx, rt, rid, rel, st, sid, max)
}

func (r *recordingReader) GetRelationTargets(ctx context.Context, rt, rid, rel string) ([]RelationTarget, error) {
	owner := r
	if r.owner != nil {
		owner = r.owner
	}
	owner.calls = append(owner.calls, fmt.Sprintf("targets %s:%s#%s", rt, rid, rel))
...
func tuple(rt, rid, rel, st, sid, srel string) CreateRelationship {
	return CreateRelationship{ResourceType: rt, ResourceID: rid, Relation: rel, SubjectType: st, SubjectID: sid, SubjectRelation: srel}
}

func TestEvaluateWithMatchesCheckOverStore(t *testing.T) {
	graph := &graphStore{}
	graph.tuples = []CreateRelationship{
		// direct
	...
	cases := []struct {
		...
		{name: "depth budget", limits: authmodel.EvaluationLimits{MaxThroughDepth: 1}, req: check("bob", "space", "leaf"), wantErr: authmodel.ErrEvaluationLimit},
		{name: "graph-state budget", limits: authmodel.EvaluationLimits{MaxGraphStates: 2}, req: check("carol", "space", "d"), wantErr: authmodel.ErrEvaluationLimit},
		...
		{name: "expansion budget", limits: authmodel.EvaluationLimits{MaxGraphStates: 2}, req: check("alice", "space", "grouped"), wantErr: authmodel.ErrEvaluationLimit},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			overStore, err := newService(graph, evaluateWithSchema(), serviceConfig{limits: tc.limits})
			if err != nil {
			...
			// become a deny.
			overEmpty, err := newService(&fakeStore{}, evaluateWithSchema(), serviceConfig{limits: tc.limits})
			if err != nil {
...
func TestEvaluateWithReadsOnlyTheReader(t *testing.T) {
	graph := &graphStore{}
	graph.tuples = []CreateRelationship{tuple("space", "s", "viewer", "user", "alice", "")}
	svc, err := newService(&fakeStore{}, evaluateWithSchema(), serviceConfig{})
	if err != nil {
		t.Fatal(err)
	}
	...
	res, err = svc.EvaluateWith(context.Background(), modelStoreView{graph}, check("alice", "space", "s"))
	if err != nil || !res.Allowed {
...
func TestEvaluateWithRejectsNilReaderAndInvalidRequest(t *testing.T) {
	...
	if _, err := svc.EvaluateWith(context.Background(), modelStoreView{&graphStore{}}, authmodel.CheckRequest{}); !errors.Is(err, sdk.ErrInvalidInput) {
		t.Fatalf("empty request: want sdk.ErrInvalidInput, got %v", err)
	}
}

func check(user, rt, rid string) authmodel.CheckRequest {
	return authmodel.CheckRequest{Principal: authmodel.PrincipalRef{Type: "user", ID: user}, Permission: "view", Resource: authmodel.Resource{Type: rt, ID: rid}}
}

func (g *graphStore) ForModel(model ReadModel) Reader {
	return &graphStore{fakeStore: fakeStore{model: &model, source: &g.fakeStore}}
}

type modelStoreView struct{ Storer }

func (v modelStoreView) ForModel(model ReadModel) PermissionReader {
	return v.Storer.ForModel(model)
}

func (r *recordingReader) ForModel(model ReadModel) PermissionReader {
	return &recordingReader{inner: r.inner.(Storer).ForModel(model), owner: r}
}

func (g *graphStore) CheckBatchDirect(ctx context.Context, resourceType string, ids []string, relation, subjectType, subjectID string, max int) (map[string]bool, error) {
	matched, err := g.FilterRelation(ctx, resourceType, ids, relation, subjectType, subjectID, max)
	if err != nil {
		return nil, err
	}
