...
func TestNewServiceDoesNotRetainCallerSchema(t *testing.T) {
	...
	svc, err := newService(&fakeStore{}, schema, serviceConfig{})
	if err != nil {
	...
	schema.ResourceTypes["injected"] = ResourceTypeDef{
		...
		Permissions: map[string]Expression{"y": AnyOf(Direct("x"))},
	}
...
func TestServiceSchemaImmutableUnderConcurrentSourceMutation(t *testing.T) {
	...
	req := authmodel.CheckRequest{Principal: authmodel.PrincipalRef{Type: "user", ID: "u1"}, Permission: "view", Resource: authmodel.Resource{Type: "project", ID: "p1"}}
	for r := 0; r < 8; r++ {
		wg.Add(1)
		go func() {
			defer wg.Done()
