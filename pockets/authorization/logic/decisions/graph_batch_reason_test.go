...
func TestCheckBatchNamesActualGrantingRelation(t *testing.T) {
	store := &fakeStore{}
	svc, err := newService(store, docReadSchema(), serviceConfig{})
	if err != nil {
		t.Fatalf("NewService: %v", err)
	}
	store.tuples = append(store.tuples,
		CreateRelationship{ResourceType: "doc", ResourceID: "d1", Relation: "owner", SubjectType: "user", SubjectID: "u1"},
	)

	results, err := svc.CheckBatch(context.Background(), []authmodel.CheckRequest{
		{Principal: authmodel.PrincipalRef{Type: "user", ID: "u1"}, Permission: "read", Resource: authmodel.Resource{Type: "doc", ID: "d1"}},
		{Principal: authmodel.PrincipalRef{Type: "user", ID: "u1"}, Permission: "read", Resource: authmodel.Resource{Type: "doc", ID: "d2"}},
	})
	if err != nil {
		t.Fatalf("CheckBatch: %v", err)
	}
	if !results[0].Allowed || results[0].ReasonCode != authmodel.ReasonGranted {
	...
	if results[1].Allowed || results[1].ReasonCode != authmodel.ReasonDenied {
...
func TestValidateSchemaErrorsDeterministicOrder(t *testing.T) {
	...
	bad := NewSchema([]ResourceSchema{
		{Name: "alpha", Def: ResourceTypeDef{
			...
			Permissions: map[string]Expression{"read": AnyOf(Direct("missing_a"))},
		}},
		{Name: "beta", Def: ResourceTypeDef{
			Relations:   map[string]RelationDef{"b": {AllowedSubjects: []SubjectTypeRef{{Type: "user"}}}},
			Permissions: map[string]Expression{"read": AnyOf(Direct("missing_b"))},
		}},
	})
