...
var setPrincipal = authmodel.PrincipalRef{Type: "user", ID: "u1"}
		...
		for _, resourceType := range []string{"space", "doc"} {
			...
			for _, id := range ids {
				res, err := svc.Check(ctx, authmodel.CheckRequest{
					Principal: setPrincipal, Permission: "view", Resource: authmodel.Resource{Type: resourceType, ID: id},
				})
				if err != nil {
					wantErr = err
					break
...
func TestFilterAuthorizedGrantsThroughACandidatesOwnAncestor(t *testing.T) {
	store := &graphStore{}
	store.tuples = []CreateRelationship{
		tuple("space", "child", "parent", "space", "middle", ""),
		tuple("space", "middle", "parent", "space", "root", ""),
		tuple("space", "root", "viewer", "user", "u1", ""),
	}
	svc := newLimitedService(t, store, setHierarchySchema(), authmodel.EvaluationLimits{})
...
func TestFilterAuthorizedBudgetOverflowMatchesCheck(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:   "through depth",
			limits: authmodel.EvaluationLimits{MaxThroughDepth: 1},
			tuples: []CreateRelationship{
		...
		},
		{
			...
			limits: authmodel.EvaluationLimits{MaxRelationTargets: 1},
			tuples: []CreateRelationship{
		...
		},
		{
			...
			limits: authmodel.EvaluationLimits{MaxGraphStates: 2},
			tuples: []CreateRelationship{
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, id := range tc.ids {
				if _, err := svc.Check(ctx, authmodel.CheckRequest{
					Principal: setPrincipal, Permission: "view", Resource: authmodel.Resource{Type: "space", ID: id},
				}); !errors.Is(err, authmodel.ErrEvaluationLimit) {
					t.Fatalf("Check(%s) = %v, want ErrEvaluationLimit (the fixture must offend)", id, err)
				}
			}
			if _, err := svc.FilterAuthorized(ctx, setPrincipal, "view", "space", tc.ids); !errors.Is(err, authmodel.ErrEvaluationLimit) {
...
func TestFilterAuthorizedRejectsOverSizeBeforeAnyRead(t *testing.T) {
	store := &graphStore{}
	svc := newLimitedService(t, store, setHierarchySchema(), authmodel.EvaluationLimits{MaxBatchSize: 2})

	if _, err := svc.FilterAuthorized(context.Background(), setPrincipal, "view", "space", []string{"a", "b", "c"}); !errors.Is(err, authmodel.ErrEvaluationLimit) {
...
// Shared parent reads are reused, while each candidate keeps independent DFS
// depth and error semantics. The count makes the cost tradeoff explicit.
func TestFilterAuthorizedReusesSharedParentReads(t *testing.T) {
	for _, n := range []int{1, 50, 500} {
		store := &graphStore{}
		store.tuples = wideDeniedTuples(n)
		svc := newLimitedService(t, store, wideSchema(), authmodel.EvaluationLimits{})
...
// BenchmarkFilterAuthorizedDeniedCandidates reports actual reader calls for the
// shared DFS, including candidate work and reused ancestors.
	...
	for _, n := range []int{50, 300} {
		b.Run(fmt.Sprintf("n=%d", n), func(b *testing.B) {
			...
			for b.Loop() {
				store := &graphStore{}
				store.tuples = wideDeniedTuples(n)
				svc, err := newService(store, wideSchema(), serviceConfig{})
				if err != nil {
	...
	for _, n := range []int{50, 300} {
		b.Run(fmt.Sprintf("n=%d", n), func(b *testing.B) {
			...
			for i, id := range deniedCandidates(n) {
				reqs[i] = authmodel.CheckRequest{Principal: setPrincipal, Permission: "view", Resource: authmodel.Resource{Type: "post", ID: id}}
			}
			reads := 0
			for b.Loop() {
				store := &graphStore{}
...
func setHierarchySchema() Model {
	return NewSchema([]ResourceSchema{
		...
		{Name: "space", Def: ResourceTypeDef{
			...
			Permissions: map[string]Expression{"view": AnyOf(Direct("viewer"), Through("parent", "view"))},
		}},
	})
}
...
func wideSchema() Model {
	return NewSchema([]ResourceSchema{
		...
		{Name: "post", Def: ResourceTypeDef{
			...
			Permissions: map[string]Expression{"view": AnyOf(
...
func wideDeniedTuples(n int) []CreateRelationship {
	out := []CreateRelationship{
...
func randomSetSchema(rnd *rand.Rand) Model {
	orgChecks := atLeastOne(rnd, []Expression{Direct("admin"), Direct("member")})

	spaceChecks := atLeastOne(rnd, []Expression{Direct("viewer"), Direct("editor")})
	if rnd.Intn(2) == 0 {
	...
	docChecks := atLeastOne(rnd, []Expression{Direct("owner"), Direct("reader")})
	if rnd.Intn(3) > 0 {
	...
	return NewSchema([]ResourceSchema{
		...
		{Name: "org", Def: ResourceTypeDef{
			...
			Permissions: map[string]Expression{"view": AnyOf(orgChecks...)},
		}},
		{Name: "space", Def: ResourceTypeDef{
			Relations: map[string]RelationDef{
				"parent": {AllowedSubjects: []SubjectTypeRef{{Type: "space"}}},
			...
			Permissions: map[string]Expression{"view": AnyOf(spaceChecks...)},
		}},
		{Name: "doc", Def: ResourceTypeDef{
			...
			Permissions: map[string]Expression{"view": AnyOf(docChecks...)},
		}},
	})
}

// atLeastOne picks a non-empty random subset of options, preserving order.
func atLeastOne(rnd *rand.Rand, options []Expression) []Expression {
	var out []Expression
...
func randomSetTuples(rnd *rand.Rand) []CreateRelationship {
	var out []CreateRelationship
