...
func TestThroughCycleWalkExpandsConvergentSuffixOnce(t *testing.T) {
	...
	schema := Model{ResourceTypes: make(map[string]ResourceTypeDef)}
	for layer := 0; layer < layers; layer++ {
		for branch := 0; branch < width; branch++ {
			name := fmt.Sprintf("node%02d_%d", layer, branch)
			def := ResourceTypeDef{Relations: map[string]RelationDef{"reader": {AllowedSubjects: []SubjectTypeRef{{Type: "user"}}}}, Permissions: make(map[string]Expression)}
			checks := []Expression{Direct("reader")}
			if layer+1 < layers {
