...
// pair or an explicitly bound ad hoc graph primitive. Primitive kinds and
// relations are disjoint from named permissions. Distinct states are the MaxGraphStates dimension of the work budget and
...
//
//   - MaxEvaluationSteps — visited frames, rules and targets, including repeats.
//   - MaxGraphStates    — distinct named states and bound primitive roots.
...
type budget struct {
	...
	// through for THIS decision: the service's model-scoped reader on Check/CheckExplain/
	...
	trace           *explainTrace
	diagnosticFacts []tuples.Tuple
	resolveSlot     func(context.Context, ResourceSlot) (authmodel.Resource, bool, error)
}

func newBudget(limits authmodel.EvaluationLimits, reader PermissionReader) *budget {
...
func (b *budget) record(step authmodel.ExplainStep) {
...
func (b *budget) chargeState(resourceType, resourceID, permission string) error {
	return b.chargeKey(stateKey{resourceType: resourceType, resourceID: resourceID, permission: permission})
}

func (b *budget) chargePrimitiveState(resource authmodel.Resource, e Expression) error {
	k := stateKey{resourceType: resource.Type, resourceID: resource.ID, kind: 1, relation: e.Relation}
	if e.Through != "" {
		k.kind, k.relation, k.permission = 2, e.Through, e.Permission
	}
	return b.chargeKey(k)
}

func (b *budget) chargeKey(k stateKey) error {
...
// observed at that navigational hop). A hop wider than MaxRelationTargets
...
// chargeStep bounds actual traversal work, including repeat visits to an
// already-seen state. Read memoization never makes CPU or trace work free.
func (b *budget) chargeStep() error {
	if b.steps >= b.limits.MaxEvaluationSteps {
		return authmodel.ErrEvaluationLimit
	}
	b.steps++
	return nil
