		...
		for _, page := range []bool{false, true} {
			for _, outcome := range []string{"exhausted", "third attempt succeeds", "store conflict", "budget", "canceled", "deadline"} {
				t.Run(fmt.Sprintf("snapshot=%v/page=%v/%s", snapshot, page, outcome), func(t *testing.T) {
					...
					svc.store.(*graphFixture).reader = svc.reader
					source := &failingTupleSnapshot{Reader: svc.reader}
					if snapshot {
						svc.store.(*graphFixture).snapshot = source.ReadTupleSnapshot
					}
...
type failingTupleSnapshot struct {
	Reader
	calls     int
	fail      error
	nilReader bool
...
func (r *failingTupleSnapshot) ReadTupleSnapshot(ctx context.Context, fn func(context.Context, tuples.Reader) error) error {
	r.calls++
	var reader tuples.Reader = graphSnapshotFacts{Reader: r.Reader}
	if r.nilReader {
		reader = nil
	}
...
func TestLookupSnapshotFailureDiscardsResults(t *testing.T) {
	for _, nilReader := range []bool{false, true} {
		t.Run(fmt.Sprintf("nil=%v", nilReader), func(t *testing.T) {
			...
			svc.store.(*graphFixture).reader = svc.reader
			source := &failingTupleSnapshot{Reader: svc.reader, fail: failure, nilReader: nilReader}
			svc.store.(*graphFixture).snapshot = source.ReadTupleSnapshot
