...
func (s *Service) lookupResourcesOperation(ctx context.Context, principal authmodel.PrincipalRef, permission, resourceType string) (authmodel.LookupResult, error) {
	...
	if err := tuples.ValidateRefField("permission", permission); err != nil {
		return authmodel.LookupResult{}, err
	}
	if err := tuples.ValidateRefField("resource type", resourceType); err != nil {
		return authmodel.LookupResult{}, err
	}
	if !s.compiled.pureGraph(resourceType, permission) {
		return s.lookupGeneral(ctx, principal, permission, resourceType, "", 0)
	}
...
func (s *Service) lookupResourcesPageOperation(ctx context.Context, principal authmodel.PrincipalRef, permission, resourceType, after string, limit int) (authmodel.LookupResult, error) {
	...
	if !s.compiled.pureGraph(resourceType, permission) {
		return s.lookupGeneral(ctx, principal, permission, resourceType, after, limit)
	}
	if len(s.compiled.permissionChecks(resourceType, permission)) == 0 {
		return authmodel.LookupResult{IDs: []string{}}, nil
	}
