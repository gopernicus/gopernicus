...
type explicitCheckSource struct {
	CheckReader
	graphOnlyFacts
}
	...
	for name, operation := range operations {
		for _, failure := range []string{"completion", "canceled", "nil reader", "typed nil reader"} {
			t.Run(name+"/"+failure, func(t *testing.T) {
				...
				calls := 0
				svc.store.(*graphFixture).snapshot = func(ctx context.Context, fn func(context.Context, tuples.Reader) error) error {
					calls++
					switch failure {
					case "nil reader":
						return fn(ctx, nil)
					case "typed nil reader":
						var absent *graphSnapshotFacts
						return fn(ctx, absent)
					}
					if err := fn(ctx, graphSnapshotFacts{Reader: reader}); err != nil {
						return err
					}
					if failure == "canceled" {
						cancel()
...
func TestCheckSnapshotValidationAndReadFreePaths(t *testing.T) {
	...
	svc.store.(*graphFixture).snapshot = func(context.Context, func(context.Context, tuples.Reader) error) error {
...
func TestCheckSnapshotsDoNotNestOrReplaceExplicitReads(t *testing.T) {
	...
	svc.store.(*graphFixture).snapshot = func(ctx context.Context, fn func(context.Context, tuples.Reader) error) error {
		calls++
		return fn(ctx, graphSnapshotFacts{Reader: reader})
	}
