...
func TestEvaluationLimitsResolveDefaults(t *testing.T) {
	got, err := authmodel.EvaluationLimits{}.Resolve()
	if err != nil {
		t.Fatalf("Resolve zero: %v", err)
	}
	want := authmodel.EvaluationLimits{
		MaxEvaluationSteps: authmodel.DefaultMaxEvaluationSteps,
		MaxThroughDepth:    authmodel.DefaultMaxThroughDepth,
		MaxGraphStates:     authmodel.DefaultMaxGraphStates,
		MaxRelationTargets: authmodel.DefaultMaxRelationTargets,
		MaxBatchSize:       authmodel.DefaultMaxBatchSize,
		MaxLookupResults:   authmodel.DefaultMaxLookupResults,
		MaxFilterScan:      authmodel.DefaultMaxFilterScan,
	}
	if got != want {
...
func TestEvaluationLimitsResolveExplicitPreserved(t *testing.T) {
	got, err := authmodel.EvaluationLimits{MaxThroughDepth: 3, MaxBatchSize: 25}.Resolve()
	if err != nil {
	...
	if got.MaxGraphStates != authmodel.DefaultMaxGraphStates {
...
func TestEvaluationLimitsResolveNegativeRejected(t *testing.T) {
	cases := map[string]authmodel.EvaluationLimits{
		"MaxEvaluationSteps": {MaxEvaluationSteps: -1},
...
func TestEvaluationLimitsReserveOverflowSentinels(t *testing.T) {
	maxInt := int(^uint(0) >> 1)
	for _, limits := range []authmodel.EvaluationLimits{
		{MaxEvaluationSteps: maxInt}, {MaxThroughDepth: maxInt},
		{MaxGraphStates: maxInt}, {MaxLookupResults: maxInt},
	} {
		if _, err := limits.Resolve(); !errors.Is(err, authmodel.ErrInvalidLimits) {
			t.Fatalf("accepted overflowing fetch/depth sentinel %+v: %v", limits, err)
		}
	}
}
...
func TestNewServiceRejectsNegativeLimit(t *testing.T) {
	_, err := newService(&fakeStore{}, testSchema(), serviceConfig{limits: authmodel.EvaluationLimits{MaxThroughDepth: -1}})
	if !errors.Is(err, authmodel.ErrInvalidLimits) {
...
func throughFolderSchema() Model {
...
func TestBudgetThroughDepthExhaustionIsError(t *testing.T) {
	...
	shallow, err := newService(store, throughFolderSchema(), serviceConfig{limits: authmodel.EvaluationLimits{MaxThroughDepth: 1}})
	if err != nil {
		t.Fatalf("NewService shallow: %v", err)
	}
	_, err = shallow.Check(context.Background(), authmodel.CheckRequest{
		Principal: authmodel.PrincipalRef{Type: "user", ID: "u1"}, Permission: "view", Resource: authmodel.Resource{Type: "folder", ID: "f1"},
	})
	...
	// not a broken schema.
	deep, err := newService(store, throughFolderSchema(), serviceConfig{limits: authmodel.EvaluationLimits{MaxThroughDepth: 5}})
	if err != nil {
		t.Fatalf("NewService deep: %v", err)
	}
	res, err := deep.Check(context.Background(), authmodel.CheckRequest{
		Principal: authmodel.PrincipalRef{Type: "user", ID: "u1"}, Permission: "view", Resource: authmodel.Resource{Type: "folder", ID: "f1"},
	})
	if err != nil {
...
func TestBudgetDefaultDepthReachesGrant(t *testing.T) {
	...
	svc, err := newService(store, throughFolderSchema(), serviceConfig{})
	if err != nil {
