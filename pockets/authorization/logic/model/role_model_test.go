...
func TestCompiledRoleModelGrantorsAreOwnedCopies(t *testing.T) {
	compiled, err := CompileRoleModel(orgModel(), nil)
	if err != nil {
		t.Fatal(err)
	}
	original := compiled.Grantors("organization", "view")
	altered := compiled.Grantors("organization", "view")
	altered[0] = "injected"
	if !reflect.DeepEqual(compiled.Grantors("organization", "view"), original) {
		t.Fatal("caller changed the compiled role grants")
	}
}
