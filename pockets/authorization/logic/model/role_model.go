// Package model defines the shared principal, resource, decision and budget
// vocabulary used by authorization services. It also compiles immutable role
// permission models shared by ordinary decisions and transactional guards.
package model
...
// relationships.SchemaEncodingVersion for the relationship kind.
...
var (
	...
	//     malformed resource type / role / permission name, a duplicate role or grantor, an
...
// relationship engine (relationships.Service) satisfies it, so the role model's
// pair-ownership rule is checked against the relationship schema without this
// package knowing the engine's shape.
type Declarer interface{ DeclaresPermission(string, string) bool }
...
// and is omitted by service construction. CompileRoleModel requires a set model.
// Global scope is assignment data, not a second
...
//     list is non-empty, duplicate-free and names only roles the type declares; every declared
...
func CompileRoleModel(model RoleModel, declared Declarer) (*CompiledRoleModel, error) {
	...
	for _, typeName := range sortedKeys(model.ResourceTypes) {
		...
		if err := ValidateRefField("resource type", typeName); err != nil {
		...
		for _, roleName := range def.Roles {
			if err := ValidateRefField("role", roleName); err != nil {
		...
		for _, permission := range sortedKeys(def.Permissions) {
			if err := ValidateRefField("permission", permission); err != nil {
				return nil, fmt.Errorf("%w: resource type %q: %s", ErrInvalidRoleModel, typeName, err)
			}
			...
			for _, roleName := range names {
				...
				if seen[roleName] {
					return nil, fmt.Errorf("%w: permission %q on %q lists granting role %q twice", ErrInvalidRoleModel, permission, typeName, roleName)
				}
				seen[roleName] = true
	...
	if err := compiled.ValidateDisjoint(declared); err != nil {
		return nil, err
	}
...
// ValidateDisjoint rejects any permission pair owned by both this model and declared.
// It also validates a compiled role model supplied to a separately assembled mutation service.
func (m *CompiledRoleModel) ValidateDisjoint(declared Declarer) error {
	if m == nil {
		return nil
	}
	if declared != nil {
		for _, typeName := range sortedKeys(m.resourceTypes) {
			for _, permission := range sortedKeys(m.resourceTypes[typeName].grantors) {
				if declared.DeclaresPermission(typeName, permission) {
					return fmt.Errorf("%w: permission %q on %q is declared by both the relationship schema and the role model",
...
// Grantors returns a sorted copy of the roles that grant the permission,
// or nil when the pair is undeclared.
func (m *CompiledRoleModel) Grantors(resourceType, permission string) []string {
	...
	return append([]string(nil), rt.grantors[permission]...)
}
