...
func newServiceWith(t *testing.T, store relationship.Storer, schema Schema, limits EvaluationLimits) *Service {
	t.Helper()
	svc, err := NewService(store, schema, Config{Limits: limits})
	if err != nil {
...
func walkPages(t *testing.T, svc *Service, principal PrincipalRef, permission, resourceType string, limit int) []string {
	t.Helper()
	all := []string{}
	after := ""
	for pages := 0; ; pages++ {
...
func assertPagedParity(t *testing.T, svc *Service, principal PrincipalRef, permission, resourceType string, want []string) {
	t.Helper()
...
func TestLookupResourcesPageParityDirect(t *testing.T) {
	...
	assertPagedParity(t, svc, PrincipalRef{Type: "user", ID: "u1"}, "delete", "post",
...
func TestLookupResourcesPageParityDirectAndThroughOverlap(t *testing.T) {
	...
	assertPagedParity(t, svc, PrincipalRef{Type: "user", ID: "u1"}, "view", "post",
...
func TestLookupResourcesPageParitySelfHierarchy(t *testing.T) {
	store := &fakeStore{}
	svc := newServiceWith(t, store, hierarchySchema(), EvaluationLimits{})
	store.tuples = append(store.tuples,
		relationship.CreateRelationship{ResourceType: "space", ResourceID: "z_root", Relation: "viewer", SubjectType: "user", SubjectID: "u1"},
		relationship.CreateRelationship{ResourceType: "space", ResourceID: "a_child", Relation: "parent", SubjectType: "space", SubjectID: "z_root"},
		relationship.CreateRelationship{ResourceType: "space", ResourceID: "m_mid", Relation: "parent", SubjectType: "space", SubjectID: "z_root"},
		relationship.CreateRelationship{ResourceType: "space", ResourceID: "f_x", Relation: "folder", SubjectType: "space", SubjectID: "m_mid"},
		relationship.CreateRelationship{ResourceType: "space", ResourceID: "b_deep", Relation: "parent", SubjectType: "space", SubjectID: "f_x"},
	)
	assertPagedParity(t, svc, PrincipalRef{Type: "user", ID: "u1"}, "view", "space",
...
func TestLookupResourcesPageEmptyResultIsNonNil(t *testing.T) {
	...
	for _, permission := range []string{"delete", "fly"} { // declared, and declared by nobody
		res, err := svc.LookupResourcesPage(context.Background(), PrincipalRef{Type: "user", ID: "nobody"}, permission, "post", "", 5)
		if err != nil {
...
func TestLookupResourcesPageIntermediateOverflowOnEveryPage(t *testing.T) {
	store := &fakeStore{}
	svc := newServiceWith(t, store, testSchema(), EvaluationLimits{MaxLookupResults: 2})
	for _, org := range []string{"o1", "o2", "o3"} {
		store.tuples = append(store.tuples,
			relationship.CreateRelationship{ResourceType: "org", ResourceID: org, Relation: "admin", SubjectType: "user", SubjectID: "u1"},
			relationship.CreateRelationship{ResourceType: "post", ResourceID: "p_" + org, Relation: "org", SubjectType: "org", SubjectID: org},
		)
	}
	for _, after := range []string{"", "p_o1"} {
		_, err := svc.LookupResourcesPage(context.Background(), PrincipalRef{Type: "user", ID: "u1"}, "view", "post", after, 1)
...
func TestLookupResourcesPageHierarchyRootOverflowOnEveryPage(t *testing.T) {
	store := &fakeStore{}
	svc := newServiceWith(t, store, hierarchySchema(), EvaluationLimits{MaxLookupResults: 2})
	...
	for _, after := range []string{"", "s1"} {
		_, err := svc.LookupResourcesPage(context.Background(), PrincipalRef{Type: "user", ID: "u1"}, "view", "space", after, 1)
		if !errors.Is(err, ErrEvaluationLimit) {
...
func TestLookupResourcesPagePushesAfterAndPageSizeToTheStore(t *testing.T) {
	...
	if _, err := svc.LookupResourcesPage(context.Background(), PrincipalRef{Type: "user", ID: "u1"}, "view", "post", "p0", 2); err != nil {
...
func TestLookupResourcesPageCancellationBeforeTheFirstStoreCall(t *testing.T) {
	inner := &fakeStore{}
	inner.tuples = append(inner.tuples, relationship.CreateRelationship{
	...
	cancel()
	if _, err := svc.LookupResourcesPage(ctx, PrincipalRef{Type: "user", ID: "u1"}, "delete", "post", "", 2); !errors.Is(err, context.Canceled) {
