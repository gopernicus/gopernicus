...
func TestRegister(t *testing.T) {
	...
	svc := comps.Service
	// With a logger.
	if err := svc.Register(pocket.Mount{Logger: slog.Default()}); err != nil {
		t.Fatalf("Register with logger: %v", err)
	}
	// Zero-value Mount (nil logger) is tolerated.
	if err := svc.Register(pocket.Mount{}); err != nil {
...
func TestRegisterLogsRoleModelPresence(t *testing.T) {
	for name, tc := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			if err := comps.Service.Register(pocket.Mount{Logger: slog.New(slog.NewJSONHandler(&buf, nil))}); err != nil {
