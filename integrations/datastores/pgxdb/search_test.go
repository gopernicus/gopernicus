...
func TestAddSearchClauseSQL(t *testing.T) {
	...
	t.Run("writes WHERE on a bare base", func(t *testing.T) {
		...
		got := buf.String()
		want := "SELECT * FROM (\nSELECT id, created_at, name, description FROM widgets\n) AS list_source WHERE " + `(("name" COLLATE "C") ILIKE @list_search ESCAPE '\' OR ("description" COLLATE "C") ILIKE @list_search ESCAPE '\')`
		if got != want {
			t.Errorf("SQL =\n%s\nwant\n%s", got, want)
		}
	...
	t.Run("preserves an existing filter", func(t *testing.T) {
		var buf strings.Builder
		buf.WriteString("SELECT id, created_at, name, description FROM widgets WHERE tenant_id = @tenant")
		args := pgx.NamedArgs{"tenant": "t1"}

		if err := AddSearchClause(&buf, args, fields, "gear"); err != nil {
			t.Fatalf("AddSearchClause: %v", err)
		}
		if !strings.Contains(buf.String(), "WHERE tenant_id = @tenant\n) AS list_source WHERE ((") {
	...
	t.Run("blank term is a no-op", func(t *testing.T) {
		for _, term := range []string{"", "   "} {
			...
			if buf.String() != "SELECT id, created_at, name, description FROM widgets" {
	...
	t.Run("a pre-existing reserved argument fails rather than being overwritten", func(t *testing.T) {
		var buf strings.Builder
		buf.WriteString("SELECT id, created_at, name, description FROM widgets WHERE note = @" + searchArg)
...
func TestWithSearchDoesNotMutateCaller(t *testing.T) {
	...
	if original.BaseSQL != "SELECT id, created_at, name, description FROM widgets" {
...
// A blank search still puts ORDER BY in the projected scope, without changing
// the caller or adding a search argument.
func TestWithSearchBlankUsesProjectedScope(t *testing.T) {
	original := ListQuery[struct{}]{
		BaseSQL: "SELECT id, created_at, name, description FROM widgets",
		Args:    pgx.NamedArgs{"tenant": "t1"},
	}
	...
	want := "SELECT * FROM (\nSELECT id, created_at, name, description FROM widgets\n) AS list_source"
	if derived.BaseSQL != want {
		t.Errorf("BaseSQL=%q, want %q", derived.BaseSQL, want)
	}
	if original.BaseSQL != "SELECT id, created_at, name, description FROM widgets" || len(derived.Args) != 1 || derived.Args["tenant"] != "t1" {
		t.Fatalf("blank search changed the original query or arguments: original=%+v derived=%+v", original, derived)
	}
}
...
func TestWithSearchReachesEveryQueryPath(t *testing.T) {
	...
	// Cursor filtering wraps the searched SELECT and preserves its predicate.
	var buf strings.Builder
	buf.WriteString(searched.BaseSQL)
	args := cloneArgs(searched.Args)
	if err := ApplyCursorPagination(&buf, args, "created_at", "id", "2026-01-01T00:00:00Z", "abc", list.DESC, false, false); err != nil {
	...
	if !strings.Contains(buf.String(), searched.BaseSQL+"\n) AS list_source WHERE (") {
		t.Errorf("the cursor predicate did not preserve the searched SELECT:\n%s", buf.String())
	}
}
