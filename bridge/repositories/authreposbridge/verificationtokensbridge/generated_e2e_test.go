...
// setupE2EServer boots the full HTTP stack for VerificationToken against a fresh
...
// seededCreateVerificationTokenRequest builds a valid create request, populating
// foreign-key fields from parent rows seeded into db.
func seededCreateVerificationTokenRequest(t *testing.T, db e2eDB) CreateVerificationTokenRequest {
	t.Helper()
	_ = db
	return validCreateVerificationTokenRequest()
}

// The suite runs authenticated — one anonymous request must still be
// rejected, proving enforcement is live in this exact stack.
func TestE2EVerificationTokenRequiresAuthentication(t *testing.T) {
	client, db := setupE2EServer(t)
	anon := client.Anonymous()
	anon.Post(t, "/verification-tokens", seededCreateVerificationTokenRequest(t, db)).RequireStatus(t, 401)
}

func TestE2EVerificationTokenCreateAndGet(t *testing.T) {
	client, db := setupE2EServer(t)

	resp := client.Post(t, "/verification-tokens", seededCreateVerificationTokenRequest(t, db))
	resp.RequireStatus(t, 201)
	id := resp.String(t, "record.token_id")
	require.NotEmpty(t, id)

	got := client.Get(t, "/verification-tokens/"+id)
	got.RequireStatus(t, 200)
	assert.Equal(t, id, got.String(t, "record.token_id"))

	t.Run("not found", func(t *testing.T) {
		id := "nonexistent-e2e-id"
		client.Get(t, "/verification-tokens/"+id).RequireStatus(t, 403)
	})
}

func TestE2EVerificationTokenList(t *testing.T) {
	client, db := setupE2EServer(t)

	client.Post(t, "/verification-tokens", seededCreateVerificationTokenRequest(t, db)).RequireStatus(t, 201)
	client.Get(t, "/verification-tokens").RequireStatus(t, 200)
}

func TestE2EVerificationTokenPaginationAdvance(t *testing.T) {
	client, db := setupE2EServer(t)
	...
	for i := 0; i < 3; i++ {
		...
		raw, err := json.Marshal(seededCreateVerificationTokenRequest(t, db))
		...
		client.Post(t, "/verification-tokens", body).RequireStatus(t, 201)
	}

	page1 := client.Get(t, "/verification-tokens?limit=2")
	...
	page2 := client.Get(t, "/verification-tokens?limit=2&cursor="+url.QueryEscape(cursor))
	page2.RequireStatus(t, 200)

	seen := map[string]bool{}
	for _, row := range page1.Data(t) {
		if m, ok := row.(map[string]any); ok {
			seen[toStr(m["token_id"])] = true
		}
	}
	advanced := false
...
func TestE2EVerificationTokenDelete(t *testing.T) {
	client, db := setupE2EServer(t)
	...
	client.Delete(t, "/verification-tokens/"+id).RequireStatus(t, 204)
	client.Get(t, "/verification-tokens/"+id).RequireStatus(t, 403)
}

func TestE2EVerificationTokenSQLInjection(t *testing.T) {
	client, db := setupE2EServer(t)
	...
	stringParams := []string{"token_id", "token_hash", "purpose", "identifier", "user_id", "s"}
	for _, param := range stringParams {
		for _, payload := range payloads {
			resp := client.Get(t, "/verification-tokens?"+url.Values{param: []string{payload}}.Encode())
	...
	otherParams := []string{"expires_at", "created_at", "order", "limit", "cursor"}
...
func TestE2EVerificationTokenOversizedPayload(t *testing.T) {
	client, db := setupE2EServer(t)
	...
	client.Post(t, "/verification-tokens", body).RequireStatus(t, 413)
}
