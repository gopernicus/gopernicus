...
// setupSecurityServer boots the full HTTP stack for VerificationToken WITH the
...
func setupSecurityServer(t *testing.T) *testhttp.Client {
	...
	store := verificationtokenspgx.NewStore(logger.NewNoop(), db.Pool)
	repo := verificationtokens.NewRepository(verificationtokens.NewCacheStore(store, nil))
