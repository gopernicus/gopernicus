...
// setupSecurityServer boots the full HTTP stack for APIKey WITH the
...
func setupSecurityServer(t *testing.T) *testhttp.Client {
	...
	store := apikeyspgx.NewStore(logger.NewNoop(), db.Pool)
	repo := apikeys.NewRepository(apikeys.NewCacheStore(store, nil))
