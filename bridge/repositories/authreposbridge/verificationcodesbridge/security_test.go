...
// setupSecurityServer boots the full HTTP stack for VerificationCode WITH the
...
func setupSecurityServer(t *testing.T) *testhttp.Client {
	...
	store := verificationcodespgx.NewStore(logger.NewNoop(), db.Pool)
	repo := verificationcodes.NewRepository(verificationcodes.NewCacheStore(store, nil))
