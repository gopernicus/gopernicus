...
// setupE2EServer boots the full HTTP stack for VerificationCode against a fresh
...
// seededCreateVerificationCodeRequest builds a valid create request, populating
// foreign-key fields from parent rows seeded into db.
func seededCreateVerificationCodeRequest(t *testing.T, db e2eDB) CreateVerificationCodeRequest {
	t.Helper()
	_ = db
	return validCreateVerificationCodeRequest()
}

// The suite runs authenticated — one anonymous request must still be
// rejected, proving enforcement is live in this exact stack.
func TestE2EVerificationCodeRequiresAuthentication(t *testing.T) {
	client, db := setupE2EServer(t)
	anon := client.Anonymous()
	anon.Post(t, "/verification-codes", seededCreateVerificationCodeRequest(t, db)).RequireStatus(t, 401)
}

func TestE2EVerificationCodeCreateAndGet(t *testing.T) {
	client, db := setupE2EServer(t)

	resp := client.Post(t, "/verification-codes", seededCreateVerificationCodeRequest(t, db))
	resp.RequireStatus(t, 201)
	id := resp.String(t, "record.code_id")
	require.NotEmpty(t, id)

	got := client.Get(t, "/verification-codes/"+id)
	got.RequireStatus(t, 200)
	assert.Equal(t, id, got.String(t, "record.code_id"))

	t.Run("not found", func(t *testing.T) {
		id := "nonexistent-e2e-id"
		client.Get(t, "/verification-codes/"+id).RequireStatus(t, 403)
	})
}

func TestE2EVerificationCodeList(t *testing.T) {
	client, db := setupE2EServer(t)

	client.Post(t, "/verification-codes", seededCreateVerificationCodeRequest(t, db)).RequireStatus(t, 201)
	client.Get(t, "/verification-codes").RequireStatus(t, 200)
}

func TestE2EVerificationCodePaginationAdvance(t *testing.T) {
	client, db := setupE2EServer(t)
	...
	for i := 0; i < 3; i++ {
		...
		raw, err := json.Marshal(seededCreateVerificationCodeRequest(t, db))
		...
		if s, ok := body["purpose"].(string); ok {
			body["purpose"] = s + "-" + strconv.Itoa(i)
		}
		client.Post(t, "/verification-codes", body).RequireStatus(t, 201)
	}

	page1 := client.Get(t, "/verification-codes?limit=2")
	...
	page2 := client.Get(t, "/verification-codes?limit=2&cursor="+url.QueryEscape(cursor))
	page2.RequireStatus(t, 200)

	seen := map[string]bool{}
	for _, row := range page1.Data(t) {
		if m, ok := row.(map[string]any); ok {
			seen[toStr(m["code_id"])] = true
		}
	}
	advanced := false
...
func TestE2EVerificationCodeDelete(t *testing.T) {
	client, db := setupE2EServer(t)
	...
	client.Delete(t, "/verification-codes/"+id).RequireStatus(t, 204)
	client.Get(t, "/verification-codes/"+id).RequireStatus(t, 403)
}

func TestE2EVerificationCodeSQLInjection(t *testing.T) {
	client, db := setupE2EServer(t)
	...
	stringParams := []string{"code_id", "identifier", "code_hash", "purpose", "user_id", "s"}
	for _, param := range stringParams {
		for _, payload := range payloads {
			resp := client.Get(t, "/verification-codes?"+url.Values{param: []string{payload}}.Encode())
	...
	otherParams := []string{"data", "attempt_count", "expires_at", "created_at", "order", "limit", "cursor"}
...
func TestE2EVerificationCodeOversizedPayload(t *testing.T) {
	client, db := setupE2EServer(t)
	...
	client.Post(t, "/verification-codes", body).RequireStatus(t, 413)
}
