...
// setupSecurityServer boots the full HTTP stack for OauthAccount WITH the
...
func setupSecurityServer(t *testing.T) *testhttp.Client {
	...
	store := oauthaccountspgx.NewStore(logger.NewNoop(), db.Pool)
	repo := oauthaccounts.NewRepository(oauthaccounts.NewCacheStore(store, nil))
