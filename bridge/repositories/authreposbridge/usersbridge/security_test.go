...
// setupSecurityServer boots the full HTTP stack for User WITH the
...
func setupSecurityServer(t *testing.T) *testhttp.Client {
	...
	store := userspgx.NewStore(logger.NewNoop(), db.Pool)
	repo := users.NewRepository(users.NewCacheStore(store, nil))
