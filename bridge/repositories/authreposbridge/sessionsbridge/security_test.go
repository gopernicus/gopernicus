...
// setupSecurityServer boots the full HTTP stack for Session WITH the
...
func setupSecurityServer(t *testing.T) *testhttp.Client {
	...
	store := sessionspgx.NewStore(logger.NewNoop(), db.Pool)
	repo := sessions.NewRepository(sessions.NewCacheStore(store, nil))
