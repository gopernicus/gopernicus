...
// setupE2EServer boots the full HTTP stack for Principal against a fresh
...
// seededCreatePrincipalRequest builds a valid create request, populating
// foreign-key fields from parent rows seeded into db.
func seededCreatePrincipalRequest(t *testing.T, db e2eDB) CreatePrincipalRequest {
	t.Helper()
	_ = db
	return validCreatePrincipalRequest()
}

// The suite runs authenticated — one anonymous request must still be
// rejected, proving enforcement is live in this exact stack.
func TestE2EPrincipalRequiresAuthentication(t *testing.T) {
	client, db := setupE2EServer(t)
	anon := client.Anonymous()
	anon.Post(t, "/principals", seededCreatePrincipalRequest(t, db)).RequireStatus(t, 401)
}

func TestE2EPrincipalCreateAndGet(t *testing.T) {
	client, db := setupE2EServer(t)

	resp := client.Post(t, "/principals", seededCreatePrincipalRequest(t, db))
	resp.RequireStatus(t, 201)
	id := resp.String(t, "record.principal_id")
	require.NotEmpty(t, id)

	got := client.Get(t, "/principals/"+id)
	got.RequireStatus(t, 200)
	assert.Equal(t, id, got.String(t, "record.principal_id"))

	t.Run("not found", func(t *testing.T) {
		id := "nonexistent-e2e-id"
		client.Get(t, "/principals/"+id).RequireStatus(t, 403)
	})
}

func TestE2EPrincipalList(t *testing.T) {
	client, db := setupE2EServer(t)

	client.Post(t, "/principals", seededCreatePrincipalRequest(t, db)).RequireStatus(t, 201)
	client.Get(t, "/principals").RequireStatus(t, 200)
}

func TestE2EPrincipalPaginationAdvance(t *testing.T) {
	client, db := setupE2EServer(t)
	...
	page1 := client.Get(t, "/principals?limit=2")
	...
	page2 := client.Get(t, "/principals?limit=2&cursor="+url.QueryEscape(cursor))
	page2.RequireStatus(t, 200)

	seen := map[string]bool{}
	for _, row := range page1.Data(t) {
		if m, ok := row.(map[string]any); ok {
			seen[toStr(m["principal_id"])] = true
		}
	}
	advanced := false
...
func TestE2EPrincipalDelete(t *testing.T) {
	client, db := setupE2EServer(t)
	...
	client.Delete(t, "/principals/"+id).RequireStatus(t, 204)
	client.Get(t, "/principals/"+id).RequireStatus(t, 403)
}

func TestE2EPrincipalSQLInjection(t *testing.T) {
	client, db := setupE2EServer(t)
	...
	stringParams := []string{"principal_id", "s"}
	for _, param := range stringParams {
		for _, payload := range payloads {
			resp := client.Get(t, "/principals?"+url.Values{param: []string{payload}}.Encode())
...
func TestE2EPrincipalOversizedPayload(t *testing.T) {
	client, db := setupE2EServer(t)
	...
	raw, err := json.Marshal(seededCreatePrincipalRequest(t, db))
	require.NoError(t, err)
	var body map[string]any
	require.NoError(t, json.Unmarshal(raw, &body))
	body["_padding"] = strings.Repeat("A", 1048576+1)

	client.Post(t, "/principals", body).RequireStatus(t, 413)
}
