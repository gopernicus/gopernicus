...
// setupSecurityServer boots the full HTTP stack for Principal WITH the
...
func setupSecurityServer(t *testing.T) *testhttp.Client {
	...
	store := principalspgx.NewStore(logger.NewNoop(), db.Pool)
	repo := principals.NewRepository(principals.NewCacheStore(store, nil))
