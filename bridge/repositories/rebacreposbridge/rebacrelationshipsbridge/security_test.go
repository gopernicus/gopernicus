...
// setupSecurityServer boots the full HTTP stack for RebacRelationship WITH the
...
func setupSecurityServer(t *testing.T) *testhttp.Client {
	...
	store := rebacrelationshipspgx.NewStore(logger.NewNoop(), db.Pool)
	repo := rebacrelationships.NewRepository(rebacrelationships.NewCacheStore(store, nil))
