...
// setupE2EServer boots the full HTTP stack for RebacRelationship against a fresh
...
// seededCreateRebacRelationshipRequest builds a valid create request, populating
// foreign-key fields from parent rows seeded into db.
func seededCreateRebacRelationshipRequest(t *testing.T, db e2eDB) CreateRebacRelationshipRequest {
	t.Helper()
	_ = db
	return validCreateRebacRelationshipRequest()
}

// The suite runs authenticated — one anonymous request must still be
// rejected, proving enforcement is live in this exact stack.
func TestE2ERebacRelationshipRequiresAuthentication(t *testing.T) {
	client, db := setupE2EServer(t)
	anon := client.Anonymous()
	anon.Post(t, "/rebac-relationships", seededCreateRebacRelationshipRequest(t, db)).RequireStatus(t, 401)
}

func TestE2ERebacRelationshipCreateAndGet(t *testing.T) {
	client, db := setupE2EServer(t)

	resp := client.Post(t, "/rebac-relationships", seededCreateRebacRelationshipRequest(t, db))
	resp.RequireStatus(t, 201)
	id := resp.String(t, "record.relationship_id")
	require.NotEmpty(t, id)

	got := client.Get(t, "/rebac-relationships/"+id)
	got.RequireStatus(t, 200)
	assert.Equal(t, id, got.String(t, "record.relationship_id"))

	t.Run("not found", func(t *testing.T) {
		id := "nonexistent-e2e-id"
		client.Get(t, "/rebac-relationships/"+id).RequireStatus(t, 403)
	})
}

func TestE2ERebacRelationshipList(t *testing.T) {
	client, db := setupE2EServer(t)

	client.Post(t, "/rebac-relationships", seededCreateRebacRelationshipRequest(t, db)).RequireStatus(t, 201)
	client.Get(t, "/rebac-relationships").RequireStatus(t, 200)
}

func TestE2ERebacRelationshipPaginationAdvance(t *testing.T) {
	client, db := setupE2EServer(t)
	...
	for i := 0; i < 3; i++ {
		...
		raw, err := json.Marshal(seededCreateRebacRelationshipRequest(t, db))
		...
		if s, ok := body["subject_type"].(string); ok {
			body["subject_type"] = s + "-" + strconv.Itoa(i)
		}
		if s, ok := body["subject_id"].(string); ok {
			body["subject_id"] = s + "-" + strconv.Itoa(i)
		}
		client.Post(t, "/rebac-relationships", body).RequireStatus(t, 201)
	}

	page1 := client.Get(t, "/rebac-relationships?limit=2")
	...
	page2 := client.Get(t, "/rebac-relationships?limit=2&cursor="+url.QueryEscape(cursor))
	page2.RequireStatus(t, 200)

	seen := map[string]bool{}
	for _, row := range page1.Data(t) {
		if m, ok := row.(map[string]any); ok {
			seen[toStr(m["relationship_id"])] = true
		}
	}
	advanced := false
...
func TestE2ERebacRelationshipDelete(t *testing.T) {
	client, db := setupE2EServer(t)
	...
	client.Delete(t, "/rebac-relationships/"+id).RequireStatus(t, 204)
	client.Get(t, "/rebac-relationships/"+id).RequireStatus(t, 403)
}

func TestE2ERebacRelationshipSQLInjection(t *testing.T) {
	client, db := setupE2EServer(t)
	...
	stringParams := []string{"relationship_id", "resource_type", "resource_id", "relation", "subject_type", "subject_id", "subject_relation", "s"}
	for _, param := range stringParams {
		for _, payload := range payloads {
			resp := client.Get(t, "/rebac-relationships?"+url.Values{param: []string{payload}}.Encode())
	...
	otherParams := []string{"created_at", "order", "limit", "cursor"}
...
func TestE2ERebacRelationshipOversizedPayload(t *testing.T) {
	client, db := setupE2EServer(t)
	...
	client.Post(t, "/rebac-relationships", body).RequireStatus(t, 413)
}
