...
// setupSecurityServer boots the full HTTP stack for RebacRelationshipMetadata WITH the
...
func setupSecurityServer(t *testing.T) *testhttp.Client {
	...
	store := rebacrelationshipmetadatapgx.NewStore(logger.NewNoop(), db.Pool)
	repo := rebacrelationshipmetadata.NewRepository(rebacrelationshipmetadata.NewCacheStore(store, nil))
