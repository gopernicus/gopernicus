...
// setupE2EServer boots the full HTTP stack for Group against a fresh
...
// seededCreateGroupRequest builds a valid create request, populating
// foreign-key fields from parent rows seeded into db.
func seededCreateGroupRequest(t *testing.T, db e2eDB) CreateGroupRequest {
	t.Helper()
	ctx := context.Background()
...
func TestE2EGroupRequiresAuthentication(t *testing.T) {
	client, db := setupE2EServer(t)
	anon := client.Anonymous()
	anon.Post(t, "/groups", seededCreateGroupRequest(t, db)).RequireStatus(t, 401)
}

func TestE2EGroupCreateAndGet(t *testing.T) {
	client, db := setupE2EServer(t)

	resp := client.Post(t, "/groups", seededCreateGroupRequest(t, db))
	resp.RequireStatus(t, 201)
	id := resp.String(t, "record.group_id")
	require.NotEmpty(t, id)

	got := client.Get(t, "/groups/"+id)
	got.RequireStatus(t, 200)
	assert.Equal(t, id, got.String(t, "record.group_id"))

	t.Run("not found", func(t *testing.T) {
		id := "nonexistent-e2e-id"
		client.Get(t, "/groups/"+id).RequireStatus(t, 403)
	})
}

func TestE2EGroupList(t *testing.T) {
	client, db := setupE2EServer(t)

	client.Post(t, "/groups", seededCreateGroupRequest(t, db)).RequireStatus(t, 201)
	client.Get(t, "/groups").RequireStatus(t, 200)
}

func TestE2EGroupPaginationAdvance(t *testing.T) {
	client, db := setupE2EServer(t)
	...
	page1 := client.Get(t, "/groups?limit=2")
	...
	page2 := client.Get(t, "/groups?limit=2&cursor="+url.QueryEscape(cursor))
	page2.RequireStatus(t, 200)

	seen := map[string]bool{}
	for _, row := range page1.Data(t) {
		if m, ok := row.(map[string]any); ok {
			seen[toStr(m["group_id"])] = true
		}
	}
	advanced := false
...
func TestE2EGroupDelete(t *testing.T) {
	client, db := setupE2EServer(t)
	...
	client.Delete(t, "/groups/"+id).RequireStatus(t, 204)
	client.Get(t, "/groups/"+id).RequireStatus(t, 403)
}

func TestE2EGroupSQLInjection(t *testing.T) {
	client, db := setupE2EServer(t)
	...
	stringParams := []string{"group_id", "name", "slug", "description", "creator_principal_id", "s"}
	for _, param := range stringParams {
		for _, payload := range payloads {
			resp := client.Get(t, "/groups?"+url.Values{param: []string{payload}}.Encode())
...
func TestE2EGroupOversizedPayload(t *testing.T) {
	client, db := setupE2EServer(t)
	...
	raw, err := json.Marshal(seededCreateGroupRequest(t, db))
	require.NoError(t, err)
	var body map[string]any
	require.NoError(t, json.Unmarshal(raw, &body))
	body["_padding"] = strings.Repeat("A", 1048576+1)

	client.Post(t, "/groups", body).RequireStatus(t, 413)
}

func TestE2EGroupMassAssignment(t *testing.T) {
	client, db := setupE2EServer(t)
...
func TestE2EGroupUpdateMassAssignment(t *testing.T) {
	client, db := setupE2EServer(t)
	...
	client.Put(t, "/groups/"+id, map[string]any{
		"name":         "edited-value",
