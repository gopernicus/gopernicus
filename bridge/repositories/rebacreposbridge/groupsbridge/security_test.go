...
// setupSecurityServer boots the full HTTP stack for Group WITH the
...
func setupSecurityServer(t *testing.T) *testhttp.Client {
	...
	store := groupspgx.NewStore(logger.NewNoop(), db.Pool)
	repo := groups.NewRepository(groups.NewCacheStore(store, nil))
