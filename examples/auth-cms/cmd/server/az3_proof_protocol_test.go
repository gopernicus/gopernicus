...
func (t *proofTranscript) render() string {
	...
	b.WriteString("authzIntegrityPolicy, inbound access policy and the principal-free tuple writer)\n")
	b.WriteString("with optional atomic audit recording enabled.\n\n")
	b.WriteString("History contains committed fact changes, including trusted setup. Inbound denials, no-ops and failed writes add no entries.\n\n")
...
func proofComposition(t *testing.T, schema decisions.Model, history *capturedHistory) authorization.Components {
	...
	policy := mutations.IntegrityPolicy{}
	if _, ok := schema.ResourceTypes[demoResourceType]; ok {
		policy = authzIntegrityPolicy()
	}
	store := authzmem.New(authzmem.WithIntegrityPolicy(policy), authzmem.WithAudit())
	if history != nil {
...
func trustGrant(t *testing.T, sm *mutations.Service, rt, rid, rel string, s relationships.SubjectRef) {
	t.Helper()
...
func proofActor(userID string) sdk.Principal { return actor(userID) }
...
func proofPoint01(t *testing.T, tr *proofTranscript) {
	...
	_, err := admitGrantRelationship(ctx, svc, proofActor("ordinary"), mutations.GrantRelationshipCommand{
	...
	tr.add(1, "Ordinary member cannot self-grant",
		"command: inbound admission then GrantRelationship(principal=user:ordinary, grant project:p1#owner <- user:ordinary)",
		fmt.Sprintf("result: rejected, errors.Is(sdk.ErrForbidden)=%v (inbound denied before Apply)", errors.Is(err, sdk.ErrForbidden)),
...
func proofPoint02(t *testing.T, tr *proofTranscript) {
	...
	rcpt, err := admitGrantRelationship(ctx, svc, proofActor("manager"), mutations.GrantRelationshipCommand{
	...
	tr.add(2, "Authorized manager grants an exact group#member subject",
		"command: inbound admission then GrantRelationship(principal=user:manager, grant doc:d2#viewer <- group:eng#member)",
...
func proofPoint03(t *testing.T, tr *proofTranscript) {
	...
	tr.add(3, "A group#admin grant does not authorize an ordinary member",
		"command: inbound admission then GrantRelationship(principal=user:manager, grant doc:d3#viewer <- group:eng#admin)",
		fmt.Sprintf("result: outcome=%s", rcpt.Outcome),
...
func proofPoint04(t *testing.T, tr *proofTranscript) {
	...
	if _, err := admitGrantRelationship(ctx, svc, proofActor("manager"), mutations.GrantRelationshipCommand{
...
func proofPoint07(t *testing.T, tr *proofTranscript) {
	...
	if _, err := comps.Mutations.AssignRole(ctx, mutations.AssignRoleCommand{Subject: p, Role: "auditor", Scope: tuples.Global()}); err != nil {
		t.Fatal(err)
	}
...
func proofPoint08(t *testing.T, tr *proofTranscript) {
	...
	trustGrant(t, comps.Mutations, "project", "p8", "owner", subj("user", "manager"))
	for _, scope := range []tuples.Scope{tuples.Global(), tuples.On("project", "p8")} {
		if _, err := comps.Mutations.AssignRole(ctx, mutations.AssignRoleCommand{Subject: p, Role: "auditor", Scope: scope}); err != nil {
			t.Fatal(err)
		}
	}
	result, err := admitUnassignRole(ctx, comps, proofActor("manager"), mutations.UnassignRoleCommand{Subject: p, Role: "auditor", Scope: tuples.On("project", "p8")})
...
func proofPoint09(t *testing.T, tr *proofTranscript) {
	...
	for r := 0; r < rounds; r++ {
		...
		revoke := func(i int, ownerID string) {
			defer wg.Done()
			rcpt, err := admitRevokeRelationship(ctx, svc, proofActor("admin"), mutations.RevokeRelationshipCommand{
...
func proofPoint10(t *testing.T, tr *proofTranscript) {
	...
	trustGrant(t, comps.Mutations, "project", "p10", "owner", subj("user", "manager"))
	before := len(history.all())
	result, err := admitGrantRelationship(context.Background(), comps, proofActor("manager"), mutations.GrantRelationshipCommand{ResourceType: "project", ResourceID: "p10", Relation: "member", Subject: subj("user", "manager")})
...
func proofPoint11(t *testing.T, tr *proofTranscript) {
	history := &capturedHistory{}
	comps := proofComposition(t, authzSchema(), history)
	trustGrant(t, comps.Mutations, "project", "p11", "owner", subj("user", "manager"))
	...
	for _, want := range []mutations.Outcome{mutations.OutcomeApplied, mutations.OutcomeNoChange} {
		result, err := admitGrantRelationship(context.Background(), comps, proofActor("manager"), cmd)
...
func proofPoint12(t *testing.T, tr *proofTranscript) {
	...
	// Generic Apply cannot bypass the explicit reason-bearing teardown method.
	_, genericErr := sm.Apply(proofSystemContext(), mutations.Command{Target: mutations.Target{Kind: mutations.TargetResource, Type: "project", ID: "p12"}, Operation: mutations.OpTeardown})
	if !errors.Is(genericErr, mutations.ErrTeardownViaTypedMethod) {
		t.Fatalf("generic teardown: %v", genericErr)
	}
	...
	tr.add(12, "Resource teardown requires an explicit command with a recorded reason",
		fmt.Sprintf("generic Apply rejects teardown=%v; the explicit method requires a reason", errors.Is(genericErr, mutations.ErrTeardownViaTypedMethod)),
		...
		"command: Mutations.TeardownResourceAuthorization(project:p12, reason=<recorded>)",
	...
	tr.add(12, "Resource teardown requires an explicit command with a recorded reason", auditLines(sink.all())...)
}
