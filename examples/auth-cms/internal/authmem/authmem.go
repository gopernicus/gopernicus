...
func (r userRepo) CreateWithPrimaryIdentifier(ctx context.Context, u user.User, ident identifier.Identifier) (user.User, identifier.Identifier, error) {
	return r.Provision(ctx, u, ident, user.InitialCredentials{})
}

func (r userRepo) Provision(ctx context.Context, u user.User, ident identifier.Identifier, credentials user.InitialCredentials) (user.User, identifier.Identifier, error) {
...
func (r passwordRepo) Set(ctx context.Context, userID, hash string) error {
	if err := ctx.Err(); err != nil {
		return err
	}
	r.mu.Lock()
...
func (r sessionRepo) GetByRefreshHash(_ context.Context, hash string) (session.Session, session.RefreshMatch, error) {
	...
	for _, s := range r.sessions {
		if s.RefreshTokenHash == hash {
			return cloneSession(s), session.RefreshMatchCurrent, nil
		}
	}
	for _, s := range r.sessions {
		if s.PreviousRefreshTokenHash != "" && s.PreviousRefreshTokenHash == hash {
			return cloneSession(s), session.RefreshMatchPrevious, nil
		}
	}
	return session.Session{}, 0, sdk.ErrNotFound
}
...
func cloneSession(s session.Session) session.Session {
	s.Authentication.Methods = append([]session.AuthenticationMethod(nil), s.Authentication.Methods...)
	return s
}
