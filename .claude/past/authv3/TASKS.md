...
- [x] AV3-9.7 — post-implementation full reviewer gate (eight read-only specialist reviews over the one untagged cut — auth-v3 + AV3D + SWP; 41 findings, **0 critical, 4 high** (DATA-1 turso Option-B re-enqueue timestamp poisons every row; SRE-1 no `recover()` in the `in_process` worker → a provider panic crashes the whole host; DOC-1/DOC-2 the SWP `sdk/capabilities/work` package + `features/jobs` signature change absent from RELEASING.md AND the NOTES inventory); no fence-bypass/stale-write path and no view-layer security defect found; all 15 guards green; all seven parked items UPHELD (SWP-3 `bytes.Clone` independently ratified by architecture + backend); cross-cutting escalation by BE-3 + SRE — the lease/generation fence has never run against real pgx/turso concurrency (live legs LOUD-SKIP), now the top residual risk; dispositioned table returned to owner, **zero files edited, no PR**)
- [x] AV3-9.8 — reviewer remediation, reverification, and PR-ready handoff (2026-07-13/14: canonical IX-01..23 table adopted by owner + all seven gates resolved — drain-only, honest non-durable proof host, truthful five-key rotation, §5.8 named-code mapper, markup-only views, `sdk.ErrUnavailable` ADDED, live DSNs authorized; four bounded implementer batches fixed every accepted finding with regressions; Batch 5 gates: `make generate` idempotent, per-module 36/36, guards 15/15, fresh dual-store 10/10, fenced live conformance 41/41 ×2 dialects `-race` (first real-DB fence run — GREEN), eight-proof livedelivery green ×4 (two gate-discovered defects root-caused as test/harness bugs — HS256 padding-bit tamper, setup-drain lease races — no product defect, no assertion weakened), IX-11 browser drive 9/9; completion record + final disposition table in 10-docs-and-closeout.md; NO PR/tag/push/commit)
