...
| `identity-resolution/` | sdk/identity Resolver/Info/Address (no User struct in sdk, ever) + NEW sdk/notify (Notifier{Kind,Notify} + Console + MailerBridge; wired set = the host's supported kinds, deny-by-absence) + auth kind-aware invitations (supported-kind predicate with email always-on, two-seam delivery fork, kind-conditional accept, migration 0013 both dialects). Owner-rewritten mid-flight to notifier-first; three gates, each catching a shipping major; close drive: phone token delivered → accepted by token live | 2026-07-10 |
| `sdk-layering/` | the intra-sdk import law, physicalized: root package = KERNEL (errors.go + context.go — errs re-homed as sdk.ErrX, id-context promoted), foundation/ (11 pkgs, root-only flat) + capabilities/ (8 pkgs, never each other) + feature the sanctioned composer; middleware evictions (cacher.Pages, tracing.Middleware), workers tracing glue deleted (YAGNI, reintroduction named); integrations/notify/mailer = the first COMPOSING integration (module 36, taxonomy amended); guards G12 (4-direction prove-can-fail) + G13 → THIRTEEN | 2026-07-10 |
| `middleware-consolidation/` | where-middleware-lives (ARCHITECTURE statement + D-D opposite fail postures): ratelimiter.Middleware relocation (narrow Allower, exact fail-open, silent-by-design), authorization.RequirePermission gate (authorizersvc impl + thin root re-export, registration-time roles-only panic, pure Check fail-closed), web.TrustProxies port from original (rightmost-minus-N, auth's spoofable leftmost-XFF fixed, live spoof-drive), auth-cms builder-under-host-recipe composition, G6 flipped exclusion-style over all non-adapter feature code; CORS/DefaultHeaders pinned as kept host surface | 2026-07-11 |
| `authv3/` | v3 identity milestone (phases 0–9): user_identifiers model (users = id/display_name/auth_revision), atomic challenge/grant/credential/contact-change rails, HMAC protector + keyer, delivery worker, passwordless (OTP + magic link), HTML/templ Views + override system, step-up credential suite, host upgrade runbook (drain-only) executed both dialects, proof-host JSON+HTML+browser transcripts; AV3-9.7 eight-review wave (41 findings, canonical IX-01..23 table) + AV3-9.8 five-batch remediation (same-site Origin fix, logout gate, §5.8 named codes, token-retention rerender, per-job recover, host supervision, purge scheduler, sdk.ErrUnavailable) + reverification incl. the fence's first live pgx/turso concurrency runs (41/41 ×2) and livedelivery ×4 green; completion record in 10-docs-and-closeout.md | 2026-07-12/14 |
