...
| 2 | **`sdk/id` is string-only** (26-char lowercase base32) — Segovia needs int, string, AND uuid identifiers | 2026-07-08 | **CLOSED 2026-07-09** — `03-id-kinds.md` | D5 executed (nanoid `New`/`NewCustom`/`UUID`, original's alphabet bug fixed, ~119.8 bits); D6 port typing + D7 pluggable generator both wait-for-demand with triggers recorded; live-proven (auth-cms drive, 21-char IDs e2e) |
