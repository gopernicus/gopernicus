...
| 2 | **`sdk/id` is string-only** (26-char lowercase base32) — Segovia needs int, string, AND uuid identifiers | 2026-07-08 | **CLOSED 2026-07-09; SUPERSEDED same week** — `03-id-kinds.md` → `04-cryptids-facility.md` | D5 executed (nanoid `New`/`NewCustom`/`UUID`, original's alphabet bug fixed, ~119.8 bits); then phase 04 (2026-07-09) folded `sdk/id` into `sdk/cryptids` (`GenerateFunc`/`IDGenerator`) and D7's demand arrived: `Config.IDs` threaded through auth+cms entity constructors (secrets stay unconditional), `Database` empty-ID convention implemented in bundled stores (0012 migrations + conformance), `google-uuid` integration (V4/V7) ships; serial/int keys parked as future explicit-int-per-resource, no generics |
