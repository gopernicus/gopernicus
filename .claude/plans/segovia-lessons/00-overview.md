...
| 2 | **`sdk/id` is string-only** (26-char lowercase base32) — Segovia needs int, string, AND uuid identifiers | 2026-07-08 | **PLANNED** — `03-id-kinds.md` (cut 2026-07-09 on the owner's ID-strategy direction: nanoid-shape custom length/alphabet like the original, stdlib UUID, no third-party libs) | D5 (generation API) + D6 (port typing = wait-for-demand; segovia v2 verified crud-free and all-string) for ratification in the phase file |
